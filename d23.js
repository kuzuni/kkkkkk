/* 23 훈련팝업 «차분기» — ref(1080x2340) 와 캡처(1080x1920) 를 세로 425 정렬해 나란히 비교한다.
...
const out=await p.evaluate(async([ru,cu,jobs])=>{
  ...
  for(const j of jobs){
    ...
    if(j.job==='vcmp'){ // 세로 대조: ref y0..y1, cap = ref-425
      const a=runs(R,'v',j.x,j.y0,j.y1,tol);
      const c=runs(C,'v',j.x,j.y0-425,j.y1-425,tol);
      res.push({job:'vcmp',x:j.x,ref:a.map(r=>r[0]+'-'+r[1]+'('+r[2]+') '+r[3]),cap:c.map(r=>(r[0]+425)+'-'+(r[1]+425)+'('+r[2]+') '+r[3])});
    }
    if(j.job==='hcmp'){
      const a=runs(R,'h',j.y,j.x0,j.x1,tol);
      const c=runs(C,'h',j.y-425,j.x0,j.x1,tol);
      res.push({job:'hcmp',refy:j.y,ref:a.map(r=>r[0]+'-'+r[1]+'('+r[2]+') '+r[3]),cap:c.map(r=>r[0]+'-'+r[1]+'('+r[2]+') '+r[3])});
    }
    if(j.job==='px'){
      res.push({job:'px',x:j.x,y:j.y,ref:px(R,j.x,j.y),cap:px(C,j.x,j.y-425)});
    }
  }
  return res;
},[uri(REF),uri(CAP),jobs]);
for(const r of out){
  if(r.job==='px'){console.log(`PX x=${r.x} refy=${r.y}  ref=${r.ref}  cap=${r.cap}`);continue;}
  console.log('==== '+(r.job==='vcmp'?('VCOL x='+r.x):('HROW ref_y='+r.refy))+' (cap좌표는 ref기준 +425 환산표기)');
