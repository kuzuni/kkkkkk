...
const BBOX = {
  '05 .wgc'    : [148, 158],
  '53 .bg53-c' : [148, 147],
  /* 12 (2026-08-26, 2차 폴리시 라운드 5회차) — 157×157 → **158×158**.
     비평가 K(엄격 임계 `max<20` 158)·L(50% 서브픽셀 교차 158.2)이 각자 다른 방법으로 같은 값을 냈고,
     4회차 스윕이 «세로에서 얻은 임계값 노이즈 판정을 가로에 일반화한» 것이 오류였다(폭은 `<20` 에서도 ref 158).
     pitch 170 은 `158+12 = 157+13` 로 유지 — 좌단 정렬은 안 건드렸다.
     근거 `docs/review/12-소환결과팝업.md` «통과 후 추가 반영 1건» · 측정표 §12-5. */
  '12 .sm-c'   : [158, 158],
  '69 .ml-i'   : [108, 108],
  '22 .qs-i'   : [106, 106],
  '70 .at-if'  : [138, 138],
  ...
  try {
    ...
    const r = await page.evaluate(() => {
      ...
      const bsnap = (label, frame) => {
        ...
        /* `k` 는 «그 화면에 설정된 배율»(141 이 연 `--ifq-k`). 이것만으로 채점하면 순환 논증이므로
           기대값은 아래 IFQK 표(승인된 값)가 쥐고, k 는 «렌더가 토큰을 정말 따르는가» 를 보는 데 쓴다. */
        out.badges.push({ label, fw: fr.width / sc,
          k: parseFloat(getComputedStyle(frame).getPropertyValue('--ifq-k')),
    ...
    /* 141 (2026-08-26) — 배지 배율은 «전 화면 한 값» 이 아니라 **화면별 입력**(`--ifq-k`)이 됐다.
       ...
       근거·실측은 docs/review/141-*.md. 여기서 «나머지는 .235 그대로» 를 같이 못 박아 둔다 —
       141 이 통일을 깬 것이 아니라 «잉크 크기만» 화면별로 연 것임을 이 게이트가 지킨다.

       12 (2026-08-26, 2차 폴리시 라운드 5회차) — **.235 → .218**. 141 이 연 그 구조를 12 가 쓴 것이다:
       공용 `.ifr>.ifq` 는 건드리지 않고 `#sumw .sm-c{--ifq-k:.218}` 로 자기 화면에서만 덮었다.
       잉크 높이 28 → ref 26(−7.7%), 비평가 4인이 «+8~12% 과대» 로 일치 지적한 건이다.
       근거 `docs/review/12-소환결과팝업.md` · index.html:7358 주석.

       검사는 두 겹이다 — (a) 화면에 «선언된» 배율이 승인된 값인가(아래 표. 몰래 바뀌면 잡힌다),
       (b) 렌더된 `font-size/프레임폭` 이 그 선언을 정말 따르는가(하드코딩 우회를 잡는다).
       (b) 만으로는 순환 논증이고 (a) 만으로는 141 이 연 구조를 다시 닫아 버린다. */
    ...
    for (const b of r.badges) {
      ...
      ck('배지 배율 선언값 ' + b.label, near(b.k, k, 0.0005), b.k + ' (승인값 ' + k + ')');
      ck('배지 글자비율 ' + b.label, near(b.fs / b.fw, b.k, 0.004),
        (b.fs / b.fw).toFixed(4) + ' (선언 --ifq-k ' + b.k + ')');
      ...
      /* 스트로크는 «글자 크기» 기준(`--st-body` × 1em)이라 프레임 폭 대비로 보면 .16 × 배율이다.
         배율이 화면별로 갈렸으므로 이 기대값도 «그 화면에 선언된» 배율을 따라간다 —
         22 는 .16 × .317 = .0507 · 12 는 .16 × .218 = .0349. */
      ck('배지 스트로크비율 ' + b.label, near(b.stroke / b.fw, 0.16 * b.k, 0.004),
        (b.stroke / b.fw).toFixed(4) + ' (기준 ' + (0.16 * b.k).toFixed(4) + ')');
