...
/* ⚑ 747 이관(2026-09-02) — 713 비평 2인의 최대 감점(«바가 하단 테두리에 눌려 붙었다»)을 갚느라
   띠가 98 → **112** 가 됐고 여유 14 는 전부 바 아래로 갔다. 363 이 세운 «띠 한가운데» 는 이제
   «배수 바와 세로 중심이 같다»(둘은 713 에서 같은 값이었다)로 읽는다: 29 + (98 − 56)/2 = **50**.
   중심은 78 로 여전히 바와 같다(`verify713` [B1]). */
const SK_BOT = 50;        // 패널 하변 기준
const SK_H = 56;          // 토글 높이
const CHROME = 15;        // .sm-panel::after (하단 크롬)
const PAD_BOT = 127;      // 패널 아래 패딩 = 그리드 하변까지 (747 — 크롬 15 + 여유 14 + 배수 바 98)
const TRACK_W = 120, TRACK_H = 46, KNOB_W = 70;
const STEP = 0.055;       // SUM_POP_STEP (252)
/* 84 하단 앵커 — 363 이 밀치면 안 되는 값(2280 프레임) */
/* ⚠ 747 — `gridBot` 만 1573 → **1559**(−14)다. 앵커(버튼·닫기·패널·리본)는 한 칸도 안 움직였고,
   그리드 하변은 앵커가 아니라 **패딩의 결과**다(패널 하변 − padding-bottom). 그래서 값을 갈아 끼우되
   바로 아래 1-15b 로 «그 결과라는 것» 자체를 산수로 묻는다 — 상수만 옮기면 다음 이동이 안 보인다. */
const A84 = { btnY: 1706, btnBot: 1854, closeY: 2066, panelY: 606, panelBot: 1686, gridBot: 1559, rbY: 538 };
...
(async () => {
  ...
  /* ───────────────────────── §1 자리 ───────────────────────── */
  {
    ...
    /* 747 — 띠 한가운데가 아니라 **배수 바와 같은 중심**이라 여유는 위아래가 다르다.
       위(그리드) = PAD_BOT − SK_BOT − SK_H = 21 · 아래(크롬) = SK_BOT − CHROME = 35.
       둘을 각각 산수로 적어 «어느 쪽으로 흘렀는지» 가 보이게 둔다(합만 재면 이동이 안 보인다). */
    ok('1-7 그리드와의 여유', +(g.sk.y - g.grid.b).toFixed(1), PAD_BOT - SK_BOT - SK_H + 0.5, 1.5);
    ok('1-7b 하단 크롬과의 여유', +(g.panel.b - CHROME - g.sk.b).toFixed(1), SK_BOT - CHROME, 1.5);
    ...
    ok('1-15 (327) 그리드 하변 (747 — 패딩 127 의 결과로 −14)', g.grid.b, A84.gridBot, 0);
    ok('1-15b 그리드 하변 = 패널 하변 − padding-bottom', +(g.panel.b - g.grid.b).toFixed(1), PAD_BOT, 0.5);
