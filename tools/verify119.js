#!/usr/bin/env node
/* 119 검증 — 89 유물 소환 비용 (지금 규칙: **고정 100**, 작업 274)
 *
 *   node tools/verify119.js
 *
 * 저장소 주인 지시(2026-08-26, 119): «유물 소환 가격은 822 가 아니라 100 으로, 한 번 소환할 때마다 1 씩 추가».
 * 저장소 주인 지시(2026-08-27, 198): «유물 소환 처음 가격 걍 1원부터 시작» — 절편만 100 → 1.
 * 저장소 주인 지시(2026-08-27, **274**): «유물소환 가격 고정값으로 100으로 하자»
 *   → «소환 1회마다 +1» 은 **폐기**. 비용은 어떤 축으로도 오르지 않는 상수 100 이고, 198 의 시작가 1 도
 *     그 곡선을 전제한 항목이라 같이 폐기된다. 이 게이트는 «유물 소환 비용» 이라는 같은 성질을 계속
 *     지키므로 파일을 새로 만들지 않고 기대값만 새 규칙으로 옮겼다(162·94 에서 쓴 것과 같은 처리).
 *     옛 곡선 3개는 여전히 «부재» 로 잡는다: 89 «800+22×ΣLv» · 119 «100+1×횟수» · 198 «1+1×횟수».
 * 지시서 [3]-(가) 기계적 작업이므로 **비평가는 띄우지 않는다**.
 *
 * 항목 (PROGRESS 119 «검증 [3]-(가)» 가 요구한 것 + 111 교훈 1 의 «두 층» 분리):
 *   [A] ⓐ 옛것이 사라졌는가 = **소스 스캔** — `RELIC_COST_BASE`·`RELIC_COST_PER` 자체가 부재 ·
 *       `relicCost` 의 `relicLvSum`·`sumRelic` 의존 부재 · 정적 HTML 의 «822»·«1» 이 부재.
 *       런타임으로는 영영 못 본다.
 *       (138, 2026-08-26) A5 는 `#rwCost` 의 **`b` 기본 숫자만** 본다 — 아이콘 마크업은 125 소관이라 A7 로 분리.
 *   [B] ⓑ 새것이 맞는가 = 새 세이브 `relicCost() === 100` · `#rwCost` 표시도 «100»
 *   [C] 1회 소환 후에도 **100** · 유물석이 정확히 100 만 차감 (274 — 더 이상 +1 이 아니다)
 ...
 *   [H] 구 세이브 — 수천 번 소환한 세이브를 열어도 **100**(어느 축으로도 역산하지 않는다) ·
...
(async () => {
  ...
  ok(!/RELIC_COST_BASE/.test(SRC), 'A1 옛 상수 `RELIC_COST_BASE`(89 800 · 119 100 · 198 1) 부재');
  ok(!/RELIC_COST_PER/.test(SRC), 'A2 옛 증분 상수 `RELIC_COST_PER`(89 22 · 119·198 1) 부재');
  ok(/const\s+RELIC_COST\s*=\s*100\s*;/.test(SRC),
    'A3 새 상수 `RELIC_COST = 100` 하나뿐 (274 — 오르는 축이 없다)');
  {
    const m = SRC.match(/const\s+relicCost\s*=\s*\(\)\s*=>[^\n]*/);
    ok(!!m && !/relicLvSum/.test(m[0]) && !/sumRelic/.test(m[0]) && /RELIC_COST/.test(m[0]),
      'A4 relicCost() 가 ΣLv·누적 소환 횟수 어느 축도 읽지 않는다(상수 반환)', m ? m[0].trim() : '정의 못 찾음');
  }
    ...
    'A4b S.cnt.sumRelic 카운터 자체는 계속 센다(61 미션·154 가이드가 쓴다 — 가격만 안 읽는다)');
  {
    ...
    ok(!!rw && /<b>\s*100\s*<\/b>/.test(rw[1]) && !/822/.test(rw[1]) && !/<b>\s*1\s*<\/b>/.test(rw[1]),
      'A5 정적 HTML `#rwCost` 기본 표기 «822»(89)·«1»(198) → «100» (274)', rw ? rw[1].trim() : '#rwCost 못 찾음');
  ...
  ok(C.cost === 100, 'C2 소환 1회 뒤에도 relicCost() = 100 (274 — +1 폐기)', String(C.cost));
  ok(C.txt === '100', 'C3 소환 직후 #rwCost 표시도 «100» 그대로(renderRelw 갱신)', C.txt);
  ok(C.sum === 1, 'C4 S.cnt.sumRelic = 1 (카운터는 계속 센다)', String(C.sum));

  /* ---- [D] 50회 뒤에도 100 · 누적 차감 = 100 × 50 = 5,000 ---- */
  ...
  /* ---- [E] 축이 아예 없다 — 유물 Lv 도 소환 횟수도 비용에 영향 없음 ---- */
  ...
  ok(E.before === 100 && E.after === 100,
    'E1 소환 7회 + ΣLv 300 을 만들어도 비용 100 (89 곡선이면 7,400 · 119 곡선이면 107 · 198 곡선이면 8)',
  ...
  const G2 = await page.evaluate(() => {
    ...
    clear(); S.relic = 99; summonRelic();          /* quiet 아님 = 첫 누름 → 안내 1건(부족분 1) */
    const loud = seen();
  ...
  /* ---- [H] 구 세이브 — 쌓인 sumRelic 이 있어도 비용은 100 ---- */
  const mk = async save => {
  ...
  ok(h1.cost === 100 && h1.lvSum === 21,
    'H1 구 세이브(sumRelic 37 · ΣLv 21)도 비용 100 — 89 곡선이면 1,262 · 119 곡선이면 137 · 198 곡선이면 38',
  ...
  ok(h2.cost === 100 && h2.sum === 0, 'H4 손상 세이브(sumRelic null)도 NaN 없이 비용 100 · 카운터 0 복구',
