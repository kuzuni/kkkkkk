...
const RUN = ({ id, frames, chase }) => {
  ...
  for (let f = 0; f < frames; f++) {
    ...
    if (!chase) enemies.forEach(e => { e.sp = 0; });
...
async function runOne(browser, url, id, chase) {
  ...
  const r = await page.evaluate(RUN, { id, frames: Math.round(SEC * 60), chase });
  await ctx.close();
  return { ...r, ...shape, errs };
}
...
(async () => {
  ...
  for (const r of after.filter(r => !r.chase)) {
