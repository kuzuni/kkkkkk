...
const OFF = [45, 65, 120, 265, 380, 445, 466, 489, 500, 558, 620, null];   /* ★ 9회차 — 시작 위상이 f2 로 옮겨지고(이동 933px)
   ...
   **265 둘째 도약 정점** · 380 f1 접지 · **466 마지막 도약 정점** · **500 착지 직후(스쿼시 최대)** · null 전환 합성.
   ...
   ★ **12회차에 445 · 489 · 558 을 더 넣었다(11장 + 합성).** 11회차 비평 T 의 요청이다 —
   «ⓓ(air 부호)가 노리는 τ≈0/1 위상이 8장 중 0장이라 코드·스윕으로만 확인했다 · 도착 순간(555~565ms)도 0장».
   445 = 마지막 도약 **이륙 직후**(τ≈0.07) · 489 = **착지 직전**(τ≈0.95) — 둘 다 `LD_LIFT` 상쇄로
   arc 가 +12(아래)가 되는 자리라 «접지 순간 그림자가 안 죽는가» 를 **화소로** 볼 수 있다.
   558 = 도착 직전 — 12회차가 계단을 경사로로 바꾼 정착 구간의 **끝**이다(보정이 160 → 1.2px 로 흘러간 상태). */
const N = OFF.length;
