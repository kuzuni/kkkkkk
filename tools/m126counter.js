...
const SPOTS = [
  ...
  /* 201(2026-08-27, 저장소 주인 지시) — «초상화»·«프레임» 탭이 폐기돼 `.pf-tab.t2` 는 더 없다.
     남겨 두면 0 매치라 «면적 0 = 절벽» 으로 항상 빨개진다(189 의 「마을」과 같은 부패한 빨강). 뺐다.
     ⚠ **빼기 전에 남은 칸으로 옮겨 재 봤고**, `.pf-tab.t1>i`(「칭호」)는 같은 CSS 인데 속공간이
        **4.6 px²**(하한 8 미만)였다 — 「초상화」의 30.4 와 6.6배 차이. 즉 이 자리는 내내 «초상화 의 ㅇ»
        하나로 초록이었고 「칭호」의 작은 받침 ㅇ 은 126 10회차의 4 → 6.08px 올림에 76% 막혀 있었다.
        그 자리를 조용히 초록으로 덮지 않고 **작업 240 으로 등재**해 두었다.
     240(2026-08-27) — 그 작업이 `--st-pft`(.115)로 닫았다. **남은 칸(`.t1`)을 여기 되살린다** —
        이 자리가 목록에서 빠져 있는 동안 아무도 안 보고 있었던 것이 애초 사고의 절반이다.
        옛 값은 «4회차 이전 고정 px» 가 아니라 **240 직전 값 6.08** 이므로 판정은 'alive'(하한만 본다).
        ⚠ 이 게이트는 **격리 렌더**라 드롭 섀도를 안 들고 간다 — 화면에서 드롭까지 켠 값은
        `tools/verify240.js` 가 따로 잰다(격리 17.8 ↔ 화면 9.6). 둘 다 있어야 이 자리가 지켜진다.
     9회차 ③ 이 «올린» 3곳 중 「19 라벨」군은 아래 «해금 미션» 이 그대로 잇는다. */
  ['19 프로필 탭 «칭호»',     '#profBtn', '#pfw .pf-tab.t1>i', 6.08, 'alive'],
