 ...
 * `| <구현 –> | <점수 –> | <횟수> | **<미착수|등재만|착수 전|등재문>`. 재현·음성항은 `tools/probe388.js`.
 * («등재문» 은 **괄호 한정이 안 붙은 것만** — `등재문(보존)` 은 «했다» 쪽이다. 작업 960.)
 */
 ...
 *
 * ⚠ **세 번째로 관행이 앞질렀다 — 머리말 «등재문» (작업 960, 2026-09-05).** 422 가 어휘를
 *   `미착수|등재만|착수 전` 셋으로 적은 뒤, 등재하는 세션들이 머리말을 **`**등재문**`** 으로 쓰기
 *   시작했다(실측 8행). 그 낱말이 어휘에 없어 §2·§3 이 통째로 못 봤고, 947 이 `done(947)` 을
 *   이력에 두고도 표가 `구현 –`·`0/5`·`**등재문.**` 인 채 남아 **다음 워커가 재선점했다**
 *   (371·378·308·383·498 에 이어 여섯 번째 · 등재 960).
 *   ⚑ **어휘에 «등재문» 을 그냥 더하면 안 된다 — 이 낱말은 뜻이 둘이다.** 실측(8행)이 그 둘을 가른다:
 *     · **맨낱말** `**등재문.**` · `**등재문** — 실측…`  = «이 행은 등재문**일 뿐**이다» = 안 했다
 *       (948·955·960·961·962 — 다섯 다 미착수)
 *     · **괄호 한정** `**등재문(보존)**` · `**등재문(아래 본문 보존) · 완료**` = «아래 본문은 **보존된**
 *       등재문이고 답은 딴 칸에 있다» = 했다 (896·909·915 — 셋 다 완료행)
 *   그래서 어휘에는 «뒤에 `(` 가 안 오는 등재문» 만 넣는다. 그냥 넣으면 실물 표에서 §2 가
 *   **896·909·915 세 행을 곧바로 빨갛게** 만들고(실측), 이 자는 모든 워커의 push 게이트라
 *   그 순간 저장소 전체의 push 가 막힌다 — 이 자가 정밀도를 재현율 앞에 두는 자리다(§3 머리말).
 *   ⚠ 세 행의 머리말은 실제로 «미착수» 를 말하지 않는다 — 896 은 «답은 위 구현 칸» 이라고
 *     **가리키기까지** 한다. 티어 스캔도 그렇게 읽으므로 헛빨강이 맞다.
 *   되돌림 시험(한정을 떼면 셋이 빨개지고, 어휘를 빼면 다섯이 조용해진다)은 `tools/verify960.js`. */
...
/* ⚠ **칸은 escape 를 지켜 나눈다(작업 960, 2026-09-05).** 445 가 이 자를 세울 때 쓴
   `line.split('|')` 은 `\|` 까지 구분자로 세어, **비고 칸 한복판의 조각**을 «마지막 칸» 으로
   집는다. 그 조각이 어휘로 시작하면 완료행이 통째로 헛빨강이 된다 — 960 자신의 행이
   그랬다(비고가 `미착수\|등재만\|착수 전` 을 인용하자 naive 는 11번째 조각 «착수 전` 으로
   적었고…» 를 머리말로 읽었다). 566 이 축 ⓒ 에서 이미 못박은 규약을 여기만 안 지키고 있었다
   («⚠ 칸을 셀 때 `\|`(escape)는 구분자가 아니다 — GFM 이 그렇게 읽는다»).
   ⚠ 이 자는 **아무것도 안 잃는다** — 실측에서 잡던 행을 놓치는 자리는 0건이다(`verify960` [7]). */
function tailHead(line) {
  const c = cellsOf(line);               /* `function` 선언이라 아래 정의가 끌어올려진다 */
  let i = c.length - 1;
