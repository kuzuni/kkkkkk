...
function swapBootstrap(src) {
  ...
  if (iife.test(src)) return { src: src.replace(iife, HEAD), how: 'IIFE 사다리' };
  /* 직결 한 줄 */
  const direct = /const \{ chromium \} = require\((['"])playwright\1\);/;
  if (direct.test(src)) return { src: src.replace(direct, HEAD), how: 'require 직결' };
  return null;
}
...
const LAUNCH_RULES = [
  ...
  { re: /await chromium\.launch\(launchOpts\(\)\)/g,
    to: () => 'await launch(chromium)   /* 931 — 폴백은 사슬이 맡는다 */' },
  /* 5) 자기 args 와 launchOpts() 를 합치던 한 줄 — args 는 살리고 폴백만 사슬로 */
  { re: /await chromium\.launch\(Object\.assign\((\{[^{}]*\}), launchOpts\(\)\)\)/g,
    to: (m, a) => 'await launch(chromium, ' + a + ')   /* 931 — 폴백은 사슬이 맡는다 */' },
];

/* ---- ⓒ 죽은 부품 — ⓑ 뒤에 아무도 안 부르는 `launchOpts()` 는 선언째 지운다 ---- */
function dropLaunchOpts(src) {
  if (/launchOpts\(\)/.test(src.replace(/function launchOpts\(\)[\s\S]*?\n\}\n/, ''))) return src;
  return src.replace(/(?:\/\*[^*]*\*\/\s*)?function launchOpts\(\) \{[\s\S]*?\n\}\n/, '');
}
