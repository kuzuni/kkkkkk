...
/* 구 세이브 호환을 위해 «id·등급» 이 바뀌면 안 되는 기존 13종 (표는 [g, cd, m])
   ...
   **작업 504(2026-08-30, T1 «자 문제»)가 484 의 자를 고치면서 m 을 다시 옮겼다** — 484 는 발수
   선언을 손으로 적은 값 그대로 믿고 m 을 맞췄는데 그 선언이 실제 타격수와 최대 14.2배 어긋나
   있었다. 발수를 실측으로 다시 적으니 m 도 따라 움직인다(값의 유래는 `tools/verify504.js`).
   ⚠ **`g`·`cd` 는 한 칸도 안 움직였다** — 세이브 호환이 걸린 축은 그쪽(id·등급)이고 484·504 는
     «세기(m)» 와 «발수(hits)» 만 만졌다. m 축의 주인은 `tools/verify484.js`(SK_DPS_REF 6.49) ·
     발수 축의 주인은 `tools/verify504.js` 이고, 이 표는 «g·cd 를 건드리지 않았다» 를 같이
     못박는 두 번째 자다. */
const OLD = { slash:[0,0.85,5.5165], shuri:[0,2.20,1.7848], multi:[1,1.10,2.3797], orbit:[1,0,0.9759],
              ice:[1,1.60,3.3176], aura:[2,0,0.6904], bolt:[2,1.40,3.0287], boom:[3,2.00,3.2049],
              poison:[3,3.20,0.7074], drain:[3,2.60,16.874], boomer:[4,2.40,6.1323],
              meteor:[4,4.00,2.5476], holy:[5,3.00,2.0495] };
