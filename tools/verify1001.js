...
/* 허용 목록 — 이 축을 인라인으로 접어도 되는 자와 **그 이유**.
   ...
   ⚑ **1003 이관(2026-09-06) — `verify989.js` 가 목록에서 빠졌다.** 1001 등재 당시엔 «다른 축 ·
     옛 이름 `DIAG_TOL`» 로 허용돼 있었는데, 1003 이 그 자의 [R2] 를 고치면서 접는 산수를
     **부품(`B998.band(rows, DIAG_TOL, 'body'|'own'|'vis')`)으로 넘겼다** — 이제 인라인으로 안 접으므로
     이 목록에 있으면 [C1] 이 «사라진 자» 로 빨개진다(허용은 «있어도 된다» 가 아니라 «있다» 를 센다).
     ⚠ 989 의 열쇠가 대각인 것은 그대로다(그 자의 축이 «본체만 ↔ 본체+제 손 부품» 이라 그렇다 —
     최대 변으로 접으면 두 눈금이 둘 다 빨개져 대조가 사라진다. 실측은 `verify989.js` 머리말). */
const ALLOW = {
