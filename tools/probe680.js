 ...
 *   [2] 흔들림이 아니다 — 선언이 K회 표본 구름 밖에 있고(부호), 그 거리가 구름의 폭보다 멀다(크기)
 *       («플레이키» 기각. ⚑ 784 — 옛 꼴 «K회 폭 < 0.25» 는 선언을 안 보는 축이라 그 자신이
 *        동전이었다 · 재현·되돌림은 `tools/probe784.js`)
...
(async () => {
  ...
  /* «플레이키니까 재실행하면 닫힌다» 를 여기서 끊는다.
     ⚠ 이 항이 없으면 다음 스윕이 또 «흔들림» 으로 읽고 네 번째 등재를 한다.
     ⚑ **784 — 이 항 자신이 동전이었다(2026-09-02).** 옛 꼴은 `spread < 0.25 && (off − tol) > 0.02`.
     첫 항의 축(K회 폭)은 **선언을 안 본다** — 선언을 표본 한복판으로 옮겨도 값이 Δ0 이라 이 항의
     이름(«재실행으로 안 닫힌다»)을 질 수 없고, 실제로 지고 있던 말은 «판이 조용한 실행에서만
     초록» 이라는 정반대였다(실측 폭 5~28% ↔ 손 문턱 0.25). 둘째 항은 [1] 에 손 상수 0.02 를 얹은
     사본이라 새로 지는 뜻이 없다. ⇒ 문턱을 다시 뽑지 않고(759-①) **판정의 모양**을 바꿨다 —
     선언과 표본 구름의 관계를 부호·크기 두 항으로 직접 묻는다(`RUL.shakeSep` · 판정 사본 0개).
     재현·되돌림·«`nearOff` > 밴드» 기각은 `tools/probe784.js`. */
  const sh = RUL.shakeSep(pz || {});
  ok(!!pz && sh.outside,
     '2 흔들림이 아니다(부호) — K회 표본이 한 개도 선언 쪽으로 안 넘어온다(구름이 통째로 선언 밖)',
     pz ? '선언 ' + pz.decl + ' ↔ 표본 ' + sh.lo + '~' + sh.hi + ' · 값 ' + pz.each.join('/') : '-');
  ok(!!pz && sh.ratio >= RUL.SHAKE_UNIT,
     '2b 흔들림이 아니다(크기) — 선언까지의 거리가 **그 실행이 스스로 잰 K회 폭**보다 멀다(손 문턱 아님 · 778-②)',
     pz ? '거리 ' + sh.gap.toFixed(2) + ' ÷ 폭 ' + sh.range.toFixed(2) + ' = ×'
          + (sh.ratio === Infinity ? '∞' : sh.ratio.toFixed(2)) + ' ≥ ' + RUL.SHAKE_UNIT
          + ' (실측 41회 ×3.0~14.2) · K회 폭 ' + (pz.spread * 100).toFixed(0) + '%' : '-');
