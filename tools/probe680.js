...
(async () => {
  ...
  /* ⚑ 759(2026-09-01) → **783(2026-09-02) 재수리**. 759 는 «판정 불가로 등재한 종을 폭 한 축으로
     ...
     이탈이 27~41% 로 흔들리는데 밴드가 마침 40% 라, 상위 둘이 **매 실행 자기 K회 폭 이내로** 가장자리에
     붙어 있다(`probe783` [1] · 10회 실측 여유/폭 −0.01~0.59). 그중 하나가 한 번 위로 흔들리면
  ...
  ok(sep.isMax,
     ...
     + ((sep.near - sep.rest[0].near) * 100).toFixed(1) + 'p (실측 10회 10.5~19.6p)');
  ok(sep.ratio >= 1.30,
