/* 작업 241 회귀 게이트 — 19 프로필 팝업이 9:16(frameH 1600)에서 바닥 227px 잘림 (T1 버그).
   ...
   처방: `.pf{top:clamp(104px, 431px, calc(100% - 1404px))}` — 값이 아니라 **상한**이다.
     · 최대값 `100% − 1404`(=1396 패널 + 8 여유): 2280 → 876 · 1920 → 516 → 둘 다 431 이 이긴다.
       **기준 프레임 기하는 한 픽셀도 안 바뀐다**(19 는 ①~④ 8점 통과 화면).
       1600 → 196 이라 이때만 235px 위로 붙는다.
...
(async () => {
  ...
  eq('[1600] .pf top = 상한 196 (= 1600 − 1396 − 8)', M[1600].pf.top, 196);
  for (const h of [1600, 1920, 2280, 2600]) {
  ...
  ok(/clamp\(104px,\s*431px,\s*calc\(100% - 1404px\)\)/.test(SRC), 'index.html 에 .pf top 상한이 있다');
  ...
  const neg = SRC.replace('top:clamp(104px, 431px, calc(100% - 1404px));', 'top:431px;');
