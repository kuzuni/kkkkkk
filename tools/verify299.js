...
(async () => {
  ...
  const measure = await page.evaluate(async () => {
    ...
    /* ⚑ 471+479(2026-08-30 주인 번복) — 364 의 «카드 우상단» 을 주인이 다시 «[받기] 버튼 코너» 로
       돌렸고, 471 규약이 «호스트 코너» 라 닷 노드가 버튼 자식으로 옮겨 갔다. 299 의 뜻(«부모 상자
       기준 우상단 사분면»)은 그대로이고 **부모가 바뀐 것**이라 셀렉터만 이관한다 — 자리를 무르게
       푼 것이 아님은 버튼 기준으로도 우상단 사분면을 그대로 요구하는 것이 못박는다. */
    sweep('13 광고 [받기] 버튼 .cn-cd .bt>.updot (471·479)', '#shopList .cn-cd .bt>.updot', '.cn-cd>.bt');
