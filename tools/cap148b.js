/* 작업 148 — 미션 텍스트 요소별 «타이트 크롭» 캡처(1:1 · 배경 투명 제외 없이 그대로).
   실행: node tools/cap148b.js  → docs/review/148-r4-e-*.png
...
(async () => {
  ...
  await shot('#tuto .tbtn', '148-r4-e-L1라벨.png');
  await shot('#tuto .tt',   '148-r4-e-L2문구.png');
  await shot('#tuto .tpg',  '148-r4-e-L3진행.png');
  await shot('#tuto .tsub', '148-r4-e-보상수량.png');

  await p.evaluate(() => { openProfile(); });
  await p.waitForTimeout(700);
  await shot('.pf-msn', '148-r4-e-해금미션바.png', 2);

  await p.evaluate(() => { closeModal(); openQuest(); });
  await p.waitForTimeout(700);
  await shot('.mbox .mhead h2', '148-r4-e-퀘스트헤더.png');
  await shot('#qsList .qs-r:nth-child(1)', '148-r4-e-퀘스트행.png', 2);
