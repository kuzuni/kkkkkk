...
(async () => {
  ...
  try {
    ...
    const subs = [
      /* 740 이관(2026-09-02) — 왼쪽(현행) 표본이 바뀌었다. 740 이 «남는 칸을 잠금 더미로
         채운다» 를 폐지하면서 **더미 칸 자체가 사라져** `real ? … : ''` 삼항이 필요 없어졌고
         (모든 칸이 실물이다), 664 가 세운 «미보유도 고를 수 있다» 는 그대로다.
         되돌림이 재현하는 결손(미보유 칸은 클릭 대상이 아니다 = `mine ?`)은 한 글자도 안 바뀌었다. */
      ["        +  ' data-wpn=\"' + real.id + '\"'",
