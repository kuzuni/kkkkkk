...
(async () => {
  ...
  console.log('[S3] 그려진 잉크 전 화면 스윕 — 소수 상자가 만든 찌그러짐 (418 · 옛 [S3]+[S4] 통합)');
  {
    ...
    const JUDGE_MIN = 180, RATCHET_CELLS = 68, RATCHET_SITES = 18;
    ...
    const KNOWN_SITES = [
      ...
      /* ── 356 16회차(2026-08-31) — **이 회차가 처음 본 두 자리.** 15회차 이후 상류가 화면을
         ...
         눈금은 601 과 같은 산수: **실측 + (한 device px 이 이 잉크에서 만드는 %) ÷ 2**. */
      { sel: 'div#mbox>div.mwell>p>img.cic', cap: 5.1,
        why: '356 16회차 — 55 길라잡이 모달 본문의 화폐 아이콘(잉크 48×50 · −4.00%). ' +
             '상자 25.9063 정사각(소수) · `.cic{1.08em} × fs 24` 몫이다. ' +
             'DSF2·3·4 로 −4.00 → −4.05 → **−2.04%** ⇒ 배율을 올리면 줄어든다 = **자의 바닥 쪽**이다' +
             '(프레임 px 로는 Δh 1.0 · 1.0 · 0.5). 잉크가 24×25 로 이 표에서 제일 작아 1px 이 4% 다. ' +
             '⚠ **418 «정수 상자» 처방의 다음 후보**지만 셀렉터가 모달 본문 공용이라 이 회차가 안 건드렸다. ' +
             '실측 −4.00%(DSF2) + 래스터 반 px 1.04% ⇒ 5.1' },
      { sel: 'div#rwCost>i>img.cic', cap: 3.7,
        why: '356 16회차 — 89 유물 소환 비용의 유물조각 아이콘(잉크 64×76 · −2.92% · 2칸 = 89 유물 + 도움말 429). ' +
             '⚠ **자의 바닥이 아니다** — DSF2·DSF3 가 **−2.92% 로 글자까지 같다**(잉크 64×76 → 96×114 = 정확히 1.5배). ' +
             '배율을 올려도 안 줄면 기하다(356 9회차 규칙) ⇒ 상자 38.8750(`1.08em × fs 36`)이 아니라 ' +
             '**그 자리의 아트·좌표**를 물어야 한다. 다음 회차의 첫 후보로 넘긴다. ' +
             '(⚠ DSF4 는 그 실행이 화면에 못 들어가 **잰 노드 0개**였다 — 수렴 판정은 DSF2·3 두 점뿐이다.) ' +
             '실측 −2.92%(DSF2·3 폭 0) + 래스터 반 px 0.78% ⇒ 3.7' },
