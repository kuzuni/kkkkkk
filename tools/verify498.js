...
(async () => {
  ...
  if (a) {
    ...
    const want = i => (i % 7 === 0 ? 1500 + i * 60 : 350 + i * 30);
    const off = a.rest.filter(r => r.dia !== want(r.i));
    yes('[3-c] 2~7일차 곡선이 399 값 그대로 (513 은 «길이» 만 바꿨다 — 한 푼도 안 옮겼다)', off.length === 0,
    ...
    is('[3-d] 3일차 440 (verify70 회귀 — n=9 가 받는 칸)',
       a.rest.find(r => r.i === 3).dia, 440);
