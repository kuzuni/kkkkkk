...
(async () => {
  ...
  /* ── §3 출석 ──────────────────────────────────────────────── */
  console.log('\n[3] 출석 — 1일차만 «환영» 칸이고 2일차 이후는 399 그대로');
  ...
  if (a) {
    ...
    const want = i => (i % 28 === 0 ? 5000 : (i % 7 === 0 ? 1500 + i * 60 : 350 + i * 30));
    const off = a.rest.filter(r => r.dia !== want(r.i));
    yes('[3-c] 2~28일차 곡선이 399 값 그대로', off.length === 0,
        off.map(o => o.i + '일차 ' + o.dia + ' ≠ ' + want(o.i)).join(' · '));
    is('[3-d] 10일차 650 (verify70 회귀 — 그 자가 이 값을 상수로 잰다)',
       a.rest.find(r => r.i === 10).dia, 650);
    yes('[3-e] 399 — 전 칸 다이아 한 종류(goldMul·rel 이 되살아나면 빨강)',
        a.keys.length === 1 && a.keys[0] === 'dia', a.keys.join(','));
    note('[3-f] 28일 합', fmtN(a.d1 + a.rest.reduce((n, r) => n + r.dia, 0)));
