...
/* ⚑⚑ 927 — [B14] 의 **칸당 표본 문턱**. 묶는 축이 «종» 에서 «종 × 기울기 칸» 으로 넓어지면
   ...
   ⚑⚑ 988 — **20 은 그 뒤 «못 재는» 자리가 됐다.** 981 이 실루엣 넷을 다시 그리자 표본이 다시
     흩어져 두 구간을 채우는 칸이 `lance@0~15°` **하나**로 줄었고, 그 순간 [B14]·[B15] 의
     «칸 ≥ 2» 조건이 빨강을 냈다 — **밴드 1.078 은 문턱 1.25 아래인데 항은 빨간** 꼴이라
     빨강이 제품을 가리키지 않았다(등재 988).
   ⚑ **문턱 사다리로 갈래를 갈랐다**(아래 [진단·988] 이 매 실행 그 표를 찍는다):
     문턱 30 → 0칸 / 25 → 1칸 / **20 → 1칸** / 16 → 3칸 / **12 → 6칸** / 10 → 6칸 / 8 → 8칸.
     칸이 **실제로 는다** ⇒ 등재문의 ⓐ(문턱이 세다)이고 ⓑ(원리적으로 못 채운다)가 아니다.
   ⚑ 그리고 927 이 20 에 멈춘 근거(«12 아래로 내려가면 바닥이 흔들린다»)는 **지금 자료에서
     재현되지 않는다** — 바닥은 문턱 30~8 에서 **최악 1.128 · 중앙 1.006~1.012 로 평평하다**
     (실루엣이 바뀌며 바닥 자체가 1.062 → 1.128 로 올라앉았고, 문턱을 따라 움직이지 않는다).
     곧 문턱을 20 에 붙들어 둘 이유가 «바닥» 에는 없다.
   ⇒ **12.** 16 이 아니라 12 인 이유는 «겨우 둘» 을 피하기 위해서다 — 16 은 3칸이라 실루엣 한 종만
     바뀌어도 다시 «칸 ≥ 2» 밑으로 떨어진다(그 취약함이 곧 이번 결손이다). 12 는 4종 × 3칸에
     6칸이라 여유가 있다. **이 변경은 자를 무르게 하지 않는다** — 칸이 늘면 [B14] 는 그 칸들을
     전부 견주므로 최악값이 **1.078 → 1.165 로 올라가고**(래칫 1.25 는 한 글자도 안 건드렸다),
     초록이 되는 칸이 아니라 **재는 칸이** 늘어난 것이다.
   ⚠ 8 로는 내리지 마라 — 최악이 1.205 로 래칫 코앞이고 칸당 표본 8 은 위 바닥 표의 밖이다. */
const B14_N   = 12;
...
(async () => {
  ...
  try {
    ...
    if (r.out && r.out.__err) {
      ...
      /* ⚑ 988 — 문턱을 인자로 받게만 바꿨다(값은 그대로 `B14_N`). 아래 **문턱 사다리**가 같은
         셈을 문턱마다 물어야 하는데, 사본을 뜨면 바닥과 판정이 서로 다른 자를 쓰게 된다(402). */
      const b14floorAt = (n) => {
        const nz = [];
        for (const b of b13bands(r.out, strokeIds, 2, { byAng: true, n })) for (const e of b.by) {
          if (e[1] < n || !(e[3] > 0) || !(e[4] > 0)) continue;
      ...
      const b14floor = b14floorAt(B14_N);
      /* ⚑⚑ 988 신설 — **문턱 사다리**. 927 이 축을 «종 × 기울기 칸» 으로 넓힌 뒤 두 구간을
         채우는 칸이 하나로 줄어 [B14]·[B15] 가 «견줄 것이 없다» 로 빨갛다. 그 빨강이
           ⓐ 문턱이 세서 칸이 안 늘어나는 것인지(= 문턱의 몫) ·
           ⓑ 이 자료로는 원리적으로 두 칸을 못 채우는 것인지(= 자가 틀렸다)
         는 **문턱을 내리며 칸이 느는지**를 보면 곧바로 갈린다. 같이 찍는 잡음 바닥이 문턱을
         내릴 때 어디서 무너지는지도 한 줄에 나온다 — 칸이 늘어도 바닥이 판정값 위로 올라오면
         그 칸은 «잰 것» 이 아니다. 출력 전용(판정은 위 `B14_N` 그대로). */
      const b14ladder = [30, 25, B14_N, 16, 12, 10, 8].map((n) => {
        const sp = b14bands(r.out, strokeIds, 2, { byAng: true, n });
        return { n, cells: sp.length, worst: sp.length ? sp[0].band : 0,
                 sps: sp.map(e => e.sp + '(' + e.bins + ')').join(','), fl: b14floorAt(n) };
      });
      {
        console.log('  [진단·927] 판정이 쓰는 묶음 «종 × 기울기 칸» — ' +
          ...
          (b14floor ? '\n    새 축의 잡음 바닥(같은 종·구간·기울기 칸을 짝/홀로 반) — ' + b14floor.n +
                    ...
                    : ''));
        console.log('  [진단·988] 문턱 사다리 — 칸이 느는가(ⓐ 문턱의 몫) · 안 느는가(ⓑ 자의 몫)');
        for (const e of b14ladder)
          console.log('    표본 ' + String(e.n).padStart(2) + ' 이상  칸 ' + e.cells +
                      '  최악 ' + (e.worst ? e.worst.toFixed(3) : '—') +
                      '  잡음 바닥 중앙 ' + (e.fl ? e.fl.md.toFixed(3) : '—') +
                      ' · 최악 ' + (e.fl ? e.fl.mx.toFixed(3) : '—') +
                      '  [' + (e.sps || '없음') + ']');
      }
      {
           ...
           그리고 이 수는 새 축의 잡음 바닥(최악 1.062)보다 **위**라 자의 흔들림이 아니다. */
      {
        ...
        /* ⚑⚑ 988 **이관** — 이 항이 묻는 것은 **밴드**다. «몇 칸을 견줬는가» 는 커버리지라
           [B16] 으로 떼어 냈다(한 축 = 한 항 — 이 파일 위 [B14] 머리말이 그렇게 적고 있다).
           떼어 내기 전에는 «밴드 1.078 ≤ 1.25» 를 찍으면서 빨간 줄이 나와 **빨강이 무엇을
           가리키는지 줄 자신이 말하지 못했다**(등재 988 의 얼굴). 조건이 사라진 것이 아니다 —
           칸 ≥ 2 는 [B16] 이 그대로 요구하고, 여기 남은 `>= 1` 은 «표본 0 이면 자동 초록» 을
           막는 항(빈 단언 금지 — LESSONS 185-②)이다. */
        ok(sp14a.length >= 1 && worst && worst.band <= BAND14,
           ...
           칸이 늘어도 기울기가 섞이면 이 항은 그대로 빨갛다. */
      {
        ...
        /* ⚑ 988 **이관** — [B14] 와 같은 이유로 커버리지 조건을 [B16] 에 넘겼다(여기 `>= 1` 은
           빈 단언 금지). 이 항이 재는 것은 «섞인 칸이 0 인가» 하나다. */
        ok(sp14a.length >= 1 && bad.length === 0,
        ...
        /* ⚑⚑ 988 신설 [B16] — **커버리지**. [B14]·[B15] 가 재던 «칸 ≥ 2» 를 여기로 옮겼다.
           한 칸뿐이면 그 밴드는 «그 종 · 그 기울기» 한 자리에서만 잰 값이라 «폭 축» 을
           대표하지 못한다 — 조건은 그대로 서 있고, 빨개질 때 **자기 이름으로** 빨개진다.
           ⚠ 이 항을 초록으로 만들려고 문턱(`B14_N`)을 내리는 것은 «자를 무르게» 가 아니다 —
             칸이 늘면 [B14] 가 견주는 자리가 늘어 최악값이 **오른다**(988: 1.078 → 1.165).
             무르게 푸는 길은 래칫(`BAND14`)을 올리는 쪽이고 그건 988 도 안 건드렸다. */
        ok(sp14a.length >= 2,
           '[B16] 폭 축을 견주는 칸이 **둘 이상** — ' + sp14a.length + '칸(' +
           (sp14a.length ? sp14a.map(e => e.sp + ' ' + e.bins + '구간').join(' · ') : '없음') +
           ') ≥ 2 · 칸당 표본 ' + B14_N +
           ' (한 칸이면 «폭 축» 이 아니라 그 한 자리의 값이다 — 등재 988)');
        /* ⚑ 988 신설 [R8] — **[B16] 이 정말 칸을 세는가.** 위 항은 지금 6칸이라 초록인데,
           그것이 «세어서» 인지 «하드코딩» 인지는 칸이 0 이 되는 자리를 실제로 돌려 봐야 갈린다.
           채울 수 없는 문턱(1e6)으로 판정과 **같은 함수**를 불러 칸이 0 임을 본다(사본 금지 402).
           ⚠ 문턱 20 을 되돌려 «1칸» 을 단언하지 않는 이유 — 그 수는 실루엣이 바뀔 때마다 움직여
             제품과 무관하게 빨개진다(그 취약함이 이번 결손 자체다). 여기서는 **구조적으로**
             비는 자리를 쓴다. */
        const none14 = b14bands(r.out, strokeIds, 2, { byAng: true, n: 1e6 });
        ok(none14.length === 0,
           '[R8] 채울 수 없는 문턱(표본 1e6)에서는 [B16] 이 빨개진다 — 칸 ' + none14.length + ' = 0');
      }
    ...
    if (rn.out && rn.out.__err) ok(false, '[R1] 사본 측정 예외 — ' + rn.out.__err);
         ...
         값을 갖고 나머지는 0 이다(= 그 구간들의 마루에 층이 아예 없다) ⇒ 밴드가 ∞ 다. */
      {
        ...
        /* ⚑ 988 — [B14] 의 조건이 «칸 ≥ 1 · 밴드 ≤ 래칫» 으로 갈라졌으므로(커버리지는 [B16])
           되돌림도 **그 조건 그대로** 물어야 한다. 칸이 0 이어도(잴 것이 없다) 빨강이고,
           칸이 있는데 밴드가 래칫 위여도 빨강이다 — 둘 다 «코어를 끄면 초록이 아니다» 다. */
        ok(!(nb14.length >= 1 && w14 <= BAND14),
           '[R6] 코어를 끄면 [B14](«종 × 기울기 칸» 폭 축)가 빨개진다 — 2구간 이상인 칸 ' + nb14.length +
           '칸 · 최악 밴드 ' +
           (isFinite(w14) ? w14.toFixed(2) : '∞') + ' (초록이려면 1칸 이상 · ' + BAND14 + ' 이하여야 한다)');
      }
