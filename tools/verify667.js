...
/* ── 기대값 (회차별 근거) ─────────────────────────────────────────────────
   ...
   · 헤더 밴드 96 / 102 = 4회차 · 리본 띠 67 / **77** = 3회차 + **969 이관**
   ⚑ 969 — 불릿형 리본 띠 과녁을 **76 → 77** 로 갈아 끼웠다(333 처방). 966 1회차가 검정 테 교차
     여덟 칸으로 ref 를 재서 «두 줄이 나란히 1.06 짧다» 를 찍었다(ref 리본1 273.00−195.88 = **77.12** ·
     리본2 138.70−61.59 = **77.11** ↔ 우리 둘 다 76.05). 아랫변은 두 줄 다 제자리라 윗변만 올렸다.
   ⚠ **허용 오차도 같이 좁혔다(±1 → ±0.5)** — 이 자리가 969 등재문이 지목한 «눈금을 타고 지나가는»
     자리다: ±1 이면 제품이 77 이어도 «76» 이라고 적힌 게이트가 초록이라, 게이트와 제품이 서로 다른
     값을 말하는 채로 굳는다(선언이 정수라 ±0.5 로 두 값이 완전히 갈린다). 화소 자·되돌림 시험은
     `tools/verify969.js`(14/14) 가 따로 세운다. */
const EXP = {
  ban: { h: 534, ntcN: 3, sH: 81, pitch: 139, cen0: 130, hdb: 96, rb: 67, art: { w: 390, h: 357 } },
  bl:  { h: 690, ntcN: 2, sH: 114, pitch: 227, cen0: 160, hdb: 102, rb: 77, art: { w: 445, h: 281 } }
};
...
(async () => {
  ...
  const chk = (c, k) => {
    ...
    /* ⚑⚑ 923 7회차 이관 — **산수만 바뀌었다**(뜻은 667 7회차 그대로: 그려질 노치 길이가 선언인가).
       바깥 곡선이 표의 배수가 아니라 **안쪽 곡선의 법선 오프셋 10** 이 된 뒤로 그 세로 스팬은
       `(len − 20)·v(0) + 2·10` 이다(배너 96.7 · 불릿 143.6). 그 식을 되풀면 선언 길이가 그대로 나온다:
         len = (스팬 − 20) ÷ v(0) + 20   ⇒ 배너 81 · 불릿 114
       ⚠ 옛 식(스팬 ÷ vmax)을 그대로 두면 76.9 / 109.2 로 **4~5px 어긋난 채 빨강**이라 자가 제품을
       못 따라간다. `NTC_LEN` 을 건드리면 여전히 빨개지는 것이 이 항이 지키는 것이다(불변). */
    ok(`A3 ${t}(${c.id}) 노치 폴리곤 세로 스팬을 «오프셋 10» 으로 되풀면 노치 길이 = ${E.sH}  (923 7회차 이관)`,
      c.ntc.every(n => near((n.sSpan - 20) / n.vmax + 20, E.sH, 1)),
      c.ntc.map(n => '(' + n.sSpan + '−20)/' + n.vmax.toFixed(3) + '+20='
        + ((n.sSpan - 20) / n.vmax + 20).toFixed(1)).join(' '));
  ...
  const two = (c, k) => {
    ...
    ok(`D2 ${t}(${c.id}) 리본 띠 높이 ${E.rb} (두 줄 다 · 969 이관 — 불릿 76 → 77 · 오차 ±1 → ±0.5)`,
      near(c.rb1.h, E.rb, 0.5) && near(c.rb2.h, E.rb, 0.5), c.rb1.h + '/' + c.rb2.h);
  ...
  /* ⚑ 969 이관 — 불릿 과녁 **8 → 8.5**. 969 가 띠 총 높이를 76 → 77 로 닫자 «테 8 + 속 60» 의
     상쇄가 풀렸다(속이 61 로 넘쳤다). 검정 테 교차로 ref 를 셋으로 가르면 «테 8.60 + 속 59.91 +
     테 8.60 = 77.11» 이라 **테가 8.5 여야 속 60 이 제자리**다 — 두 항이 같이 닫힌다.
     ⚠ 배너 7 은 그대로(형마다 두 값). 화소 대조는 `tools/verify969.js` [7]. */
  ok('[G7] 리본 검정 테(보이는 두께 = 테 + inset)가 불릿 8.5 · 배너 7 — ref 8.45~8.65 / 7.11~7.21 (969 이관 · 옛 8)',
    g10.every((r) => near(r.ink, r.ban ? 7 : 8.5, 0.6)), g10.map((r) => r.ink).join(' / '));
  ...
  /* ⚑ 969 이관 — 불릿 65 = 77 − 테 6×2(그림자를 지우면 inset 몫 5 가 통째로 속으로 간다). 옛 64 는
     띠가 76 이던 시절의 값이다 — 되돌림 시험의 «되돌린 뒤 값» 도 제품을 따라 움직인다. */
  ok('R9 inset 그림자를 지우면 [G7b] 이 빨개진다 (속이 65/55 = ref 보다 +8.5% 로 돌아간다 · 969 이관 · 옛 64)',
    after10.every((r) => near(r.inner, r.ban ? 55 : 65, 0.6)), after10.map((r) => r.inner).join(' / '));
