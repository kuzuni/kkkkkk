...
(async () => {
  ...
  /* ⚑ 833 3회차 이관 — 이 항이 못박던 것은 «521» 이라는 값이 아니라 **9회차가 좌단만
     ...
     **자리를 비우지 않고 방향만 갈아 끼운다**:
       · 리본1(424) 은 **여전히 8회차 자리 그대로** — 833 은 w1 을 안 건드렸다.
       · 리본2 는 833 3회차 자리(530).
     «판·꼬리 Δ0» 는 이 항이 아니라 [F3]·[F8] 이 직접 잰다(833 이 `--gx` 를 같은 Δ 만큼
     키워 판 중심을 붙박았고, 그래서 [F3] 이 무수정으로 초록이다). */
  ok('[G2] 리본1 우단은 8회차 자리 424 · 리본2 는 833 3회차 자리 530 (좌단·판·꼬리 Δ0)',
    g9.filter(r => r.id === 'noads').map(r => r.rbR).every((v, i) => near(v, [424, 530][i], 1.5)),
