 ...
 *
 * 지키는 성질: **오늘 남은 수령 횟수가 있는 광고 상품 버튼에만 레드닷이 뜨고,
 *               받아서 소진하면 그 칸만 즉시 꺼지며, 경로(탭바 «상점» · 재화 카테고리 탭)까지 이어진다.**
 *   [A] 전부 남은 국면 — 6칸 전부 점등 · 재화 탭 배지 점등 · 탭바 «상점» 칸 점등
 *   [B] 섞인 국면 — 2칸 소진(«구매 완료» 라 버튼 자체가 없다) · 나머지 4칸만 점등(짝이 안 어긋난다)
...
window.__ads = function(){
  ...
  return [...document.querySelectorAll('#shopList .cn-cd')].slice(0, COIN_ADS.length).map(cd => {
    ...
    return { done:false, id:bt.dataset.cnad, alert:bt.classList.contains('alert'),
      ...
      ad:(e=>{const r=e.getBoundingClientRect();return [r.left,r.top,r.width,r.height];})(bt.querySelector('.ad')) };
  });
};
...
(async () => {
  ...
  ok(live.length === 6, '[A] 광고 상품 6칸이 전부 살아 있다 (판정 재료)', live.length + '칸');
  ok(live.every(a => a.alert && a.dot && a.dot.display !== 'none'),
    '[A] 6칸 전부 점등 · 부품이 보인다 (#shopw 스코프 짝 되돌림 감시)',
    live.filter(a => a.alert && a.dot && a.dot.display !== 'none').length + '/6');
  ...
  ok(p0.length > 0 && p0.every(p => p.q[0] > 0.5 && p.q[1] < 0.5), '[G] 우상단 사분면(299) — «받기» 국면',
    p0[0].q.map(v => v.toFixed(3)).join(','));
  ok(p0.length > 0 && p0.every(p => p.slack > 0), '[G] 봉우리 1.3 에서도 라벨·(n/n)·▶AD 를 안 밟는다 — «받기» 국면',
  ...
  ok(pn.length === 6 && pn.every(p => p.slack > 0 && p.q[0] > 0.5 && p.q[1] < 0.5 && p.clip.every(v => v > 0)),
  ...
  /* ══ [B] 섞인 국면 — 2칸 소진 ═══════════════════════════════════════════ */
  await page.evaluate(() => { S.daily.adBuy = { a1: 0, a4: 0 }; openShopPage(null, 'coin'); syncShopSumBtns(); });
  await page.waitForTimeout(400);
  ads = await page.evaluate(() => __ads());
  ch = await page.evaluate(() => __chain());
  const done = ads.filter(a => a.done), alive = ads.filter(a => !a.done);
  ok(done.length === 2 && alive.length === 4, '[B] 2칸이 «구매 완료» 로 잠긴다 (판정 재료)',
    '소진 ' + done.length + ' · 남음 ' + alive.length);
  ok(done.every(a => a.dots === 0), '[B] 소진 칸에는 닷 노드가 아예 없다(죽은 마크업 0)',
    '남은 노드 ' + done.reduce((s, a) => s + a.dots, 0));
  ok(alive.length > 0 && alive.every(a => a.alert && a.dot && a.dot.display !== 'none'), '[B] 남은 4칸만 점등', alive.length + '/4');
  ...
  const after = await page.evaluate(() => {
    ...
    return { relic: S.relic, left: adLeft(COIN_ADS[1]), stillThere: !!bt,
      alert: bt ? bt.classList.contains('alert') : null, dots: document.querySelectorAll('#shopList .cn-cd .bt[data-cnad="a2"] .updot').length };
  });
  ...
  const audit = await page.evaluate(() => {
    ...
    const hosts = [...document.querySelectorAll('#shopList .cn-cd>.bt[data-cnad]')].filter(h => h.querySelector('.updot'));
  ...
  ok(audit.n === 6 && audit.offBad === 0 && audit.onBad === 0,
