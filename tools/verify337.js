/* 작업 337 — 41 팝업 내장 재화 바(.pcb) 재화 아이콘 «잉크» 게이트.
 *
...
async function capInk(page, kind, sel) {
  ...
  await page.evaluate(() => {
    if (document.getElementById('v337-freeze')) return;
    const s = document.createElement('style'); s.id = 'v337-freeze';
...
(async () => {
  ...
  try {
    ...
    ok('[2] 젬 실루엣 세로는 자산 몫이라 ref −5px 다 (기록만 — 아트 필요표 337)',
    ...
    ok('R-2 보정을 걷어내면 젬 시안 몸통이 −15% 아래로 떨어진다',
      (bad.dia.col.w - REF.dia.col[0]) / REF.dia.col[0] * 100 < -15,
  ...
  console.log('\nVERIFY337 ' + (fail === 0 ? 'PASS' : 'FAIL') + ' — ' + pass + '/' + (pass + fail));
