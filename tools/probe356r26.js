 ...
 *
 * ⚑ 이것이 «있을 법한 구멍» 이 아니라 **실재하는 구멍**인 근거는 소스에 있었다:
 *   `index.html:1060`(26회차 당시)  `.ibtn.lock::after{content:'🔒'; … font-size:calc(var(--ih,82px)*.66)}`
 *   A2 좌측 사이드의 **잠금 자물쇠**는 순전히 의사 요소로 그려졌다. 그 형제 아이콘 `span.si` 는
 *   [A] SCOPE 의 첫 줄부터 감시 중인데, 그 위에 얹히는 🔒 는 **감시 밖**이었다.
 *   ⚠ **그 선언은 2026-09-01 에 사라졌다** — 호스트가 없어 한 번도 안 걸리는 죽은 규칙이었고
 *     **629 가 선언째 걷어냈다**(`tools/verify629.js` 23/23). 구멍이 실재했다는 근거로는 그대로
 *     남지만(그 규칙이 있었을 때 감시 밖이었던 것은 사실이다), **지금 소스에서 찾지 마라.**
 *     이 자의 축은 규칙 수가 아니라 «화면에 그려진 의사 요소» 라 삭제의 영향은 0이다.
 *
 * ── 무엇을 세는가 ──────────────────────────────────────────────────────────
...
(async () => {
  ...
  if (REVERT) {
    /* ⚑ 되돌림이 이 회차의 본체다 — 새 축이 [A] 의 재탕이 아니라는 것은 말이 아니라 수로 선다.
       ...
       ⚠ 주입 자리를 `.ibtn.lock::after`(26회차 당시 제품에 있던 유일한 그림문자 의사 규칙)로
         잡으면 안 됐다 — 그 셀렉터는 **DOM 에 한 번도 안 붙었다**(작업 49 가 `.ibtn[data-lock]` 을
         지웠다 · **629** 로 등재 — 등재 경쟁으로 626 에서 번호가 개정됐고, 629 가 그 두 줄을
         선언째 걷어냈다). 호스트가 없으면 의사 요소도 없어서 «주입해도 0자리» 가 나오고,
         그건 자가 눈먼 것이 아니라 **표본이 없는 것**인데 되돌림 시험은 그 둘을 못 가른다.
         ⇒ 규율은 삭제 뒤에도 그대로다: **살아 있는 호스트에 건다.** */
