...
const LEDGER = {
  ...
  'probe866.py': {
    ...
    fix: "**없음(면역).** ⚑ 갈고 나니 테가 가로 2.31 ↔ 세로 2.65 로 갈렸는데 **그것은 창의 인공물**이다 — 세로를 재는 열 `vx = l + 10` 이 알약의 둥근 캡 어깨 위라 테를 비스듬히 가로지른다(열 스윕: 가운데 중앙값 **2.23** ≈ 가로 2.31 · 캡 **4.91**). ⇒ 904 의 «등방 2» 는 살아 있고 `verify866` 과녁은 **안 옮겼다**. 창을 옮기는 것은 «재는 것을 바꾸는» 일이라 **945** 로 등재. 자가 매 실행 다시 찍는다 — `probe866.py --ring-sweep` · `verify932` [7-f][7-g].",
  },
  'scan934.py': {
