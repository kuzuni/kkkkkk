...
async function measure(browser, url) {
  ...
  const out = await ev(() => {
    ...
    /* ⚑⚑ 985 — **궤도각을 선언값에 세운다**(855 «주사위» · 936 «상자» 에 이어 세 번째 축).
       `orbitAng` 은 `step()` 이 `orbitAng += dt*2.4` 로 **누적**하는 각이고(index.html 26891·26914),
       그 `dt` 는 진짜 시계다 — `page.goto` 뒤 1.1초와 바로 아래 `putFoe()` 의 «적이 나올 때까지»
       루프가 그 값을 **판마다 다른 자리**에 놓는다. `spiral` 은 발사각을 `base = orbitAng*0.7`
       로 잡으므로(26291) 같은 발이 판마다 다른 각으로 찍혀 본체 bbox 가 **89×112 · 91×112 ·
       93×111** 로 흔들렸다(등재 실측 · 본체 화소 수는 4790~4805 로 거의 그대로 = «자리» 가 아니라
       «각» 이 흔들린 것이다). 못박으면 4판이 **105×97 · 본체화소 4862** 로 붙박이가 된다.
       ⚠ **`putFoe()` 뒤**여야 한다 — 그 안의 `step()` 이 각을 다시 굴린다.
       ⚠ 세우는 값은 손 상수가 아니라 **선언값 0**(index.html 25173 `orbitAng = 0`)이다 — 936 이
         상자를 제품의 «집»(`WORLD.w/2`)에서 판 것과 같은 자리다(402 «사본을 지운다»).
       ⚠ 등재문이 준 첫 후보(«1.1초를 프레임 수로 바꾼다»)는 **재현이 기각했다** — 프레임 수로
         바꿔도 `dt` 가 진짜 시계라 각이 그대로 흔들린다. 갈래를 가른 표는 `tools/probe985.js`. */
    putFoe(); orbitAng = 0;
