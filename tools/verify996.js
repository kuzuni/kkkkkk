 ...
 *   **999 로 등재된 잔여**여서, 지우거나 상자를 좁혀 감추지 않고 «안/밖» 을 갈라 둘 다 찍었다 —
 *   **밖이 0 건**이어야 통과이고, 안의 값은 판정이 아니라 **관찰**이었다(래칫 [2-d] 가 크기만 묶었다).
 * ⚑ **999 가 그 잔여를 닫았다(2026-09-06 · 이관).** 뿌리는 그림이 아니라 **부스러기**였다 —
 *   `cap710` 의 `clearFx` 가 여덟 배열을 치우면서 `nums` 를 빠뜨려, 규격 시전이 남긴
 *   **피해 숫자 한 장**이 마지막 줄 `rico` 칸에 찍혔고 그 값이 «용사 자리» 드리프트를 타고 갈렸다.
 *   ⇒ [2-d] 를 래칫에서 **판정(안도 0)** 으로 조였다. 2,000 에 둔 채 지나갔으면
 *     «999 가 통째로 되돌아가도 초록인 게이트» 가 된다. 되돌림 시험은 `tools/verify999.js` [R].
 *
...
(async () => {
  try {
    ...
    /* [2-d] **999 이관(2026-09-06)** — 여기는 «잔여가 커지지만 마라» 는 래칫(안 ≤ 2,000화소)이었다.
       999 가 그 잔여를 닫았으므로 **래칫을 판정으로 조인다** — 뿌리는 `cap710` 의 `clearFx` 가
       `nums` 를 안 치워 규격 시전이 남긴 **피해 숫자 한 장**이 `rico` 칸에 찍히던 것이고,
       그 글자 값이 «용사 자리» 드리프트를 타고 판마다 갈렸다(등재 999 · `docs/review/999-*.md`).
       ⚠ **래칫을 2,000 에 두고 지나갔으면 «999 가 통째로 되돌아가도 초록인 게이트» 가 된다**
         (ROUTINE 328-330 이 «이관이 이 작업의 핵심 교훈» 으로 못박은 자리와 같은 꼴).
         잔여를 «관찰» 로 두던 문장도 같이 걷는다 — 이제 관찰할 잔여가 없다.
       ⚠ 되돌림 시험까지 갖춘 자는 `tools/verify999.js` [R] 이다(이 항은 «되돌아왔는지» 만 본다). */
    ok('[2-d] 999 잔여 0 — 블록 «안» 도 흔들림 0 (999 가 닫았다 · 이관)',
       fr.all - fr.out === 0, '안 ' + (fr.all - fr.out) + '건 · 전체 ' + fr.all +
       (fr.bx ? ' · bbox x ' + fr.bx.x0 + '..' + fr.bx.x1 + ' y ' + fr.bx.y0 + '..' + fr.bx.y1 : ''));
    if (fr.all > fr.out) info('999 잔여가 되살아났다 — `node tools/verify999.js` 로 뿌리를 확인하라');
