 ...
 *   [6] **남은 몫은 리본이고 여기서 갚지 않았다** — 두 리본 높이가 여전히 선언 76 이다(969 등재).
 *       이 항이 빨개지면 969 가 처리된 것이니 [1] 과녁이 아니라 **이 주석**을 갱신하라.
 *   [R] 되돌림 시험 — `height:72px;margin-bottom:10px` 사본을 실제로 먹이면 틈이 **47 → 49** 로
 *       벌어지고 [1] 이 빨개진다(무르게 푼 수리가 아님을 증명한다).
 *
...
(async () => {
  ...
  /* ── [6] 남은 몫은 **리본**이고 여기서 안 갚았다(969 등재) ───────────── */
  {
    const hs = shots[2280].cards.filter((c) => !c.cls.includes('ban1')).map((c) => parseFloat(c.rbCss));
    ok(hs.every((h) => Math.abs(h - 76) < 0.01),
      '[6] 불릿형 리본 띠 높이가 **여전히 76**이다 — 틈의 나머지 ~1.0px 을 알약으로 갚지 않았다(969)',
      hs.join(' / '));
  }
