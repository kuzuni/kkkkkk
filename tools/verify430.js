 ...
 *                문양 8종 · 잉크 bbox 21×21 · 중심 (32,35) — 433 이 35.5 에서 0.5 올렸다.
...
/* ⚑ 490(주인 확정 2026-08-30) — 두 칸이 **자리를 맞바꿨다**: 수정 광산 초록 → **하늘색** ·
   룬의 제단 파랑 → **초록**(룬강화석 재화 아이콘을 초록으로 바꾸면서 그 입장권도 같이 간다).
   나머지 여섯은 430 그대로다. */
const NAMED = {
  gold:   ['황금 동굴', '노랑'], dia:    ['수정 광산', '하늘색'],
  relic1: ['고대 유적', '갈색'], relic2: ['잊힌 신전', '회색'],
  relic3: ['용의 무덤', '빨강'], relic4: ['창세의 제단', '흰색'],
  stone:  ['각성의 동굴', '주황'], rstone: ['룬의 제단', '초록'],
};
const WINDOW = {
  '노랑': { h: [45, 65],   c: [40, 999], l: [70, 95] },
  /* 490 — «초록» 이 룬의 제단으로 옮겨오면서 **어두운 초록**이 됐다. L* 하한을 40 → 32 로 내린 것은
     느슨하게 푼 것이 아니라 **회색조 사다리가 그 자리를 지정하기 때문**이다: 고정 여섯의 L* 가
     30·46·56·74·86·100 이라 «인접 8 이상» 을 지키며 새 색이 들어갈 수 있는 구간은 37.5~38.4 와
     63.7~66.0 **둘뿐**이다(490 review §4 의 산수). 어두운 초록도 초록이다 — 색상각은 그대로 100~150. */
  '초록': { h: [100, 150], c: [40, 999], l: [32, 85] },
  /* 490 — «하늘색». 채도 하한이 40 이 아니라 30 인 것은 밝은 하늘색이 sRGB 에서 구조적으로
     C 40 을 넘기 어렵기 때문이고, L* 상한 72 는 **흰 문양 대비**(INK_MIN 2.4)가 정하는 값이다
     (L* 68.6 을 넘으면 흰 문양이 채움과 2.4 대비를 못 낸다 — 등재문의 «L* ≥ 70» 은 그래서 못 쓴다). */
  '하늘색': { h: [185, 208], c: [30, 999], l: [58, 72] },
...
(async () => {
  ...
  if (!missing.length) {
    ...
    if (!guard(BOX, 'C4')) {
      ...
      /* 433(2026-08-30): 중심 y 35.5 → **35**(속띠 23..47 의 한가운데) · 덩치는 Δ0. 자는 tools/verify433.js. */
      const off = KEYS.filter((k) => Math.abs(BOX[k].cx - 32) > 1 || Math.abs(BOX[k].cy - 35) > 1);
      ok(off.length === 0, 'C5 문양 중심이 8장 같은 자리다 (32, 35) ±1', off.length ? off.join(',') : '8/8');
    }
  }
