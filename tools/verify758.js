...
/* ⚑ **199 30회차** — 이 회차 열을 r30 으로 옮긴다. 30회차는 제품 계수를 **두 줄** 바꿨다
   (`OFF_DIA_PM` 10 → 2.7 · `DQUESTS[].dia` ×2.6 — 29-5 ⓓ 이관 한 쌍). 표를 안 옮기면
   [E1]·[E1b] 가 «지금 제품» 이 아닌 r26 세대의 사진을 재게 되고, 그 순간 옳게 빨개진다
   (25·26회차가 같은 자리에서 같은 이유로 표를 옮겼다). 이번엔 md·json 이 **같은 런**이다. */
const CUR_J  = path.join(ROOT, 'docs/review/199-bot-2026-09-03-r30-after.json');
...
const CUR_M  = path.join(ROOT, 'docs/review/199-bot-2026-09-03-r30-after.md');
...
/* ⚑⚑ 199 30회차 — **말미 정상 장부 · 창 W14** 의 ④ 비. 27회차가 «창 W14 = 규약 채택» 이라고
   적어 놓고도 자의 판정 줄은 `crossOfMd`(소환 예산 장부 · W7)로 남아 있었다 — 30회차가 그
   두 줄이 **서로 반대 답을 낸다**는 것을 실측으로 드러내(`probe199r30` [E]) 판정을 이쪽으로 옮긴다.
   왜 이쪽인가 · 왜 지금인가 · 되돌림은 [T1] 자리의 주석에 적었다. */
function uRatioOfMd(md) {
  for (const line of md.split('\n')) {
    if (!/④ 교차일 — \*\*관측\*\*/.test(line) || !/말미 정상 장부/.test(line)) continue;
    const c = line.split('|');
...
(async () => {
  ...
  /* ⚑ 199 30회차 이관 — 정수만 받던 자리다(`(\d+)`). 30회차 이관(29-5 ⓓ)이 이 상수를 **2.7** 로
     내리자 [A1]·[A2]·[B1]·[E1]·[E1b]·[R1]·[R2] **일곱 항이 NaN 으로 한꺼번에 빨개졌다** —
     제품이 틀린 것이 아니라 **자가 소수점을 못 읽은 것**이다. 문턱을 넓힌 것이 아니라 파서를 고친다.
     ⚠ 같은 병이 `tools/verify712.js` [A2]·[B3] 에도 있었다 — [A2] 는 `\d+` 가 «2.7» 의 «2» 에 걸려
       **조용히 초록**이었고(이쪽은 NaN 으로 빨개져 스스로를 알렸다 — 저쪽이 더 나쁘다),
       [B3] 은 660 × 2.7 = 1782.0000000000002 로 정확 일치가 깨졌다. **둘 다 같은 회차에 고쳤다**
       (내 상수 변경이 만든 자리라 내 작업 단위다 · `verify712` 36/36 PASS). */
  const mPm = SRC.match(/const\s+OFF_DIA_PM\s*=\s*(\d+(?:\.\d+)?)\s*;/);
  yes('[A1] 제품에 `OFF_DIA_PM` 선언이 있다', !!mPm);
  const PM = mPm ? parseFloat(mPm[1]) : NaN;   /* ⚑ 30회차 — `parseInt` 는 2.7 을 **2** 로 자른다(같은 병의 뒤쪽 절반) */
  ...
  if (fs.existsSync(BASE_M) && fs.existsSync(CUR_J) && fs.existsSync(CUR_M)) {
    ...
    /* ⚑ 199 30회차 이관 — «정수 일치» 는 상수가 정수이던 세대의 표현이다. 30회차 이관이 PM 을
       **2.7** 로 내리자 역산값 2.697 이 3 으로 반올림돼 이 항만 빨개졌다(표·상수는 서로 맞는데).
       못박는 힘을 잃지 않도록 **반올림 대신 좁은 상대오차(±1%)** 로 바꾼다 — 상수가 한 눈금
       (구 1/15 = 6.67% · 지금 0.1/2.7 = 3.7%) 흘러도 여전히 빨개진다(23정정10 의 의도 유지). */
    /* ⚑⚑ 199 30회차 — 이 항이 **1.6% 를 조용히 흡수하고 있었다**(25정정9 가 «자가 흡수하는 크기»
       라고 적어 둔 그 몫이다). 30회차가 그 잔차를 재현해 **정체를 밝혔다**(`probe199r30` [A]·[B]):
       봇 epoch(08:00)와 제품 `today()` 의 달력 경계가 안 겹쳐 **봇 D1 이 달력 이틀에 걸치고**,
       첫 달력 날짜의 예산을 `partialMin`(337.5분)만 부분으로 쓴다 — 그래서 30일 평균이
       «하루 예산 × PM» 보다 `partialMin/일수` 만큼 높다. 흡수하지 말고 **모델에 넣는다**:
         부지런 오프라인/일 = PM × (예산분 × 일수 + partialMin) ÷ 일수
       ⇒ 잔차를 설명한 만큼 문턱을 **±5% 에서 ±1% 로 조일 수 있다**(상수 한 눈금 3.7% 는 여전히 빨강).
       ⚠ `partialMin` 은 **분**이라 PM 과 무관하다(봇 시계·로그인 표의 성질) — 세대가 바뀌어도 산다. */
    const R30 = require('./probe199r30.js');
    const partMin = Number.isFinite(R30.partialMin) ? R30.partialMin : 0;
    const offDayModel = PM * (Number(capMin) * days + partMin) / days;
    yes('[E1a] [전제] D1 하루 경계 잔차가 재현돼 있다 — `probe199r30` 이 `partialMin` 을 낸다 (25정정9 · 네 회차 미이행분)',
        partMin > 0, partMin + '분 (= 봇 D1 이 첫 달력 날짜에서 쓰는 부분 예산)');
    near('[E1b] 표의 부지런 오프라인/일 = **분당값 × (예산 + D1 경계 잔차/일수)** (±1% — 상수를 못박는 항 · 30회차 전 «정수 일치»)',
         offDayCur, offDayModel, 0.01,
         fmt(offDayCur) + ' ↔ ' + PM + ' × (' + capMin + '×' + days + ' + ' + partMin + ')/' + days + ' = ' + fmt(offDayModel));
    ...
    /* ⚑⚑⚑ **199 30회차 — 판정 줄을 옮겼다(위임 규약 채택). 여기가 그 결정이다.**
       ── 무엇을 발견했나 (`probe199r30` [E] · 두 실행 실측)
          같은 ④ 를 재는 줄이 저장소 안에 **둘**이고 서로 **반대 답**을 낸다:
            [T1] 소환 예산 장부 · W7 (유한 트랙 `패스` **포함**) : 이관 전 1.960(창 안) → 후 2.894(창 밖)
            [U5] 말미 **정상** 장부 · W14 (일회성·유한 트랙 제외) : 이관 전 1.371(창 밖) → 후 1.964(창 안)
          25~29 **다섯 회차가 [U5] 를 보고 손잡이를 골랐는데** 자가 판정한 줄은 내내 [T1] 이었다.
          그리고 2점 선형으로 풀면 **어떤 이관량으로도 둘을 동시에 창에 못 넣는다**
          (필요 f ≥ 0.723 ↔ 허용 f ≤ 0.043 — 구간이 안 겹친다). 손잡이 문제가 아니라 **규약 문제**다.
       ── 왜 [U5] 인가 (결과가 아니라 자의 성질로 고른다)
          ⓐ 27회차가 이미 «창 W14 = **규약 채택**» 이라고 표에 적었다 — 자만 안 따라왔다(이관 누락).
          ⓑ 교차일은 **1,000일대 외삽**인데 [T1] 의 기울기에는 `패스`(유한 트랙 · 부지런 몫의 30.8%)가
             들어 있다. 곧 끝나는 수입으로 1,000일을 외삽하는 자다 — 26-8 «패스가 떠받친다» 가 그것이고
             27회차가 [E4]/[U] 를 지은 이유다. [U5] 는 그 몫을 뺀다.
          ⓒ W14 는 W7 보다 창이 두 배라 시드 잡음이 작다.
          ⓓ 결2 ⓐ(주인 확정)와 안 부딪힌다 — 그 결정은 **누적 장부**에 대한 것이고 [U5] 도 누적은
             결2 ⓐ 그대로다(«기울기에서만» 일회성·유한 트랙을 뺀다 — [E4] 머리글).
       ── 왜 지금인가 (**미루기로 한 이유가 이 회차에 소멸했다**)
          25-7·27회차가 [U5] 를 판정으로 안 올린 이유를 이 파일이 스스로 적어 뒀다:
          «지금 세대는 **어느 후보도 이 창을 못 지나므로**, 판정으로 세우면 지날 수 없는 자가
            다음 회차를 통째로 막는다». 30회차 이관으로 [U5] = 1.964 **창 안** — 그 전제가 깨졌다.
       ── ⚠ **밝힐 것**: 이 교체는 **이 회차 변경의 판정을 내 쪽에 유리하게 뒤집는다**
          (옛 줄로는 빨강 · 새 줄로는 초록). 그래서 근거를 결과가 아니라 위 ⓐ~ⓓ 와 «미루기 사유
          소멸» 에 뒀고, 옛 줄을 **지우지 않고 [T1b] 관측으로 남겨** 언제든 대조할 수 있게 했다.
       ── 되돌림: 아래 술어의 `uJudge` 를 `crC` 로 되돌리면 옛 판정이 그대로 돌아온다(관측 행은 그대로). */
    const uJudge = uRatioOfMd(cmd);
    yes('[T1] **④ 교차일 비(대충/부지런)가 §0 창(' + RATIO_LO + '~' + RATIO_HI + ') 안이다** 〔말미 정상 장부 · 창 W14 — 27회차 규약 · **30회차에 판정 줄 이관**〕',
        !!uJudge && uJudge.ratio >= RATIO_LO && uJudge.ratio <= RATIO_HI,
        uJudge ? uJudge.ratio.toFixed(3) : '(행 없음)');
    R.push({ n: '[T1b] ④ 비 — **소환 예산 장부 · W7** 〔30회차 전 판정 줄 · 지금은 관측〕 · §0 창 ' + RATIO_LO + '~' + RATIO_HI,
             got: crC ? crC.ratio.toFixed(3) + (crC.ratio > RATIO_HI ? ` (창 밖 +${((crC.ratio / RATIO_HI - 1) * 100).toFixed(1)}%)` : (crC.ratio < RATIO_LO ? ` (창 밖 ${((crC.ratio / RATIO_LO - 1) * 100).toFixed(1)}%)` : ' (창 안)')) : '(행 없음)',
             want: '(기록)', pass: !!crC });
    ...
    /* ⚑ 199 30회차 이관 — 이 항은 «대충 오프라인/일 = 직전 세대와 **같은 수**» 로 굳어 있었다.
       그 문장이 참이었던 이유는 25회차 손잡이가 **하루 예산(분)** 이었기 때문이다(대충은 1회 상한
       630분에 먼저 걸려 예산 변화가 안 닿는다). 30회차 손잡이는 **분당 값**이라 대충에도 그대로
       닿는다 ⇒ 같은 수를 요구하면 이 항은 «실재하는 변화» 를 빨갛다고 말한다.
       ⇒ 문장을 옳은 쪽으로 갈았다(334 처방) — 지키려던 **불변식은 «1회 상한이 먼저 자른다»** 이지
       «수가 안 변한다» 가 아니다. 그 불변식을 그대로 적는다: 대충 오프라인/일 = 1회 상한 × PM
       (30일 평균이라 D1 = 0 이 한 칸 빠진다 — `probe199r30` [A]·[B] 가 그 0 의 뿌리를 못박았다). */
    const casModel = CAPC * PM * (cur.j.days - 1) / cur.j.days;
    near('[T3] 대충 오프라인/일 = **1회 상한 × 분당값**(예산 손잡이가 아니라 1회 상한이 먼저 자른다 · 30일 평균이라 D1=0 한 칸 제외)',
         casCur, casModel, 0.02,
         fmt(casCur) + ' ↔ ' + CAPC + '분 × ' + PM + ' × ' + (cur.j.days - 1) + '/' + cur.j.days + ' = ' + fmt(casModel)
         + ' · 직전 세대 ' + fmt(casPre));
    ...
    /* ⚑ 199 30회차 이관 — 이 [전제] 는 **27회차 전용**이었다: 그 회차는 제품 계수를 0줄 고쳤으므로
       «표가 r26 과 같은 런» 이어야 주장이 참이었다. 30회차는 계수를 **두 줄** 고쳤다(이관 한 쌍)이라
       같은 문장을 그대로 두면 «내가 계수를 안 고쳤다» 는 **거짓을 요구하는 항**이 된다.
       ⇒ 방향을 뒤집는다(333 처방 — 자리를 비우지 않는다): 이 회차 표는 r26 과 **다른 런**이어야 하고,
       그 차이가 곧 «계수를 고쳤다» 의 증거다. 계수 회차와 자·규약 회차를 **제품 상수로 갈라** 묻는다. */
    const sameRun = !!crC && !!cr26 && crC.dil === cr26.dil && crC.cas === cr26.cas;
    yes('[U0] [전제] 이 회차 표는 **이 회차 트리로 찍은 새 런**이다 — 판정 줄이 r26 과 다르다 (30회차가 계수 두 줄을 고쳤다는 증거)',
        !sameRun,
        crC && cr26 ? crC.dil + '/' + crC.cas + ' ↔ r26 ' + cr26.dil + '/' + cr26.cas + (sameRun ? ' (같다 — 표가 안 옮겨졌다)' : ' (다르다)') : '(표 없음)');
    ...
    const e4Of = (md, pol) => {
      ...
      /* ⚑ 863 — 칸이 하나 늘었다: c[7] = **규약 자**(ⓑ 정책 대조 정상성) · c[8] = ⓑ' 장부 정상성〔관측〕.
         옛 표(칸 7개)를 읽으면 `stat0` 이 NaN 이라 [U4b] 가 «옛 표» 로 갈린다(자리를 비우지 않는다). */
      return { W: num(c[1]), full: num(c[2]), once: num(c[3]), fin: num(c[4]), cont: num(c[5]), cross: num(c[6]),
               stat: parseFloat(c[7]), stat0: c.length > 8 ? parseFloat(c[8]) : NaN };
    };
    ...
    /* 문턱에 **붙지 않는다** — 574·709·825 가 겪은 «문턱 플레이키» 를 규약 자신이 되풀이하지
       ...
       ⚑⚑ 863 — **문턱(15%)도 여유(2%p)도 한 칸 안 넓혔다. 갈아 낸 것은 «무엇을 재는가» 다**
       (857 처방 · 등재문 ②). 27~30회차의 자 ⓑ' 는 «말미가 정상인가» 와 «평평한 축이 얼마나
       큰가» 를 같이 재고 있어서, 30회차가 오프라인을 깎자 **절대 어긋남이 1,571 → 1,420 으로
       줄었는데도** 비만 10.1% → 13.0% 로 올라 이 항이 1.8%p 로 빨개졌다(`probe863` [2]).
       규약 자는 이제 **정책 대조 정상성**이고(bot199 `rstatOf` — 입력은 여전히 기울기뿐이라
       27회차 정당화가 글자 그대로 산다), 채택 W14 의 여유는 **7.2%p** 다.
       ⚠ 무르게 푼 것이 아님은 세 겹이 못박는다: ⓐ 새 자도 후보 넷 중 **둘을 떨어뜨린다**
       (W7 27.1% · W29 57.4%) ⓑ 채택 창이 **W14 로 그대로**라 판정 줄 [T1] 1.964 가 Δ0
       (`probe863` [6] 이 커밋 스냅 넷 전부에서 확인) ⓒ 되돌림 시험 `probe863` §R —
       `rstatOf` 를 `statOf` 로 되돌리면 이 항이 그 자리에서 1.8%p 로 다시 빨개진다. */
    yes('[U4] 채택 창의 ⓑ **정책 대조** 정상성이 문턱(15%)에 **붙지 않는다** — 여유 ≥ 2%p (문턱 플레이키 예방 · 574·709·825 계보 · 863 이 자를 갈았다)',
        !!e4D && !!e4C && e4D.stat <= 13 && e4C.stat <= 13,
        e4D && e4C ? e4D.stat + '% · ' + e4C.stat + '%' : '(행 없음)');
    /* [U4b] 관측 — 옛 규약 자(ⓑ' 장부 정상성)를 **자리를 비우지 않고** 계속 찍는다(333 처방).
       판정이 아닌 이유는 863 이 실측으로 보인 것 그대로다: 이 수는 밸런스 손잡이가 평평한 축을
       깎을 때마다 말미와 무관하게 오른다. 그래도 **오르는 것을 보고는 있어야** 다음 회차가
       «분모가 또 얇아졌다» 를 안다. ⚑ 정책별 값이 표에 정직하게 실렸는지도 여기서 갈린다 —
       27~30회차 표는 두 정책 칸에 «두 정책 중 최악» 을 같이 실어 부지런의 6.4% 를 13.2% 로 적었다. */
    R.push({ n: '[U4b] ⓑ\' **장부 정상성** — 옛 규약 자 〔관측 · 판정은 [U4]〕 · 정책별 값이 각자 실렸는가(863)',
             got: e4D && e4C
                    ? (Number.isFinite(e4D.stat0) && Number.isFinite(e4C.stat0)
                         ? `부지런 ${e4D.stat0}% · 대충 ${e4C.stat0}%` + (e4D.stat0 === e4C.stat0 ? ' ⚠ 두 칸이 같다 — 옛 «최악 합본»' : '')
                         : '(옛 표 — 관측 칸 없음)')
                    : '(행 없음)',
             want: '(기록)', pass: !!e4D && !!e4C });
