 ...
 *    «칸4 좌상 30° F7.0» 이 그것이다(462 의 결함이 아니라 z-순서 문제 · **465** 로 등재).
...
const DEGS = [45, 60, 75];       /* 아래 코너 — 구분선(465)이 안 닿는 각도 */
const TOP_DEGS = [45, 60, 75];   /* 위 코너 — 같은 이유 */
const REC_DEGS = [0, 15, 30];    /* [기록] 전용 — 465 가 사는 각도 */
  ...
  try {
    ...
    for (const [hn, sel, setup, endIs, midI] of HOSTS) {
      ...
      for (const endI of endIs) {
        ...
        /* [기록] 465(구분선 z-순서)가 사는 각도 — 값만 남긴다 */
        const Rec = await scan(page, p, farB, REC_DEGS);
        console.log('    [기록] ' + tag + ' ' + farB + ' 0~30° (구분선 `.stab-sep` 이 지나는 자리 = **465**) — '
