...
(async () => {
  ...
  try {
    ...
    /* 클리어를 «진짜로» 돌린다 — 전장을 비우고 step 을 한 틱 태우면 클리어 분기가 걸린다 */
    const run = async (first) => page.evaluate((first) => {
      S.stage = 12;
      S.best = first ? 12 : 999;          /* first = 최고 기록 갱신(다이아 지급) */
      S.bossFarm = false; promo = null;
      player.dead = 0;
      enemies.length = 0; spawnQ.length = 0;
      msgT = 0; msgTxt = ''; msgLast = ''; msgLastT = -1e9;
      const dia0 = S.dia;
      step(0.016);
      return { txt: msgTxt, diaGain: S.dia - dia0, msgT };
    }, first);
    ...
    const rows = [
      ['① 클리어 문구(최고 기록 갱신)', a.txt],
      ['① 다이아 지급', a.diaGain + ' (>0 이어야 한다)'],
    ...
    const ok = !bad(a.txt) && !bad(b.txt) && a.diaGain > 0 && ink.n > 0 && ink.x0 >= 0 && ink.x1 < ink.w && !errs.length;
    console.log('\nPROBE160 ' + (ok ? 'OK' : 'BAD'));
    process.exit(ok ? 0 : 1);
  } catch (e) {
