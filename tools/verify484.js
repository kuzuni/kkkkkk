 ...
 *   [E] 실전 — 헤드리스 60초 전투에서 실측 타격수 ↔ 모델 발수가 ±10% 안(114 하네스 준용)
...
/* 484 이전의 «안 건드리는 축» — [cd, hits 선언]. 484 는 m 만 바꾸므로 이 표는 영구히 같아야 한다.
   (hits 가 빈칸인 것은 선언이 없다는 뜻 — 모델이 «1» 로 떨어지는 자리다) */
const OLD_SHAPE = { slash:[0.85,''], shuri:[2.20,''], stone:[1.30,1], curve:[1.60,2],
  multi:[1.10,''], orbit:[0.00,''], ice:[1.60,''], whirl:[1.60,2], aura:[0.00,''],
  bolt:[1.40,''], arrow:[1.50,2], rico:[2.00,2], spiral:[2.60,6], boom:[2.00,''],
  poison:[3.20,5], drain:[2.60,''], frost:[2.10,4], bounce:[2.20,5], boomer:[2.40,2],
  meteor:[4.00,''], gale:[2.80,12], drone:[6.00,16], flask:[3.60,4.5], holy:[3.00,''],
...
(async () => {
  ...
  const A = await page.evaluate(() => {
    /* 제품의 `stat.dps` 소스를 직접 읽어 게이트 모델이 그것과 같은지 본다 — 상수를 손으로
       옮겨 적는 게이트는 제품이 움직이면 조용히 부패한다(319·333·334 계열). */
    const src = Object.getOwnPropertyDescriptor(stat, 'dps').get.toString();
    return { hasShuri: /shuri'\s*\?\s*8/.test(src), hasBolt: /bolt'\s*\?\s*3/.test(src),
             hasMulti: /multi'/.test(src), hasCd0: /:\s*dmg\s*\*\s*3/.test(src),
             ref: SK_DPS_REF, n: SKILLS.length };
  });
  ok(A.hasShuri && A.hasBolt && A.hasMulti, 'A1 게이트 발수 모델 = `stat.dps` 의 모델(shuri 8 · bolt 3 · multi 가변)');
  ok(A.hasCd0, 'A2 지속형(cd 0)은 제품도 «×3» 으로 센다');
  ok(A.n === 27, 'A3 스킬 27종', String(A.n));
  ok(Math.abs(A.ref - 1.84) < 1e-9, 'A4 SK_DPS_REF = 1.84 (27종 기본 DPS 의 평균에서 온 값)', String(A.ref));

  /* ── [B] 등급 안 DPS 동일 ───────────────────────────────── */
  const B = await page.evaluate(() => {
    /* multi 는 발수가 강화 Lv 로 자란다 — 티어 축의 자는 23494 규약대로 Lv1 기준 3발 */
  ...
  const C = await page.evaluate(() => {
    ...
    const H = s => s.id === 'shuri' ? 8 : s.id === 'bolt' ? 3 : s.id === 'multi' ? 3 : (s.hits || 1);
    const real = s => skillDmg(s) * H(s) / (s.cd > 0 ? s.cd : 1 / 3);
    const per = [];
    GRADE.forEach((_, g) => {
      const t = SKILLS.filter(s => s.g === g && !s.sup);
      if (!t.length) return;
  ...
  /* ── [D] 안 건드린 축 ───────────────────────────────────── */
  const D = await page.evaluate(old => {
    const badCd = [], badHit = [];
    SKILLS.forEach(s => {
      const o = old[s.id];
      if (!o) { badCd.push(s.id + '(표에 없음)'); return; }
      if (Math.abs(s.cd - o[0]) > 1e-9) badCd.push(s.id + ' ' + o[0] + '→' + s.cd);
      const h = s.hits === undefined ? '' : s.hits;
      if (String(h) !== String(o[1])) badHit.push(s.id + ' ' + o[1] + '→' + h);
    });
  ...
  ok(D.badHit.length === 0, 'D2 27종 발수 선언(hits)이 484 전 값 그대로 — 260 의 «선언 정정» 두 건 포함',
     D.badHit.join(' / ') || '위반 0');
  ...
  const R = await page.evaluate(old => {
    const H = s => s.id === 'shuri' ? 8 : s.id === 'bolt' ? 3 : s.id === 'multi' ? 3 : (s.hits || 1);
    const D = (s, m) => s.cd > 0 ? m * H(s) / s.cd : m * 3;
