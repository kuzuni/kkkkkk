...
/* ⚑ 644(2026-09-01) — 36.09 → **38.50**. 이 기준선은 «543 이전 트리에서 비행 중 다이아 잉크가
   몇 px 이었나» 인데, 그 값은 상자(55 × FX3_FLYS 이전 배율)에 **`cur-dia.svg` 의 채움비**를 곱한 것이다.
   644 가 그 아트의 viewBox 를 잉크 bbox 로 잘라 채움비 .9375 → **1.0** 이 되었으므로
   같은 «543 이전» 세계의 값도 36.09 / .9375 = **38.50** 으로 다시 잰다.
   ⇒ 이 자가 묻는 «×3 인가» 는 한 칸도 안 무뎌진다 — §R 되돌림(FX_GRAIN_SC=1)이 실제로 38.50 을
   찍는 것이 그 증거이고(아래 [R] 항), 그 항과 [C] 가 **같은 상수**를 쓰므로 값을 임의로 적을 수 없다. */
const BASE_DIA_INK = 38.50;      /* 비행 중 다이아 잉크 지름 (644 이관 — 36.09 × 64/60) */
