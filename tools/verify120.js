...
/* 813 1회차 — 하한 44 → **50**. 14회차의 44 는 «안내문 위 41 과의 역전만 없애는» 최솟값이라
   여백으로서는 하한에 눌린 채였고, 754 6회차의 자가 «안내문 하변 ↔ 하단 코너 브래킷» 을
   기준 간극의 **22%** 로 [❌] 냈다(코너 브래킷이 27.2 를 먹는다). 50 이면 29.9% 로 판정선을 넘고
   14회차의 근거(역전 금지)는 50 > 41 로 더 넉넉해진다. **clamp 하한이라 1600 한 프레임만 본다.** */
const G3_MIN = 50, G3_MAX = 104;   /* 14회차 44 → 813 50 · 안내문 위 41px 고정과의 역전 해소 */
