/* 작업 120 게이트 — 89 유물 페이지(#relw)가 «재화 바 밑 ~ 탭바 위» 영역을 **꽉 채우는가**.
   ...
   지시(PROGRESS 120 검증절)를 그대로 항목화한 것:
     ...
     ⑦ 89 측정 규격 불변 — 슬롯 151² · 수반 400×**226** · 코스트 **260×53.3** · 가로 중심 540.
     (904 이관 — 866 의 57.8 은 자가 만든 값이었다: `probe866.py` 의 걸음이 알약 하변 아래
      한 행짜리 밝은 틈을 건너뛰어 ref 세로를 24 → 26 으로 읽었다. 이 항은 «규격이 안
      움직였나» 를 묻는 자리라 새 규격으로 따라간다 — 근거는 `verify904`·`probe904`.)
...
/* ⚑ 813 5회차 이관 — **아래 블록이 «프레임의 함수» 에서 «레퍼런스 상수» 로 내려왔다.**
   옛 값은 `GAP2_PX 38 + g3 = clamp(44, spare×.1325 − 38, 104)`(총량 82~104)이고,
   그 clamp 은 12회차가 «하단 여백이 26 → 159px 로 6.1배 표류» 를 막으려고 넣은 것이다.
   859 가 그릇을 1527 로 캡해 표류의 원인(패널 길이)이 사라졌고, 813 5회차가 채점 2인(CN·CO)의
   처방대로 총량을 **36 상수**로 내렸다(제품 `--rw-g3` 주석). 옛 상수는 계보로 남긴다. */
const GAP2_PX = 12;               /* 수반↔안내문 — 5회차: 38 → 12(총량 36 = 12 + g3 24) */
const GAP2_PX_R4 = 38;            /* 4회차까지의 값 — [R] 사본·계보용 */
...
/* 813 3회차 — 18회차의 `AB_RATIO`(아치 위:아래 1:3.797)를 **격자 위:아래 47:53** 으로 갈았다.
   둘 다 같은 레퍼런스(486×687 크롭 · 패널 환산 1080×1527)에서 잰 값이고 분해가 다르다:
     18회차  아치 위 133 : 아치 아래 505        → 아래에 «자라지 않는 내용» 이 섞여 있다
     3회차   패널상변→격자상변 308.9~312 : 격자하변→수반상변 346.7~349 = **47 : 53**
   후자는 813 2회차의 비평가 CH·CI 가 **각자 독립으로** 낸 값이다(두 값의 차 1%). */
/* 813 4회차 [G1] — 47 → **48**. 3회차 A/B 판정의 CH 처방(«다음 비 48:52», 5장 전부)이다.
   CI 의 55:45 는 2600 스트립 411px 이라 `scan813b.py` 축에서 14회차 반려선 너머라 안 받았다. */
const GT_SHARE = 0.48;
...
const STRIP_SHARE = 0.206;        /* 813 3회차 — 옛 `LINTEL_STRIP_MAX = 190`(px). 아래 주석 */
const GT_FLOOR = 110;             /* 813 2회차 [E2] 신설 232 → **8회차 [H1] 110** (아래 주석)
   ⚑ 232 안의 146(= 바 98 + 위 24 + 아래 24)은 **867 이 바를 벽에서 격자 아래로 데려간 뒤
     주인 없는 예약**이었다. 벽에 남는 것은 20(금테) + 66(상인방) + 24(들보↔아치 정점)뿐이다.
     이 자는 제품 식의 **거울**이므로 값이 어긋나면 [1600] ⑥ 이 곧바로 빨개진다. */
/* 12회차 — 하단 여백을 «패널 바닥에서» 역산하고 clamp 로 묶었다(비평 AB ⑥·AC ⑥: 26 → 159px, 6.1배).
   1920·2280 은 clamp 안쪽이라 불변이고, 1600 만 하한 34 · 2600 만 상한 120 에 걸린다. */
const G3_CONST = 24;               /* 813 5회차 — 상수. 아래 둘은 4회차까지의 clamp(계보) */
const G3_FLOOR_R10 = 17;           /* 813 10회차 — 짧은 프레임의 하한(120 «안내문 ↓ 그릇 하변 ≥ 12» 역산) */
...
(async () => {
  ...
  try {
    for (const H of HEIGHTS) {
      ...
      /* [859-b] **하단 앵커** — 2회차에 «세로 중앙» 에서 바꿨다(제품 주석 `--rw-oy`):
         중앙에 두면 수반이 위로 올라가 420 이 닫아 둔 «33 재화 팝업 ↔ 유물 소환 버튼» 겹침이
         다시 열리고(2280 −40.2px), 그것을 팝업 쪽에서 풀면 63·407 이 못박은 «9:19 에서 한 칸도
         안 움직인다» 가 깨진다. 하변을 패널 바닥에 붙이면 그 축이 859 전 그대로 남는다.
         레퍼런스 장면이 «위가 벽·아래가 바닥» 이라 남는 세로가 위로 가는 것이 물리적으로도 맞다. */
      ck(`[${H}] ② 그릇이 패널 **하단 앵커** (아래 띠 0 · 남는 세로는 전부 위)`,
        Math.abs(P.b - W.b) < 0.6 && (W.t - P.t) >= -0.6 &&
        Math.abs((W.t - P.t) - (P.h - W.h)) < 0.6,
      ...
      /* 813 5회차 — clamp 이 아니라 상수 24 다(제품과 같은 값 · 위 GAP2_PX 주석).
         ── ⚑ 813 10회차 이관 — **다시 창이 됐다. 단 «위» 가 아니라 «아래» 로만 열린다.** ──
         5회차의 상수 24 는 «푸터 여백은 프레임이 길어져도 안 자란다» 를 지킨 것이고, 그 문장은
         **자라는 쪽**만 말한다. 10회차 채점 2인(EG·EH)이 각자 «1600 에서 유동 풀만 −37.8% 로
         눌리는데 이 쌍만 안 줄어 아치가 굶는다» 를 재서, 제품이 상한 24 는 그대로 두고
         **하한 17 까지 압축**되는 어파인 clamp 로 바뀌었다. 이 줄은 제품 식의 **거울**이다.
         되돌림: `const g3 = G3_CONST;` 이면 9회차와 항등이다. */
      const g3 = Math.min(G3_CONST, Math.max(G3_FLOOR_R10, (W.h - 830) * 0.033 + 1.9));
      const bt = W.h - 88 - g3 - GAP2_PX - 226;    /* 수반 구획 상변(그릇 기준 — 859 · 866 이 216 → 226) */
      const T = bt - 516;
      /* 16회차 — «아래 최소» 137 → 80 (받침 40 + 접합선 띠 13 + 바닥 27). 계단 최소 1단 84 를 뺐다.
         따라서 av 상한식의 231(=94+137) → 174(=94+80) → **182(=94+88, 879 7회차)**. 1600 만 이 상한에 걸리므로 1600 만 움직인다.
         ⚠ 아래 gt 의 셋째 인자는 **137 그대로다** — 그쪽은 «아래에 이만큼 예약하라» 는 하한이라
            낮추면 gt 가 위로 올라갈 여지를 얻어 바닥을 되레 빼앗는다(1920 이 1단 → 0단이 됐다). */
      /* ⚑ 879 7회차 — **174 → 182**(아래 예약 80 → 88 · 바닥 27 → 35). 6회차 채점 2인의
         1순위(«바 하변 ↔ 수반 상변» 띠만 잔존율 0.553)를 av 에서 갚은 자리다. 재원의 상한은
         «배수 바가 안 줄어드는 av 하한 134.46» 이 정한다(`verify879` [1]·[2d]). 1600 만 이 인자에
         걸리므로 **1600 만 움직인다** — 긴 프레임은 186 이 이긴다(아래 ⑥ 이 그것을 잰다). */
      const av = Math.min(186, (T - 182) / 2);
      ...
      /* ── 813 3회차 [F1] 이관 — **«아치 위:아래 1:3.797» → «격자 위:아래 47:53».** ──
         둘 다 같은 레퍼런스에서 잰 값이고, 바뀌는 것은 «무엇을 무엇과 나누는가» 다.
         18회차의 분해는 «아치 아래 B» 안에 자라지 않는 내용(수반 216 · 안내문 88 · 두 여백)을
         품고 있어, B 가 비례로 커지라는 규칙은 **격자↔수반 한 칸**만 키운다. 비평가가
         아홉 명째 짚은 결함이 그것이고(120 18회차 다섯 · 754 6회차 둘 · 813 1회차 둘),
         813 2회차의 CH·CI 가 각자 독립으로 «패널 +999 일 때 그 칸이 +689(68.9%)» 라는
         **같은 숫자**와 «위 : 가운데 = 47 : 53» 이라는 **같은 처방**을 냈다(각자 ref 에서 뽑았고
         두 값의 차 1%). 3회차가 그 처방을 넣고 **같은 두 사람에게 A/B 를 물어 승인받았다** —
         ③ CH 3 → 5 · CI 2 → 4 (①·④ 도 각각 +1). */
      const gtRule = T * GT_SHARE;
      /* 813 2회차 [E2] 이관 — **`GT_FLOOR` 신설.** 754 6회차의 [❌] `.rw-lintel ↓ #rwMulBar`
         ...
         ⚑ **813 8회차 [H1] — 그 예약의 절반 이상이 867 이후 유령이다.** 867 이 바를 벽에서
           격자 아래(아치 니치)로 데려갔으므로 «24 + 98 + 24 = 146» 은 벽에 없는 몫이다.
           ⇒ 110 = 20 + 66 + 24(들보↔아치 정점). 1600 의 gt 는 이제 `av + 94` 가 정하고
           아치 안쪽이 +2.0px(210.8 → 212.8) · 긴 프레임은 Δ0px.
           ⚠ 754 6회차의 [❌](벽이 아치를 따라간다)가 되살아나지 않는 이유는 **바가 그 벽에
             없기 때문**이다 — 바의 여유는 이제 `probe813` [C] 의 «격자 하변 ↔ 바 상변» 이 잰다. */
      ...
      /* 813 5회차 — 하한 32(코너 브래킷) → 12(금테 안쪽 테두리 7 + 5). 브래킷과는 안내문
         **상자**까지 가로 겹침이 0 이 됐다(`.rw-cap{left:40px;right:40px}` · verify813 [2d]). */
      /* 813 6회차 — .375 → .468. 2회차가 쓴 «ref 아래/위 0.58~0.62»(1회차 CF·CG)를 5회차
         채점(CP 0.75 · CQ 0.82)이 뒤집었고, 세 번째 자 `tools/scan813c.py` 가 화소로 갈랐다:
      ...
      /* ⚑⚑ 887 이관 — .5 → **.551**. 7회차의 «1.00» 은 **자가 만든 값**이었다:
         레퍼런스 쪽은 아래 끝점을 «금테 띠 «안»» 으로 잡았고(띠가 ref 2px ↔ 우리 캡처 5px 이라
         같은 규약이 두 쪽에서 서로 다른 양을 훔친다), 우리 쪽 [3] 은 Range 줄 상자로 쟀다
         (위 +3 · 아래 −3 이 **비만 상쇄**해 1.00 으로 읽혔다). 테두리 조립체 최상단으로
         두 쪽을 맞추면 **ref 0.90 ↔ 우리(그때) 0.76** 이다 ⇒ 잉크 총량 44 를 10:9 로 나눠
         상자 축 아래 = 36 × .551. 이 거울은 `tools/verify887.js` [5] 가 화소로 지킨다.
         되돌림: .551 → .5(7회차) · .468(6회차) · .375(5회차). */
      /* ⚑ 813 10회차 이관 — 제품이 «비례 .551» 에서 **어파인**으로 바뀌었다(거울 오프셋이
         절대량이라, 총량이 프레임마다 다른 순간 비례 계수는 화소 비를 흘린다 — 1600 0.810).
         이 줄은 제품 식의 **거울**이므로 같은 식으로 따라간다.
         되돌림: `(GAP2_PX + g3) * 0.551` 이면 9회차와 항등이다. */
      /* ⚑⚑ 905 이관 — 위 끝점의 자가 두 그림에서 밑판 외곽선의 **반대편**을 짚고 있었다
         (우리 −2px · ref +2px = 같은 물체가 아니다). 위도 아래와 같은 걷개(«마지막 칠해진 행»)로
         재면 ref 는 위 12 : 아래 9 = **0.750** 이고, 목표비만 .9 → .75 로 바꾸면 계수가
         `.4286·T + 1.21` 이다. 이 줄은 제품 식의 **거울**이라 같은 값으로 따라간다.
         되돌림: `* 0.4737 + 2.57` 이면 10회차와 항등. 상세 `docs/review/905-안내문위끝점자.md`. */
      const iGap = Math.max(12, (GAP2_PX + g3) * 0.4286 + 1.21);
      ...
      /* [L1] 규칙이 «실제로 상수인가» 를 **렌더 결과에서** 되잰다 — 위 항목은 식을 다시 쓴 것이라
         식과 코드가 함께 틀리면 둘 다 통과한다. 3회차부터 재는 비는 «아치 위:아래» 가 아니라
         **«격자 위(패널상변→격자상변) : 격자 아래(격자하변→수반상변)»** 다 — 비평가 2인이
         각자 레퍼런스에서 그 두 구간을 재서 47:53 을 냈고, A/B 대조에서 그 비가 서 있는지를
      ...
      /* ── 813 3회차 이관 — **px 상한 190 → «패널의 20.6%» (레퍼런스 자신의 몫).** ──
         14회차가 이 띠에 상한을 건 근거는 «부재 0개 · 평균 휘도 29.6 · 66.6%가 32 미만 =
         육안으로 검은 띠» 였다. 그 세 근거 중 **첫째가 18회차에 사라졌고**(벽 결 투입),
         813 3회차에 비평가 2인이 **각자 그 띠를 다시 재** 판정을 뒤집었다:
           CH — 가로 줄눈이 **피치 47px 등간격**으로 여섯 줄 · 최장 무변화 연속 구간 **41px**
                (레퍼런스는 같은 자리에서 **133px = 밴드의 99.3%** 를 연속으로 아무것도 없이 간다) ·
                양자화 색 52 vs 레퍼런스 26 · 디테일 행 19% vs 레퍼런스 0%
           CI — 열평균 표준편차 8.59(ref 자기 여백 6.38) · 8단계 색 55종(ref 42) ·
                가로 레일 밀도 1.4개/100px 로 **띠를 2.09배 늘려도 안 묽어진다**
         둘 다 «죽은 검은 여백이 아니다 · 대가를 치를 값어치가 있다» 로 판정했고, A/B 점수가
         ③ CH 3 → 5 · CI 2 → 4 로 올라갔다.
         ⇒ 상한을 **지우지 않고**, 14회차가 지키려던 것(«이 띠가 화면을 과하게 먹지 않는다»)을
           px 가 아니라 **레퍼런스 자신의 지분**으로 다시 적는다 — 레퍼런스는 같은 자리에
           패널의 **20.6%** 를 쓴다(그것도 완전히 죽은 채로). 우리 최댓값은 2600 의 14.7% 다.
           비가 되면 프레임이 길어져도 자가 안 늙는다(옛 190px 은 2280 에서 9.7%, 2600 에서
           8.3% 라 **같은 규칙이 프레임마다 다른 엄격도**로 작동하고 있었다).
         ⚠ 이 항이 빨개지면 «올려라» 가 아니라 **띠를 다시 재라**(`tools/scan813b.py`) — 근거가
           «높이» 가 아니라 «결이 있는가» 이기 때문이다. */
      /* 859 — 이 «띠» 는 장면 안의 벽면이므로 기준은 패널이 아니라 **그릇**이다.
         패널로 재면 그릇 밖의 «이어받는 벽» 까지 띠로 세어(2280 485px) 규칙이 뜻을 잃는다. */
      ck(`[${H}] ② 상인방 위 스트립 ≤ 그릇의 ${(STRIP_SHARE * 100).toFixed(1)}% (레퍼런스 자신의 몫)`,
        r.lintel.t - W.t <= W.h * STRIP_SHARE + 0.6,
        `${(r.lintel.t - W.t).toFixed(1)}px (상인방 상변 ${r.lintel.t.toFixed(1)} · 그릇 상변 ${W.t.toFixed(1)})`);
      ...
      ck(`[${H}] ⑦ 수반 400×226 · 코스트 260×53.3 · 가로 중심 540 (904 이관 — 866 은 57.8)`,
        Math.abs(r.basin.w - 400) < 0.6 && Math.abs(r.basin.h - 226) < 0.6 &&
        Math.abs(r.cost.w - 260) < 0.6 && Math.abs(r.cost.h - 53.3) < 0.6 &&
      ...
      /* ── 893 이관(2026-09-04) — **위 항목은 «억제» 를 이유도 안 묻고 통과시킨다.** ──
         `stSup` 분기는 구간이 0 이기만 하면 «하변 = 수반» 하나만 보므로, 계단이 **다섯 프레임
         전부에서 사라져도** 이 자는 초록이다(`verify893` [R1] 이 그 사본을 실제로 굴려 찍는다).
         893 이 «13회차가 못박은 계단 1단이 조용히 사라졌다» 로 등재된 뿌리가 여기다 —
         제품은 16·18회차 결정(비평 AJ·AK 처방)을 지키고 있었고, 사라진 것은 **그 결정을 지키는 자**다.
         333 처방대로 자리를 비우지 않고 **묻는 항을 한 줄 더** 세운다: 억제가 정당한가,
         즉 «재원(접합선+14 ↔ 수반)이 한 단에 못 미쳐서» 인가.
         재원이 있는데 0단이면 그것은 예산이 아니라 **누가 계단을 죽인 것**이다. */
      const stRoom = r.mid.t - (r.ground.t + 14);
      ck(`[${H}] ③ 계단 억제는 «재원이 한 단에 못 미칠 때만» 정당 (893 — 위 항목이 이유를 안 묻는다)`,
        stSup ? stRoom < STEP_PITCH : stRoom >= STEP_PITCH - 0.001,
        `재원 ${stRoom.toFixed(1)} ${stRoom < STEP_PITCH ? '<' : '≥'} 피치 ${STEP_PITCH} · 계단 ${stSup ? '억제(0단)' : `${vis.length}단`}`);
      if (vis.length) {
