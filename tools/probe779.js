...
(async () => {
  ...
  /* ⚠ **여기에 절대 문턱을 새로 적지 않는다**(«여유 < 500ms» 라고 적었다가 기계가 느린 날
     그것이 새 플레이키가 된다 — 775-④ «플레이키를 고치러 가서 플레이키를 새로 심지 마라»).
     물어야 할 것은 관계다: **창까지의 여유가 실행 간 흔들림 자체보다 작다**면, 그 흔들림 하나로
     킬이 창 안에 들어온다 = 동전이다. 둘 다 같은 실행 묶음에서 잰 값이라 기계 속도에 안 끌린다. */
  const mgFk = Math.min(...fkOk) - 500, jitter = Math.max(...fkOk) - Math.min(...fkOk);
  ok(mgFk < jitter,
     '2 창까지의 **여유가 실행 간 흔들림보다 작다** — probe766 이 붙는 500ms 는 분포의 어깨 위다(동전이 되는 이유)',
     '최소 첫 킬 ' + Math.min(...fkOk) + 'ms − 창 500ms = 여유 ' + mgFk + 'ms < 흔들림 ' + jitter + 'ms');
