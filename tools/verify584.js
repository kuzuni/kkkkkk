...
(async () => {
  ...
  ok(/const TP_CUR_PX = 72;/.test(CODE) && cur.temp && near(cur.temp.w, 72, 3),
    '[2-e] ★ 686 — 단련 화폐는 전용 상수 `TP_CUR_PX`(72)를 읽는다(룬은 기준선 53 그대로 = [2-d])',
    cur.temp && `${cur.temp.w} ↔ TP_CUR_PX 72 (기준선 ${cur.base.w} 대비 ${p1(cur.temp.w / cur.base.w * 100)}%)`);
