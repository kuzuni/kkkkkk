...
(async () => {
  ...
  /* ⚑ 부호를 단언하지 않는다 — 그것이 [C1] 이 한 짓이다(721 «잡음 폭 안에서 부호를 물었다»).
     ...
     어떤 바를 놓아도 동전인 자리이므로, 775 §7 대로 **재현되는 것만 남긴다**:
       · **[5-a]** — 재현되는 것은 «멀다» 가 아니라 **«두 이탈의 R회 분포가 겹친다»** 다
         (실측 6/6 겹침). 그것이 곧 «누가 더 먼가» 가 동전이라는 [C1] 진단이고,
         분포 하나가 튀어도 안 뒤집힌다(775 §4 «분포가 겹치면 최솟값의 이름이 갈린다» 의 정면).
  ...
  ok(sLo <= iHi && iLo <= sHi,
     '5-a K회 평균을 내도 ⓐ·ⓑ 두 이탈의 R회 분포가 **겹친다** — 그래서 «누가 더 먼가»(=[C1])는 그 실행이 뜬 장면이 정하는 동전이다',
     'mStd ' + pc(sLo) + '~' + pc(sHi) + ' · mImm ' + pc(iLo) + '~' + pc(iHi)
     + ' ⇒ 겹치는 구간 ' + pc(ovLo) + '~' + pc(ovHi) + ' (K평균 점추정 mStd ' + pc(mStdAvg) + ' · mImm ' + pc(mImmAvg) + ')');
