...
(async () => {
  ...
  /* [6-cr] — **되돌림 시험(새 상수·새 실행 0).** «좁다» 가 POP 고정 덕인지 **표본 수 덕**인지
     ...
     이 항이 잰 것은 표본 수가 아니라 POP 고정이다.
     ⚠ **1회차에 여기에 새 동전을 한 번 심었고 자가 3회차에 잡았다**(778 §4 가 미리 경고한 자리 —
     «플레이키를 고치러 가서 플레이키를 새로 심지 않았는지 새 항도 K회 돌려라»). 처음에는
     **자유 판을 3회로 깎아** 견줬는데 그 폭이 44% → 21% 로 주저앉아 채택 눈금 24% 아래로 내려가
     **5회 중 1회 빨개졌다**. 한쪽만 깎는 보정은 보정이 아니라 **새 편향**이다 — 깎는 쪽을
     «표본을 더 가진 쪽»(채택 눈금 K회 → R회)으로 뒤집어 **양쪽을 같은 표본 수로** 맞췄다.
