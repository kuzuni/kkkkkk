...
(async () => {
  ...
  if (D.__err) ok(false, 'evaluate 실패: ' + D.__err);
    ...
    ok(D.tn.x2 <= D.tc.x, '[D3] 이름 잉크가 비용 상자를 안 밟는다', D.tn.x2 + ' ≤ ' + D.tc.x);
    ok(D.tb.x === 632 && D.tb.y === 128 && D.tb.w === 340 && D.tb.h === 74,
       '[D4] [단련] 버튼 584 값 Δ0 (632,128 340×74)', D.tb.x + ',' + D.tb.y + ' ' + D.tb.w + '×' + D.tb.h);
    ok(D.tc.x === 552 && D.tc.y === 14, '[D5] 비용 줄 Δ0 (552,14)', D.tc.x + ',' + D.tc.y);
