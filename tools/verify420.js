 ...
 *   §N 음성   — 버튼을 위로 올리면 §2 하한과 §1 겹침·덮임 축이 **실제로 빨개지고**, 그 «직전» 세기에서는
 *               **아직 초록이다**(888 완화의 짝 · 세기는 924 부터 실측 여백에서 판다 — 상수 40/80 폐지)
 *
 ...
 *
 * ⚑ **924(2026-09-05) — 888 이 신설한 §N 자신이 같은 부패를 앓았다(제품 0줄 · 115/116).**
 *   음성 시험의 **«세기»** 40·80 도 버튼의 그때 자리에서 나온 상수였다 — 여백이 30 → **72.2** 로
 *   벌어지자 ↑40 은 32.2 에서 멈춰 하한을 **안 문다**. ⇒ 세기를 실측 여백에서 파는 항등식으로 바꾸고
 *   («하한을 파고드는 힘 = 여백 − 30» · «겹침을 깨는 힘 = 여백»), **문턱의 양쪽을 다 묻는다**
 *   (직전 세기에서는 아직 초록 — 세기를 키운 것만으로 초록을 살 수 없게).
 */
const fs = require('fs');
...
(async () => {
  ...
  try {
    ...
    /* ⚑ 924(2026-09-05) — **세기 40·80 이 또 「남의 자리에서 온 상수」였다**(888 ① 의 한 겹 위).
       420 이 선 세계는 「여백 30」(버튼 상변 = H − 566)이었고, 그 세계에서 ↑40 은 하한을 10px
       파고들었다. 그 뒤 버튼이 두 번 내려가(888 이 잰 F 566 → 530 · 짧은 프레임은 더 아래) 1600 의
       여백이 **72.2** 가 되자 ↑40 은 **32.2 에서 멈춘다** — 음성항이 «빨개져야 할 때 빨개지는가» 를
       묻는 대신 **아무것도 안 무는 시험**이 됐다(115/116 · 제품 0줄 · 사용자가 보는 결함 없음).
       ⚠ 문턱을 30 → 32 로 올려 초록을 사는 길은 **금지다** — 음성항이 지키려던 것을 없애는 길이다.
       ⇒ **세기를 실측 여백에서 판다.** 하한을 파고드는 데 필요한 힘은 항등식 «지금 여백 − 30» 이고,
         겹침을 깨는 힘은 «지금 여백» 이다. 둘 다 세로 스택이 어디로 걷든 자가 스스로 다시 잰다.
       ⚠ 세기를 키운 것만으로 초록을 살 수 없게 **양쪽을 다 묻는다** — 파고들기/겹치기 «직전» 세기에서는
         §2·§1 이 **아직 초록**이어야 한다. 그것이 「문턱이 30 에, 겹침이 여백 0 에 있다」 는 증거다
         (한쪽만 물으면 «충분히 크게 올렸다» 가 시험을 대신한다). */
    console.log('\n§N 음성 — 버튼을 위로 올리면 §2 하한과 §1 겹침 축이 실제로 빨개진다');
    const g0 = M[1600].gap;                    /* 실측 여백 — 세기의 재료다(상수가 아니다) */
    const need = +(g0 - GAP).toFixed(1);       /* §2 하한을 파고드는 데 필요한 힘 */
    const MG = 8;                              /* 문턱 양쪽으로 확실히 넘기는 여유(부분화소·정착 흔들림) */
    const L = { bite: Math.max(1, Math.ceil(need) + MG), keep: Math.max(1, Math.floor(need) - MG),
                over: Math.max(1, Math.ceil(g0) + MG),   nearg: Math.max(1, Math.floor(g0) - MG) };
    ok(need > MG, `[음성] 전제 — 지금 여백 ${g0} 이 하한 ${GAP} 보다 ${need}px 크다 (§2 가 초록이라야 음성 시험이 뜻을 가진다)`);
    console.log(`  · 세기 = 실측 여백 ${g0} 에서 판 값 : 하한 ↑${L.bite}(직전 ↑${L.keep}) · 겹침 ↑${L.over}(직전 ↑${L.nearg})`);
    const upBite = await read(browser, 1600, { raise: L.bite });
    const upKeep = await read(browser, 1600, { raise: L.keep });
    const upOver = await read(browser, 1600, { raise: L.over });
    const upNear = await read(browser, 1600, { raise: L.nearg });
    ok([upBite, upKeep, upOver, upNear].every((m) => m.opened.ok), '[음성] 버튼을 올린 사본도 같은 화면이 열린다');
    near(`[음성 1600 ↑${L.bite}] 여백이 올린 만큼 그대로 줄어든다`, g0 - upBite.gap, L.bite, 0.6);
    ok(upBite.gap < GAP, `[음성 1600 ↑${L.bite}] 여백이 ${GAP} 미만으로 떨어진다 — §2 하한이 여전히 문다 (실제 ${upBite.gap})`);
    ok(upKeep.gap >= GAP, `[음성 1600 ↑${L.keep}] 파고들기 직전 세기에서는 §2 가 아직 초록이다 — 문턱은 ${GAP} 에 있지 그 위가 아니다 (실제 ${upKeep.gap})`);
    ok(upOver.ovY > 0, `[음성 1600 ↑${L.over}] §1 「겹침 없음」 이 실제로 깨진다 (세로 겹침 ${upOver.ovY})`);
    ok(upOver.worst > 0, `[음성 1600 ↑${L.over}] §1 「부품 덮임 0%」 도 같이 깨진다 (최댓값 ${upOver.worst}%)`);
    ok(upNear.ovY <= 0 && upNear.worst === 0,
      `[음성 1600 ↑${L.nearg}] 겹치기 직전 세기에서는 §1 이 아직 초록이다 — 겹침의 문턱은 «여백 0» 이다 (겹침 ${upNear.ovY} · 덮임 ${upNear.worst}%)`);
