...
try {
  ...
  ok('[A-b] §2 요약이 축 ⓒ 의 «판정 불가» 를 세어 찍는다(못 본 것을 초록으로 안 부른다)',
     /축 ⓒ 판정 불가 \d+건/.test(a.out));
  ...
  /* ── [E] 음성 — 칸 수가 7 이 아니면 «빨강» 이 아니라 «판정 불가» 다 ── */
  console.log('\n[E] 음성 — 칸 수가 7 이 아닌 행은 판정 불가로 센다(위치를 못 믿는다)');
  const un0 = Number(/축 ⓒ 판정 불가 (\d+)건/.exec(a.out)[1]);
  ...
  ok('[E-a] 같은 산문 구현 칸이라도 칸이 8개면 초록이다', e.code === 0, 'code ' + e.code);
  const un1 = Number(/축 ⓒ 판정 불가 (\d+)건/.exec(e.out)[1]);
