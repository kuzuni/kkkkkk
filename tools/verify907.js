 ...
 *
 * 무엇을 묻나
 *   [1] 손잡이 — `tools/pwlaunch.js` 의 `resolveArgs()` 가 **조건을 갖춘 게이트**에만 깃발을 기본으로 준다
 *       (`raster907.qualifies()` 가 entry 파일 자신을 읽어 ①∧② 를 본다). 캡처 하네스(`cap*.js`)는
 *       조건을 갖춰도 안 준다(291 선례 — 연출 캡처는 일부러 한복판을 찍는다).
 ...
 *   [3] 전수·범위 — 조건을 갖춘 게이트가 **전부** 깃발 아래고([3-b]), 재현기는 **일부러 밖**이며([3-c]),
 *       **조건 밖 게이트 400여 개는 한 칸도 안 바뀐다**([3-d]). 새 게이트가 조건을 갖추면 자동으로 켜지므로
 *       «깃발을 적는 것을 잊는» 자리가 없다. 게이트가 아닌 자가 결정성을 원하면 `launch(chromium, det(opts))`.
 *
...
(async () => {
  ...
  ok(R({}, 'verify432.js', {}).includes(F),
     '[1-a] 조건을 갖춘 게이트(`verify432`)는 기본으로 깃발이 켜진다', F);
  ok(!R({}, 'cap12.js', {}).includes(F),
     '[1-b] 캡처 하네스(`cap*.js`)는 기본이 꺼짐 — 291 선례(연출은 한복판을 찍는다)');
  ok(!R({}, 'verify655.js', {}).includes(F),
     '[1-g] 조건 밖 게이트(`verify655` — 태그 교체 없음)는 깃발이 안 켜진다(라스터 경로 불변)');
