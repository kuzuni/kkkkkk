  ...
  try {
    ...
    await blk('§R', async () => {
      ...
      /* R2 — §3 의 역전은 **501 과 502 가 같이** 만든다. 그래서 되돌림도 둘을 같이 되돌린 사본으로 잰다.
         ...
         ⚑ 580 이관(2026-08-31) — 옛 식의 **리터럴 110·0.6 은 «수리 전 세계» 의 절대값**이라,
            580 이 이 세계의 이속 눈금을 통째로 `SPD_SC` 배로 옮기자 그 사본만 옛 눈금에 남아
            잡몹이 플레이어(×2)에게 그냥 따돌려졌다(실측 보스 0.4/s ↔ 잡몹 0.2/s = 역전이 안 뒤집힘).
            되돌려야 하는 것은 **눈금이 아니라 규칙**(«스테이지가 오르면 빨라진다» + «천장이 없다»)이므로
            같은 눈금 위에서 되돌린다 — 옛 식에 `SPD_SC` 를 곱한다. 이러면 이 절은 580 이 어떤 배수를
            쓰든 «501·502 이전이면 역전이 뒤집힌다» 만 묻는다(368 처방 — 상수를 게이트에 박지 않는다). */
      const both = RAW
        ...
        .replace(/sp: Math\.min\(MOB_SPD_BASE \* T2\.sp \* rnd\(0\.9, 1\.1\), PLAYER_SPEED \* MOB_SPD_CAP\),/,
          'sp: (110 + 3*s*0.2) * SPD_SC * T2.sp * rnd(0.9, 1.1),')
