...
(async () => {
  ...
  if (A.err || B.err) { ok(false, 'V0 표본을 못 얻었다', (A.err || '') + ' ' + (B.err || '')); }
    ...
    ok(A.body >= 1.35, 'A1 씬 A 사거리 ≥ 1.35 몸길이 — ' + p2(A.body),
       '수리 전 0.41 · 1회차 0.83 · 2회차 1.36~1.82 · 등재문 0.59');
    /* 비평 2인(CX·CY)이 **중앙값**으로 적었다 — 평균만 지키면 «긴 알 둘이 표를 만든다» */
    ok(A.bodyMed >= 1.20, 'A2 중앙값도 ≥ 1.20 몸길이 — ' + p2(A.bodyMed),
       '1회차 채점 CX 0.77 · CY 0.61 · 2회차 채점 CZ 2.18 · DA 2.54');
    ok(A.net >= 33, 'A3 총 이동 평균 ≥ 33px — ' + p2(A.net) + 'px',
       '수리 전 18.5px · CV 24.0 · CW 21.4 — 3회차는 «벽까지» 대신 «방의 82~100%» 라 이동이 조금 짧다(대가는 각도)');
    ...
    ok(A.fanGap <= 140, 'C3 끝점이 온 원에 퍼진다 — 가장 큰 빈 각 ≤ 140° · ' + p2(A.fanGap) + '°',
       '2회차 채점 CZ «40.8° 부채»(= 빈 각 319°) · DA «65° 쐐기»(= 빈 각 295°)');
