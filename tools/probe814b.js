...
(async () => {
  ...
  /* ⚠ 문턱을 «든 프레임 수» 에서 «표본 간격» 으로 갈아 끼웠다(12회차).
     `bIn <= 2` 는 **런너 속도 상수**였다 — 같은 트리에서 4연속 실행이 2·2·2·3 으로 갈렸고
     3 이 나온 실행만 빨개졌다(플레이키 계보 574·709·825·854·873·876). 그런데 이 항이 실제로
     주장하는 것은 «몇 장이 들었나» 가 아니라 **«표본이 램프를 해상할 수 없다»** 다. 그 성질은
     간격이 정한다: 간격이 수명의 1/4 이상이면 620ms 안에 4장을 못 넘고 **어디에 떨어지는지가 운**이다.
     ⇒ 간격을 주 항으로 세우고 장수는 그 산수의 짝(≤ 620/155 = 4)으로 남긴다.
     이 항은 무르게 푼 것이 아니다 — live 캡처가 실제로 촘촘해지면(간격 < 155ms) 빨개지고,
     그때는 «live 로는 판정 못 한다» 는 주장 자체를 다시 봐야 한다. */
  const bMinGap = 620 / 4;
  ok(bGap >= bMinGap && bIn <= 4, `[3-a] live 벌은 판정에 못 쓴다 — 표본 간격 평균 ${bGap.toFixed(0)}ms 가 수명 620ms 의 1/4(${bMinGap.toFixed(0)}ms) 이상이라 램프를 해상할 수 없다 (수명 안에 든 프레임 ${bIn}/8 ≤ 4 — 몇 장이 드는지·어디 떨어지는지가 **운**이다). CR4 «+49.7%» 와 CR6 «0px» 는 같은 제품의 **두 운**이다`);
