...
(async () => {
  ...
  /* ── ⚑ 작업 859 — **표를 다시 갈아 끼웠다(4회차와 같은 이유·같은 규칙).** ──
     ...
     859 의 이동은 **출처가 둘뿐**이다:
       ⓐ **그릇 캡 1527**(`--rw-ph`/`--rw-oy`) — 패널이 1527 보다 긴 세 프레임에서 그릇이
          «위로 oy, 아래로 oy» 만큼 안쪽으로 들어온다 ⇒ 위 앵커 요소(상인방·격자·바닥·접합선)는
          **아래로**, 패널 바닥에서 역산하는 요소(수반·코스트·라벨·안내문)는 **위로** 움직이고,
          아래에서 역산하는 것의 높이는 그만큼 줄어든다. 1600·1841 은 캡이 안 걸려(패널 ≤ 1527)
          **Δ0 이거나 oy 1px 이내**다 — 그것이 «짧은 프레임은 픽셀 하나 안 움직인다» 의 증거다.
       ⓑ **아래 예약 137 → 139**(계단 1단 복귀 — 제품 주석) — 격자·상인방·바닥이 2px 더 내려간다.
  ...
  const ALLOW813 = {
    1600: {},
    1920: { gridt: 2.2, midt: -39.1, basint: -39.1, costt: -39.1, labt: -39.1,
            capt: -47.78, lintelt: 27.89, floort: 2.2, floorh: -55.2,
            groundt: 2.2, groundh: -55.2, stepst: -39.1 },
    2280: { gridt: 25.92, midt: -184.69, basint: -184.69, costt: -184.69, labt: -184.69,
            capt: -214.89, lintelt: 51.61, floort: 25.92, floorh: -258.92,
            groundt: 25.92, groundh: -258.92, stepst: -16.67, stepsh: -167.98 },
    2600: { gridt: 32.31, midt: -344.69, basint: -344.69, costt: -344.69, labt: -344.69,
            capt: -374.89, lintelt: 58, floort: 32.31, floorh: -425.31,
            groundt: 32.31, groundh: -425.31, stepst: -8.69, stepsh: -336 },
  };
  {
    const pre = preTree();
    if (!pre.ok) {
