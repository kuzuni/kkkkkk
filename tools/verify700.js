...
(async () => {
  ...
  /* ── [B] 자리 (네 프레임) ─────────────────────────────────────────────── */
  {
    ...
    /* ── 879 1회차 이관 (333 처방 — 자리를 비우지 않고 **방향을 뒤집는다**) ──────────────
       옛 [B1]·[B2] 는 «높이 98 · 폭 646 · 좌 216» 을 **다섯 프레임 전부**에 물었다. 879 가
       1600 에서만 바를 **등방 축소**(`--rw-mbs`)하면서 그 두 항이 빨개졌는데, 두 항이 지키려던
       약속은 «리터럴» 이 아니라 둘이다:
         ⓐ 96·437 규약 = «셸 안의 비가 두 벌이 되면 안 된다» ⇒ 지켜야 할 것은 **종횡비**이고
           (등방이면 `.stab.on` 정지점 표가 한 벌 그대로다), 축소가 아니라 **비균등**이 금지다(356).
         ⓑ 866 스냅 = «바가 격자 3열 행 모듈에 앉는다» ⇒ 그 모듈이 그대로인 **긴 네 프레임**의
           약속이다. 1600 은 866 이 스냅을 만든 이유(«격자 행과 같은 폭이라 한 줄로 읽힌다»)를
           879 가 정반대로 쓰는 프레임이라 같은 리터럴을 물으면 수리 자신을 금지하게 된다.
       ⇒ [B1] 은 «종횡비 불변 + 긴 프레임 98» · [B2] 는 «긴 프레임 스냅 + 1600 은 축소되 중심 유지»
         로 가른다. **[B1b] 를 신설**해 «비균등 스케일 0» 을 따로 못박는다 —
         이 항이 없으면 높이만 줄이는 사고(공용 부품의 정지점 표가 두 벌이 되는 그 사고)가
         [B1] 의 종횡비 항을 그대로 통과할 수 없다는 것을 아무도 안 묻는다(328-330 교훈). */
    const LONG = rows.filter(r => r.H !== 1600), SHORT = rows.find(r => r.H === 1600);
    const AR = BAR_W / SHELL_H;                       /* 646 : 98 = 6.5918… */
    ok(LONG.every(r => eq(r.bar.h, SHELL_H)),
      '[B1] 셸 높이 98 — 긴 네 프레임(96·437 규약 · 니치가 넉넉해 879 배율이 1 이다)',
      rows.map(r => r.H + ':' + r.bar.h).join(' · '));
    ok(rows.every(r => Math.abs(r.bar.w / r.bar.h - AR) < 0.01),
      '[B1b] 셸 종횡비 646:98 이 다섯 프레임 전부 불변 — 축소는 **등방**이어야 한다(356 · 879)',
      rows.map(r => r.H + ':' + (r.bar.w / r.bar.h).toFixed(3)).join(' · '));
    ok(LONG.every(r => eq(r.bar.w, BAR_W) && eq(r.bar.l, BAR_L)),
      '[B2] 폭 646 · 좌 216 — 866 이 격자 3열 행 모듈(216..862)에 스냅했다(긴 네 프레임)',
      rows.map(r => r.H + ':' + r.bar.w + '@' + r.bar.l).join(' · '));
    /* ⚑ 879 4회차 이관(333 — 자리를 비우지 않고 방향을 조건으로 바꾼다). 옛 항은 «1600 은 **반드시**
       줄어 있다»(폭 < 646)를 단언했다 — 879 1~3회차가 니치를 **격자 상자** 하변에서 재는 바람에
       예약을 18.25px 과하게 잡았고, 그 과잉 축소가 «규약» 으로 굳어 있던 자리다. 꼬리를 바로 재자
       1600 도 니치에 그대로 들어가 폭 646 이 됐다. **이 항이 지키려던 것은 «줄어 있다» 가 아니라
       «줄더라도 좌우 대칭으로 줄어야 한다»** 이므로 축소를 **조건**으로 돌린다
       (879 가 되살아나 다시 줄이면 그때 대칭을 다시 묻는다). */
    ok(SHORT && SHORT.bar.w <= BAR_W + 1 && Math.abs((SHORT.bar.l + SHORT.bar.r) / 2 - 539) < 1.2,
      '[B2b] 1600 의 바는 니치를 **넘지 않고**(폭 ≤ 646) 줄든 안 줄든 **중심 539 는 그대로** — 879 의 원점 50%',
      SHORT ? SHORT.bar.w + '@중심 ' + ((SHORT.bar.l + SHORT.bar.r) / 2).toFixed(1)
        + (SHORT.bar.w < BAR_W - 1 ? ' (축소 중)' : ' (축소 없음 — 879 4회차)') : '없음');
  ...
  /* ⚑ 813 10회차 — 표를 다시 적는다. 기준이 `merge-base origin/main HEAD` 라 앞 회차의
     등재분(8회차 [H1] · 887)은 이제 main 안이라 Δ0 으로 읽힌다 — 남겨 두면 «등재된 이동을
     기다리는데 실제는 0» 으로 이 항이 빨개진다(8회차가 같은 이유로 다시 적었다).
     이번 회차의 **출처는 하나**다 — `--rw-g3` 를 상수 24 에서 «짧은 프레임에서만 압축되는»
     어파인 clamp(하한 17)로 바꿨고, 그 압축분이 `--rw-bt` 를 타고 수반을 **아래로** 민다:
     · 1600 g3 24 → 17.8 ⇒ 수반 구획(mid·basin·cost·lab·steps·floor·ground)이 **+6.19px 아래**로
       (바닥에서 역산하는 floor/ground 는 높이가 그만큼 −6.19), 그 아래 예산이 늘어
       격자도 +3.09 내려온다(clearance av + 94 가 이기는 프레임이다)
     · 안내문(cap)은 **+3.14** — 어파인 분할(.4737·T + 2.57)이 887 의 화소 비 0.90 을
       총량이 줄어든 뒤에도 지킨 몫이다(비례 .551 로 두면 0.810 으로 샌다 — `verify887` [6])
     · 긴 세 프레임은 **cap +0.22 · steps 높이 −0.02** 뿐이다 — g3 는 상한 24 에 걸려 Δ0 이고,
       움직인 것은 어파인이 상자 축에서 만드는 반올림 몫(19.62 vs 19.60)이다.
     옛 표(8회차 [H1]: 1600 gridt −2 · floort/groundt −2 · 높이 +2 · 887 capt −1.84 네 프레임)는
     이 주석에 남긴다 — 어디서 왔는지를 지우지 않는다(333). */
  const CAP_R10 = { capt: 0.22, stepsh: -0.02 };
  const ALLOW813 = {
    /* ⚑ 879 5회차 — `gridt` **3.09 → −15.04**. 격자만 아치 안에서 위로 옮긴 양(`--rw-gof`)이다.
       5회차 채점 2인(ER·ES)이 각자 독립으로 «위 띠는 긴 프레임의 72.8% 를 지켰는데 격자↔바는
       46.0% 만 남았다» 를 1순위로 적어, 그 둘의 잔존율을 같게 맞춘 자리(`verify879` [7]).
       ⚠ **1600 한 프레임만** 움직인다 — 식이 av=186 에서 스스로 0 이 되므로 긴 네 프레임은 Δ0
       (`verify879` [7b]). 값을 «창» 으로 넓히지 않고 실측치 그대로 등재한다(±0.51) —
       gof 가 다른 값으로 드리프트하면 여기가 빨개지는 것이 이 표의 일이다. */
    /* ⚑⚑ 879 7회차 — **표를 다시 적었다**(위 주석의 규약 그대로: 기준이 `merge-base origin/main HEAD`
       라 앞 회차 등재분은 이제 main 안이고 Δ0 으로 읽힌다 — 남겨 두면 «등재된 이동을 기다리는데
       실제는 0» 으로 이 항이 빨개진다). 옛 표(813 10회차 + 879 5회차)는 위 주석에 남는다:
         1600 gridt −15.04 · midt/basint/costt/labt/floort/groundt/stepst +6.19 · capt +3.14 · 긴 프레임 CAP_R10.
       **이번 회차의 출처도 하나다** — `--rw-av` 둘째 인자의 «아래 예약» 174 → 182(80 → 88):
         · av 139.09 → 135.09 ⇒ gt = av + 94 도 4 내려가고 `--rw-fl` = gt + 516 + av 는 **8** 내려간다
           ⇒ 지면선·지면(floor·ground)이 **−8**, 바닥에서 역산하는 높이가 **+8**
         · 격자는 gt −4 에 `--rw-gof` 자동 보정 +1.28(px, 그려진 값)이 얹혀 **−5.28**
         · 수반 구획(mid·basin·cost·lab·steps)·안내문(cap)은 **Δ0** — `--rw-bt` 는 바닥 앵커라
           av 를 안 탄다(그래서 «바↓수반» 이 늘어난 것이다 · `verify879` [2b])
         · **긴 네 프레임은 한 줄도 없다** — 그쪽은 `186px` 인자가 이긴다(`verify879` [3]·[7b]). */
    /* ⚑ 904 이관 — **등재된 이동 한 건, 네 프레임 공통**: `costh` **−4.5**(57.8 → 53.3).
       866 이 ref 세로를 24 대신 26 으로 읽어(=`probe866.py` `edge()` 의 국면 없는 2칸 건너뛰기)
       테를 4.5+2.2 로 키운 만큼 알약이 부풀어 있던 것을 되돌린 값이다 — 근거·되돌림은
       `verify904`(13/13)·`probe904`(9/9). ⚠ **`costt`(위 149)는 빈 칸이 정답**이다: 줄어든
       4.5px 은 **아래에서만** 나오므로 상변은 한 픽셀도 안 움직인다(`verify904` [1e]·[4a]).
       ⚠ 알약은 프레임 무관 고정 부품이라 **네 프레임에 똑같이** 적는다 — 한 프레임만 적으면
       «고정 부품인데 프레임마다 다르다» 는 거짓이 통과한다. */
    /* ⚑ 926 이관 — **등재된 이동 한 건, 1600 한 프레임**: `lintelh` **−20.78**(66 → 45.22).
       띠1(패널 상단 ↓ 아치 정점)은 `--rw-gt` 첫째 인자가 정하는 94 상수인데 그 안의 20(금테)과
       66(상인방)이 프레임 불변이라 **압축을 clearance 혼자** 뒤집어썼다(잔존 0.190 ↔ 다른 띠
       0.520~0.639 · 채점 2인 EW·EX 독립 실측). 926 이 상인방에게 그 압축을 나눠 지웠다 —
       `k = min(1, pool/108)` · pool = gt − av − lt ⇒ 1600 k 0.6852 · 상인방 45.22 · clearance 28.78.
       ⚠ **`lintelt`(상변 20)는 빈 칸이 정답**이다 — 줄어든 20.78 은 **아래에서만** 나온다
         (`--rw-lt` 하한 20 은 안 건드렸다 · `verify926` [5]).
       ⚠ **긴 네 프레임은 빈 칸이 정답**이다 — pool 이 108 이라 k = 1 이고 상인방은 66 그대로다
         (`verify926` [3] · `verify120` [②b]). 한 줄이라도 긴 쪽에 적어야 하면 그것은 새는 것이다.
       근거·되돌림 `docs/review/926-상인방압축배분.md` · 게이트 `verify926`(13/13). */
    1600: { gridt: -5.28, floort: -8, floorh: 8, groundt: -8, groundh: 8, costh: -4.5,
            lintelh: -20.78 },
    1920: { costh: -4.5 },
    2280: { costh: -4.5 },
    2600: { costh: -4.5 },
