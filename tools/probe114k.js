 ...
 *   E. **크기 규격** — 링 최대 반경 = r×0.30 = **Ø78** (운석 경로).
 *      18회차까지는 «크기를 깎지 마라» 였고 19회차도 자리·형태만 바꿨는데, 그러고도 BC[20](적의 341%)·
 *      BD[11](3.4배)가 독립으로 크기를 지적해 **두 회차째**가 됐다 → 두 처방이 겹치는 Ø70~80 으로 내렸다.
 *
...
async function run(p) {
  ...
  return await p.evaluate(async () => {
    ...
    for (let trial = 0; trial < 12; trial++) {
      ...
      chainBoomFx(bx, by, R, '#ffb45c', [0.42, 0.78], 0.30, 0.14);
...
(async () => {
  ...
  ok(Math.abs(maxR - 130 * 0.30) < 0.6, 'E. 크기 규격 — 최대 반경 ' + maxR.toFixed(1) + ' = 130×0.30 = **Ø78** (BC[20] Ø80 · BD[11] Ø70 이하 처방이 겹치는 값)');
