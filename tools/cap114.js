...
const SCENES = [
  ...
  /* ★ 7회차에 비평가 2명이 **독립적으로 «장면 안에 적이 1기»** 라고 적었다(둘 다 표창 8발 중 7발을
     «허공» 으로 채점). 실제로는 6기를 세우지만 **논리 뷰포트가 540×998** 이라 dist 430 은 6기 중
     2기를 화면 밖으로, 나머지도 가장자리로 밀어낸다 — 프레임만 보면 정말 «적이 없는 쪽으로» 날아간다.
     코드는 맞았다(같은 장면에서 8발 전부 최근접 적 방위 8.6° 이내 · 45° 초과 0발)는 것을 실측으로
     확인했고, 틀린 것은 하네스다. 6기가 전부 프레임 안에 들어오는 거리로 낮춘다.
     같은 이유로 recast 200ms 는 볼리를 계속 쌓아(8→17→26발, 640ms 동안 소멸 0) 수명·감쇠를
     채점 불가로 만들었다 → 창(640ms) 안에 **한 볼리만** 들어오게 벌린다. */
  { key:'trail',  skills:['shuri','ice'], dist:210, n:8,  gap:80, cast:'shuri', recast:700, crit:0 },
  ...
  /* 13회차 — 낙하가 등속이라는 AQ⑩·AR#3 공통 지적을 받아 index.html 에서 중력(gy)을 넣었고,
     낙하 시간이 0.69 → 0.651s 로 줄었다. lead 240 은 «가속이 이미 붙은 뒤» 만 담아 프레임당
     낙하량이 77 → 94(+22%)로만 보인다 — 설계값(55 → 100, 1.8배)을 채점하려면 앞을 더 담아야 한다.
     lead 를 160 으로 당기면 착탄이 프레임 7 근처라 앞 6장에 낙하, 뒤 6장(480ms)에 화구·여진이 담긴다 */
  { key:'boom',   skills:['meteor'],      dist:150, n:12, gap:80, cast:'meteor', recast:1400, crit:0, lead:160 },
  /* 번개도 같은 함정 — recast 200ms 로 계속 새 번개가 생겨 «잔광이 560ms 내내 안 꺼진다»(AH)가 됐다.
     단발로 재면 잔광 알파는 1 → 0.69 → 0.43 → 0.23 → 0.09 → 0 으로 480ms 에 완전히 죽는다(실측).
     감쇠를 채점하려면 창 안에 한 번만 쳐야 한다 */
  { key:'bolt',   skills:['bolt'],        dist:240, n:8,  gap:80, cast:'bolt',  recast:700, crit:0 }
];
