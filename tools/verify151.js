...
/* ⚠ **세대 상수는 손으로 적는다**(151 규약 — 제품 파생 금지). 제품에서 읽으면 «값을 바꿔도
   자는 초록» 이 되어 C2·C3·C5 가 아무것도 안 지킨다. 값이 바뀐 회차가 여기도 같이 고친다. */
const OFF_MUL_ON = 1.381, OFF_DAY_H = 11, OFF_CLAIM_H = 10.5, DAILY_MAX_D = 60;   /* 199 22회차 — 1회 상한 10.5h · 25회차 — 하루 예산 11h · **26회차 — 이용권 배율 1.2 → 1.381**(결3 ⓑ «구 +4h 동급 이상» · verify758 [P1]·[P2]) */
...
(async () => {
  ...
  /* ⚑ 199 26회차 이관 — 배율이 1.2 → 1.381 이 됐고, 표기 자리도 `toFixed(1)` 사본에서
     **725 의 `fmtMul`** 한 벌로 옮겼다(«×1.4» 로 +1.4% 부풀려 적히던 자리 · `verify758` [P4]).
     그래서 이 항이 묻는 문자열은 «×1.2» 가 아니라 **«×1.381배»**(꼬리 «배» 까지)다 —
     세대 상수 `OFF_MUL_ON` 에서 만들되 **제품에서 읽지는 않는다**(151 규약). */
  const MUL_TXT = '이용권 ×' + OFF_MUL_ON + '배';
  ok('C3 01 팝업 문구 — «1회 최대 10시간 30분» + «하루 ' + OFF_DAY_H + '시간» 두 축 · 보유는 «' + MUL_TXT + '» 를 같이 적는다(725 fmtMul)',
    /1회 최대 10시간 30분/.test(C.txtOff) && new RegExp('하루 ' + OFF_DAY_H + '시간').test(C.txtOff)
    && !/이용권 ×/.test(C.txtOff) && C.txtOn.includes(MUL_TXT),
    C.txtOff + ' ‖ ' + C.txtOn);
