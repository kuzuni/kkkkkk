...
(async () => {
  ...
  /* ⚑ 701 이관(2026-09-02) — 이 항이 **소스 글자**를 잠그고 있어서, 701 이 «비용» 을 배수 계획값
     (`temperPlan(k, trMul).cost`)으로 바꾸자 뜻이 그대로인데도 빨개졌다. 뜻은 처음부터
     «두 항뿐»(아이콘 1 + 수 1 · 낱말 0)이었으므로 **글자가 아니라 그 구조를 묻도록** 다시 적는다:
       · `curIc(` 정확히 1회  · `fmt(` 정확히 1회  · 한글 낱말 0건  · 셋째 항 0건
     이렇게 두면 «단련» 이 되살아나거나 항이 하나 더 붙는 순간 여전히 빨개지고(670 의 실제 과녁),
     비용을 어떤 식으로 구하느냐는 그 식의 주인(686·701)에게 맡긴다.
     ⚠ 라벨이 «실제로 낼 값» 을 적는다는 것은 `verify701` [F2] 가 화면에서 직접 잰다. */
  {
    const m = CODE.match(/\n\s*btn: *(curIc\('tstone'[\s\S]*?),\n\s*ok:/);
    const expr = m ? m[1] : '';
    const cnt = (re) => (expr.match(re) || []).length;
    ok(!!expr && cnt(/curIc\(/g) === 1 && cnt(/fmt\(/g) === 1 && !/[가-힣]/.test(expr),
      '[2-e] 라벨식이 «아이콘 + 비용» 두 항뿐이다(아이콘 1 · 수 1 · 낱말 0 — 686 3회차의 껍데기 `<b class="tbn">` 는 항이 아니다)',
      expr ? expr.replace(/\s+/g, ' ') : '`btn:` 라벨식을 못 찾았다');
  }
  ...
  /* ⚑ 769 이관 — 가로 340 은 **584 가 «자릿수 최악» 으로 고른 값**이었는데, 769 가 그 최악을
     다시 재 보니 340 짜리 그릇의 실제 예산은 7자리였다(686 이 fs 38 → 52 로 키우며 조용히 깎였다).
     같은 회차가 왼쪽 `.td` 의 과예약(392 → 236)을 걷어 그 폭을 여기로 넘겼다 ⇒ **496**.
     670 이 이 항으로 지키려던 것은 «내 라벨 수리가 상자를 안 건드렸다» 이므로 **폭의 근거**를
     그대로 묻는다: 자릿수 예산이 584 가 적어 둔 10자리 이상인가([5-d] 와 같은 축) · 세로는 686 값. */
  ok(Math.abs(geo.w - 496) < 0.5 && Math.abs(geo.h - 173) < 0.5,
    '[5-a] ★ 버튼 **가로** 496(769 — `.td` 과예약을 넘겨받았다) · 세로는 686 코어 173(+립 5)',
    geo.w + '×' + geo.h);
  ...
  /* ⚑ 769 이관 — 위 문단은 **686 시점의 장부**다. 769 가 `.td` 과예약(392 → 236)을 걷어
     그 폭을 버튼에 넘기면서(340 → 496) 예산이 **10자리로 되돌아왔다**(실측 여유 +39.3px).
     686 이 «라벨 확대의 대가» 로 치렀던 자릿수를 값 두 개로 되찾은 셈이라, 항의 술어를
     그때의 6자리가 아니라 **10자리**로 다시 적는다(333 처방 — 무르게 남겨 두면 예산이
     또 깎여도 이 항은 조용하다). 되돌림은 `.tr-tp>.tb` 폭 496 → 340 뿐이다. */
  ok(geo.room10 > 0,
    '[5-f] ★ 자릿수 예산 — 769 가 `.td` 과예약을 넘겨받아 **10자리**로 복귀(686 시점 6자리)',
    `6자리 여유 ${geo.room6} · 7자리 ${geo.room7} · 10자리 ${geo.room10}`);
