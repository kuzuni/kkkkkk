...
(async () => {
  ...
  /* ⚑ **199 25회차 이관**(`verify650` R5·R6 과 같은 표본) — 하루 예산 `OFF_DAY_CAP_MIN` 이
     1,440분 → **660분**으로 내려가 «잘리는 자리» 가 바뀌었다(옛 [13500, 20700, 9900] · 4번째
     20,250초). 새 세대: ① 13,500초(225분×1.5 = 337.5분 소비 · 잔량 322.5분) → ② 5.75h 는 잔량
     322.5분(19,350초)에서 잘리고 ×1.5 가 잔량을 정확히 채워 `offNoGain()` 반려(소비 0) →
     ③ 9,900초(165분×1.5 = 247.5분 · 누적 585분) ⇒ **잔량 75분 = 4,500초**. 전부 정수 = 이 절이
     재는 «드리프트 0» 은 그대로다. 값은 제품에서 파생하지 않고 여기 다시 적는다(151 규약). */
  const EXP = [13500, 19350, 9900];                 /* 부지런 프로필 앞 세 수령의 자리비움(초) · 하루 예산 660분 세대 */
  ok('R13 표본이 실시계를 안 탄다 — 앞 세 수령의 자리비움이 프로필·예산 값과 정확히 같다 (드리프트 0)',
    A.ad.preSec.length === EXP.length && A.ad.preSec.every((s, i) => s === EXP[i]),
    '[' + A.ad.preSec.join(', ') + '] = [' + EXP.join(', ') + ']');
  ok('R14 그래서 4번째의 자리비움도 정수 초다 (예산 잔량 75분 = 4,500초 · 199 25회차 세대)',
