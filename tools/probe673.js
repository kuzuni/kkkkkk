...
(async () => {
  ...
  /* ⚑ **199 25회차** — 하루 예산이 1,440분 → 660분으로 내려가면서 «버려지는 몫» 이 바뀌었다.
     옛 세대는 4번째 수령(337.5분 = 23.44%)이 통째로 버려졌고, 새 세대에서 옛 표현식이 놓치는
     것은 **마지막 잔량 75분 = 11.36%** 다(전 세대와 같은 뿌리 — 이득 없는 수령을 ×1 로 안 받는다).
     차이의 «크기» 가 아니라 «있다» 가 이 항의 축이지만, 값을 적어 두어야 세대가 바뀐 것을
     다음 회차가 알아본다(151 규약 — 제품에서 파생하지 않는다). */
  ok('[3] ⚑ 그 차이가 하루 예산의 11.4% 다 — 계수가 아니라 계측기 결함 (199 25회차 세대 · 구 23.4%)',
    Math.abs((user.min - old.min) / cap * 100 - 11.36) < 0.1,
  ...
  /* ⚑ **199 25회차 이관(333 처방 — 자리를 비우지 않고 축을 바로 세운다)** — 옛 술어는
     «거절 0회» 였다. 그것이 참이었던 것은 하루 예산 1,440분이 **하루 안에서 한 번도 안 물렸기**
     때문이고, 예산이 660분으로 내려간 지금은 **유저 경로도** 마지막 두 수령에서 빈손이다
     (잔량 0 = 줄 것이 없다 — 673 이 잡은 «광고 난간에 막혀 버린다» 와는 다른 사건이다).
     ⇒ 물어야 하는 것은 처음부터 «봇이 유저와 **같은 자리에서** 거절되는가» 다. 옛 표현식은
     여전히 다르므로([2]·[4]) 이 항은 무뎌지지 않는다. */
  ok('[6] 봇의 거절 자리가 유저 경로와 같다 — 네 번째 수령을 «봇만» 버리지 않는다',
    bot.refused === user.refused, '봇 거절 ' + bot.refused + '회 ‖ 유저 ' + user.refused + '회');
