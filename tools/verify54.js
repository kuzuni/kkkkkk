...
const GEO = [
  ...
  /* 22회차 — 윗변 기울기를 ref(−0.0976/px) 로 되돌리자 우변이 옛 상자 top(463) 위로 나가
     상자를 **455/237** 로 올렸다(바닥 692 불변 · 나머지 백분율은 절대 y 보존으로 재계산). */
  ['.rk-pod.p2', 50, 455, 328, 237],
...
(async () => {
  ...
  const probe = await page.evaluate(async (b64) => {
    ...
    return {
      ...
      /* 22회차 — 윗변이 기울기 교정으로 옮겨져 띠 표본점도 새 선 위로 옮긴다(x85 491 · x345 465) */
      rim2: [[85, 491], [345, 465]].map(([x, y]) => [x, y].concat(px(x, y - 3))),
  ...
  /* 22회차 — **기울기를 ref 로 맞춘 뒤의 선**. 옛 기대값 {85:487,150:483,320:471,345:469} 는
     기울기 −0.0626/px 짜리라 ref(−0.0976/px, 측정표 §3-1a: 프레임 489@x120 → 469@x325)보다
     37% 완만했다. 새 선 = 중심 (214,478) 고정 + ref 기울기 → y(x) = 492.03 − 0.0976·(x−70.2).
     ref 선(489 − 0.0976·(x−120)) 과는 전 구간 **1.8px 차이로 평행**하다 — 그 1.8px 은 마스크
     보정차(검정외곽 스캔은 반대로 «1.4px 낮다» 고 읽는다)라 쫓지 않는다(교훈 16). */
  const WANT2 = { 85: 491, 150: 484, 320: 468, 345: 465 };
