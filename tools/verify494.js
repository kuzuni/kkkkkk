...
console.log('[9] 20회차 — ① 판정 줄 · 단일 정책 «비» 칸 · [E3] 축별 기울기 · §8 상설 대조');
{
  ...
  /* ⓑ «비» 칸 — 빈칸·«-» 금지 (977: 두 표 모두) */
  {
    ...
    /* ⚑⚑ 977 — 이 항이 **단일 정책 표만** 봐서 규약의 절반이 무방비였다. 33회차가 세운
       «창 역량» 줄은 비 칸이 맨 `—` 라 여기서 1건 빨강이었지만(그래서 등재됐다), 정책이
       둘인 표의 **분모 0** 자리는 맨 `'-'` 인 채 **아무 자도 안 짖었다**(LESSONS 232-① —
       «빨강 옆에는 헛초록이 같이 산다»). ⇒ 판정을 한 벌로 적고 **두 표에 각각** 건다:
         · 단일 정책 — 비는 존재할 수 없다 ⇒ 모든 칸이 **말해야** 한다.
         · 두 정책   — 비는 존재한다 ⇒ **수이거나**, 수가 아니면 이유를 **말해야** 한다.
       ⚠ 두 표를 한 항으로 묶지 않는 이유: 묶으면 «수» 를 허용해야 해서 단일 정책 쪽이
       다시 «다른 실행의 수가 앉아도 초록» 으로 되돌아간다(정정4 가 막은 그 자리). */
    const DECL = (c) => /못 잰다|이 실행의 수가 아니다/.test(c);
    const NUM  = (c) => /^_*\**\s*(-?\d|Infinity|∞)/.test(c);
    /* [G] 표의 데이터 행만 — 단일 정책은 3칸(축·값·비), 두 정책은 4칸(축·값·값·비) */
    const gRows = (t, n) => (t.split('## [G]')[1] || '').split('\n')
      .filter(l => /^\| /.test(l) && cellOf(l).length === n && !/^\|---/.test(l) && !/^\| 축 \|/.test(l));
    const badOf = (t, n, pass) => gRows(t, n).filter(l => !pass(cellOf(l)[n - 1] || ''));
    const rows = gRows(om, 3);
    const bad  = badOf(om, 3, DECL);
    ok(rows.length >= 10 && bad.length === 0,
       `단일 정책 [G] 의 «비» 칸이 전부 «못 잰다» 이거나 **출처를 밝힌 대조 값**이다 (행 ${rows.length} · 어긴 행 ${bad.length}건`
       + (bad.length ? ` — 첫 행 «${cellOf(bad[0])[0].slice(0, 40)}»` : '') + ')');
    const rows2 = gRows(md, 4);
    const bad2  = badOf(md, 4, (c) => NUM(c) || DECL(c));
    ok(rows2.length >= 10 && bad2.length === 0,
       `두 정책 [G] 의 «비» 칸이 전부 **수**이거나 못 재는 이유를 스스로 적는다 (행 ${rows2.length} · 어긴 행 ${bad2.length}건 — 977: 맨 «-» 가 여기서는 초록이었다`
       + (bad2.length ? ` — 첫 행 «${cellOf(bad2[0])[0].slice(0, 40)}»` : '') + ')');
    /* §R977 되돌림 — 위 두 항이 «아무 문자열이나 통과시키는 자» 가 아님을 픽스처로 못박는다.
       맨 `—`·맨 `-` 는 두 판정 모두에서 빨갛고, 수는 두 정책 쪽에서만 초록이어야 한다.
       (이 시험이 없으면 판정을 `/./` 로 무르게 풀어도 위 두 항은 초록이다 — LESSONS 232-①) */
    {
      const FX = ['## [G] 픽스처', '| 축 | 부지런한 유저 | 대충 유저 | 비 |', '|---|---|---|---|',
                  '| 맨 대시 | 1 | 2 | — |', '| 맨 하이픈 | 1 | 0 | - |',
                  '| 수 | 1 | 2 | 1.275 |', '| 말한다 | 1 | 0 | — ⚠ **분모(대충 유저)가 0 이거나 없다 — 못 잰다** |'].join('\n');
      const b2 = badOf(FX, 4, (c) => NUM(c) || DECL(c)).length;
      const b1 = badOf(FX, 4, DECL).length;   /* 단일 판정은 «수» 도 안 받는다 */
      ok(b2 === 2 && b1 === 3,
         `§R977 되돌림 — 픽스처에서 맨 «—»·«-» 가 실제로 빨갛다 (두 정책 판정 ${b2}/2 · 단일 판정 ${b1}/3 = 수까지 거부)`);
    }
