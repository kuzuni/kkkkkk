...
const ALLOW = [
  ...
  "'\u{1F3AB} 프리미엄 패스 — '",              /* 589 결제 목업 우편 제목 머리글자(재화 아님 · 화면엔 안 온다) */
...
(async () => {
  ...
  /* ── 작업 607 — **새 항목(589 결제 목업 우편 제목)에도 같은 짝을 단다.** 370 이 세운 규약이
     «허용 목록에 한 줄을 더할 때마다 그 항목이 무엇을 여전히 잡는가를 같이 못박는다» 이므로,
     항목만 갈아 끼우고 시험을 안 옮기면 목록의 절반이 짝 없이 남는다(그 자리가 곧 A3 의 다음 부패다). */
  /* R2b — 새 항목이 «프리미엄 패스» 라는 말이 아니라 **🎫 라는 글리프**에 걸려 있는가. */
  R('§R2b 607 항목은 글리프까지 못 박는다(우편 제목 머리글자를 🪙 로 바꾸면 빨강)',
    b => b.replace("'\u{1F3AB} 프리미엄 패스 — '", "'\u{1FA99} 프리미엄 패스 — '"),
    (r, got) => got.length === 1);
  /* R4b — 제품에서 그 줄이 사라지면 **A3 이 먼저** 잡는다(방금 고친 부패가 그대로 재발하지 않는다). */
  R('§R4b 그 줄이 사라지면 A3 이 죽은 항목으로 잡는다',
    b => b.replace("t:'\u{1F3AB} 프리미엄 패스 — '", "t:'프리미엄 패스 — '"),
    r => r.dead.length === 1 && r.dead[0].indexOf('프리미엄 패스') >= 0);
  ...
  const sweep = await page.evaluate(async (ART_N) => {
    ...
    const steps = [
      ...
      ['우편-결제확인', () => { buyPassPrem('stage'); openMail(); }],
    ];
