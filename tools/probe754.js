...
const EXEMPT = [
  ...
  /* 815 등재(2026-09-03, 위임 규약 채택) — 6회차 비평가 CE 가 «상변이 38 → 732 로 표류한다» 로
     지목한 자리다. `probe815` 재현이 수치는 확인하고 판정은 뒤집었다:
       ① 06 은 측정표 §0 이 픽셀로 판정한 **바닥 시트**다 — 하변이 앵커이고 «바닥은 네비바에 잘린다».
       ② 긴 프레임에서 시트 위 띠가 자라는 것은 지시서 [2] «남는 높이는 전투 캔버스가 흡수» 그 자체다.
       ③ 짧은 프레임에서 시트가 위로 자라는 것은 **403(주인 지시)** 이 세운 기하이고 가드 142 는 351 것이다.
       ④ «상변이 뒤 사이드 레일을 가로로 자른다» 는 **레퍼런스가 그렇게 그렸다**(ref 패널 상변 577 ·
          좌측 퀘스트 두루마리 555..665 중 89px 가림 — `docs/review/815-06시트그릇앵커모드.md` §2-ⓑ).
     CE 처방을 주입하면 기준 프레임이 깨진다(상·하 동시 앵커 = 시트 1584 → 1958 · 피치 400 → 549.8 /
     중앙 피벗 = 하변이 네비바에서 187px 뜬다). 그 대가는 `verify815` [R] 이 직접 잰다. */
  { sel: '.eqp',       why: '06 장비 «바닥 시트» — 하변 앵커가 곧 껍데기 규격(측정표 06 §0) · 상변 = max(142, 프레임−1764) 는 403(주인 지시)·351 가드·레퍼런스 1584 의 산수 · 근거·되돌림은 verify815' },
];
