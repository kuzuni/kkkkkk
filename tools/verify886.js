...
(async () => {
  ...
  /* ⚑ 879 7회차 이관(333 — 자리를 비우지 않고 «무엇이 상수인가» 로 옮긴다).
     옛 항은 «1600 은 **절반**»(bandPed ≥ bandAll × 0.5)이었는데, 그 0.5 는 성질이 아니라
     **그때의 두 수가 40/80 이었다는 우연**이다. 879 7회차가 «아래 예약» 을 80 → 88 로 올리자
     같은 받침이 45.5% 가 되어 이 항만 빨개졌다 — 재는 대상(받침)은 한 픽셀도 안 움직였는데.
     ⇒ 사실인 쪽을 못박는다: **받침은 프레임 무관 40 × 조상 배율**이고, 그 아래에는 접합선 띠
       13 이 들어갈 자리가 **항상 남는다**(= 이 구간은 «빈 곳» 이 아니다 — 879 §18 의 주장).
     비율은 판정이 아니라 **기록**으로 남긴다(1600 50.0% → 45.5%). */
  ok(FRAMES.every(H => Math.abs(r[H].bandPed - 40 * r[H].sc) <= 0.6)
     && FRAMES.every(H => r[H].bandAll - r[H].bandPed >= 13 * r[H].sc - 0.6),
    '[4] «지면선 ↓ 수반» 구간은 빈 곳이 아니다 — 받침이 **40 × 조상 배율**(프레임 무관)을 먹고, 그 아래에 접합선 띠 13 자리가 남는다(879 §18 의 자 · 7회차에 «절반» 이라는 우연을 걷어냈다)',
    FRAMES.map(H => H + ':' + r[H].bandPed + '/' + r[H].bandAll
      + '(' + (r[H].bandPed / r[H].bandAll * 100).toFixed(1) + '%)').join(' · '));
