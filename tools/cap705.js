#!/usr/bin/env node
/* 705 캡처 — 19 프로필 · 20 종합스탯을 **짝으로** 찍는다(같은 팝업의 두 탭이므로 나란히 봐야 한다).
 *
 *   node tools/cap705.js [회차]     기본 r1 → docs/review/705-r1-{19,20}-{2280,1600}.png
 *
...
(async () => {
  ...
  for (const H of [2280, 1600]) {
    ...
    await page.screenshot({ path: path.join(OUT, `705-${R}-19-${H}.png`) });
    await page.evaluate(() => { const e = document.querySelector('.pf-tgl>.lb'); if (e) e.click(); });
    await page.waitForTimeout(900);
    await page.screenshot({ path: path.join(OUT, `705-${R}-20-${H}.png`) });
    await ctx.close();
    console.log('찍음: 705-' + R + '-{19,20}-' + H + '.png');
  }
  await browser.close();
