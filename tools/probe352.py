...
def main():
    ...
    # ⚑⚑ 970 (2026-09-06) — 원점 **2027 → 2028**. «어느 정수가 맞나» 가 아니라 **ref 와 cap 이
    #   서로 다른 규칙을 쓰고 있었다**: cap 은 셸상변 1960 + 테두리 **7** = 1967 인데
    #   ref 만 2021 + **6** = 2027 이었다(437 이 ref 테두리를 7 로 확정한 뒤에도 이 리터럴만 안 따라왔다).
    #   직접 단면이 못박는다 — ref x420 세로 y2021..2027 이 검정 **7행** · cap x420 y1960..1966 도 **7행**.
    #   958 2회차·968 이 «두 자를 섞은 짝» 이라 부른 그 얼굴의 세 번째다.
    #   ⚠ 이 한 줄로 눈금이 30.6 → **29.6** 이 된다(리터럴 1px 당 ∓1.1~1.3). 968 의 과녁문
    #   «rx 32.7 ± 1.3» 은 그 ±1.3 이 통째로 이 리터럴이었으므로 **폐기**다 — 새 판정은
    #   `tools/probe970.py` 가 든다(원점 셋을 그림에서 부분 화소로 찾고 대조군으로 자기검산한다).
    radius(ref7, 292, 551, 2028, 'ref')
    if has:
