...
const TOAST_SITES = [
  /* 757 이관 — 757(만렙 잉여 조각 환급)이 이 자리를 «도감 완성 전체 환불» 에서 «만렙 조각 환급»
     으로 넓히면서 문구가 바뀌었다(401 과 같은 꼴 — 757 은 잘못한 게 없고 조각을 상수로 박아 둔
     이쪽이 낡았다). 149 가 재는 성질(«팝업이 아니라 토스트로 나간다»)은 그대로다.
     ⚠ 문구만 갈아 끼우면 «757 이 통째로 사라져도 초록» 이 되므로 그 근거(환급가 파생·만렙 범위)는
       `tools/verify757.js` 가 따로 못박는다(333 처방). */
  ['조각 환급 — 대상 없음',      '환급할 조각이 없습니다'],
  /* 720 이관 — 720(주인 지시 «도감 완성이 아니라 최대치 달성으로 Lock»)이 이 자리의 **이름**을
     바꿨다. 판정(`allMaxed` = 전 종 보유 + 전 종 만렙)은 720 전에도 그대로였고 어긋난 것은
     이름이었다(`probe720` [1]·[3]) — 401·757 과 같은 꼴로 **720 은 잘못한 게 없고** 옛 이름을
     상수로 박아 둔 이쪽이 낡았다. 149 가 재는 성질(«팝업이 아니라 토스트로 나간다»)은 그대로다.
     ⚠ 조각을 `MAXED_TXT + ` 까지 잡는 이유: 문자열만 갈아 끼우면 «720 이 이름을 도로 두 자리에
       따로 적어도 초록» 이 된다. 이름의 단일 출처·판정 축은 `tools/verify720.js` 가 못박는다(333 처방). */
  /* ⚑ 800 이관(주인 지시 2026-09-02 «스킬 환급 만들었으니까 뽑는거 막지마봐 도감완성으로») —
     720 이 이름을 `MAXED_TXT` 로 모아 둔 이 토스트는 **차단이 폐지되면서 자리째 사라졌다.**
     757 조각 환급이 «만렙 뒤 조각은 쓸 데가 없다» 는 전제를 없앤 결과다.
     갈아 끼울 **살아 있는** 조각이 없다(같은 계열의 반려는 아래 «206 합성 반려» 가 이미 잡는다)
     ⇒ 310 선례대로 **항목과 재료를 같이 지운다**. 그 부재는 `tools/verify800.js` [C]·[E] 와
     `tools/verify720.js`(방향을 뒤집었다)가 못박는다. */
  ['소환 재화 부족',             '더 필요합니다\');\n    return;'],
  ...
  ['206 조각 환급 결과',   "조각 환급 — ' + curIc("],   /* 757 이관 — 위 «조각 환급 — 대상 없음» 과 같은 이유 */
...
const WORST = [
  ...
  /* 800 이관 — 720 이 이름을 갈아 끼운 이 워스트케이스는 차단 폐지와 함께 사라졌다(§1 주석).
     아래 «206 조각 환급»(배너 이름 + 다이아 축)과 «206 합성 반려»(부위 이름 축)가 이미 그 두 축을
     재고 있으므로 복제하지 않고 **항목과 재료(`D.coll`)를 같이 지운다**(310). */
