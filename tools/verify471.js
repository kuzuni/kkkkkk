...
const EXC = {
  ...
  '35 패스 탭 #psBar .pt>.bdg':
    '같은 이유로 프레임 변에 묶이지만 **플러시가 아니다** — 마지막 칸 우변 1074(프레임 1080)라 ' +
    '필요 후퇴는 1074 + 21 − 1080 = **15**다. 6회차에 21 → 15 로 내렸다(§10 ① · 5회차 BU·BV 독립 일치). ' +
    '두 자리는 «한 식이되 값이 다르다» — 그 산수는 [S] 가 매번 다시 푼다',
  '21 도감 세트 [강화] .clb-btn>.updot (516)':
    '`.cl-body{overflow-x:hidden}`(폭 860) 안이고 버튼 우변 뒤로 남는 폭이 13px 뿐이다 — ' +
    '규약 11 이면 60 쥬시 등장 봉우리(`jzDotIn` scale 1.3 → 링 반지름 27.3)에서 오른쪽 3.3px 이 잘린다. ' +
    '516 이 가로만 16 으로 밀어 봉우리에서 2.3px 여유를 뒀다(자는 `probe516` [D]·[E]). ' +
    '⚑ **이 예외는 471 5회차 «뒤에» 생겼고 6회차까지 이 자에 없었다** — 자에 없는 예외는 ' +
    '[A] 음성항(«예외는 정확히 n자리»)을 조용히 헐겁게 만든다',
  /* 611(2026-09-01) — 사유가 «코너 점유(Lv.n)» 에서 **클립 산수** 로 바뀌었다(주인 재보고).
     점유물 `Lv.n` 은 좌상단 거울로 옮겼고 카드 overflow:hidden 도 걷었다 — 남은 제약은
     격자 하나다. 구 값(right:8/top:45 = dxR 26.5/dyT 63.5 임의 좌표)으로 되돌리면 반려. */
  '05 카드 .wgc>.updot':
    '가로만 규약 11 → 22(516 예외 ⑤ 와 같은 산수) — 격자 우변 뒤 여유 6px(카드 838 ↔ ' +
    '`.wm-grid{overflow-x:hidden}` 844)인데 마지막 열이 잠금 더미가 아니라 **실물**이다(무기 ' +
    '등급당 5종 = WPN_COLS). 규약 11 이면 정지부터 4px, 등장 봉우리(링 27.3)에서 10.3px 반달 — ' +
    '필요 조건 in-x ≥ 27.3 − 6 = 21.3 → 22. 세로는 클리핑이 없어 규약 11 그대로(걸침 유지)',
...
(async () => {
  ...
  /* ── [S] 프레임 변 예외의 **산수** — 상수가 아니라 식을 단언한다 (6회차 신설) ─────────────
     §10 ① 이 두 회차를 잃은 자리다. 5회차까지 두 예외(01 탭바 · 19 패스 탭)의 사유를 «자기 바깥
     반지름» 이라고 적어 두었는데 **그 말이 맞는 것은 호스트가 프레임 변에 플러시일 때뿐**이고,
     19 는 마지막 칸 우변이 1074 라 6px 이 근거 없이 물러나 있었다(비평가 BU·BV 독립 일치).
  ...
  [['HUD 탭바 .tab .bdg', 21], ['35 패스 탭 #psBar .pt>.bdg', 15]].forEach(([k, want]) => {
    const r = has.find(x => x.label === k);
    ok(r && Math.abs(r.needIn - want) <= 0.5,
      '[S] 프레임 변 예외 — 필요 후퇴를 식으로 다시 풀면 ' + want + ' 다 — ' + k,
      r ? '마지막 칸 우변 ' + r.maxHR + ' + 바깥R ' + r.outR + ' − 1080 = ' + r.needIn : '없음');
    ok(r && Math.abs(r.dxR - r.needIn) <= 0.5,
      '[S] 제품이 그 값을 쓰고 있다 (더 물러나 있으면 «근거 없는 여백» 이다) — ' + k,
      r ? '제품 dxR ' + r.dxR + ' ↔ 필요 ' + r.needIn : '없음');
  });
  /* 음성 — 두 자리가 «한 식이되 값이 다르다» 는 것 자체를 못박는다.
     같은 값으로 되돌리면(둘 다 21) 19 가 위 항에서 빨개진다. */
  {
    const a = has.find(x => x.label === 'HUD 탭바 .tab .bdg');
    const b = has.find(x => x.label === '35 패스 탭 #psBar .pt>.bdg');
    ok(a && b && Math.abs(a.needIn - b.needIn) > 1,
      '[S] 음성 — 두 예외는 같은 식이되 값이 다르다 (한 상수로 묶으면 한쪽이 근거를 잃는다)',
      a && b ? '탭바 ' + a.needIn + ' ↔ 패스 탭 ' + b.needIn + ' (플러시 1080 ↔ 1074)' : '없음');
    ok(b && b.cutMax && b.cutMax[1] === 0,
      '[S] 내린 값이 실제로 안 잘린다 (15 는 «최소» 이지 «부족» 이 아니다)',
      b ? '우측 잘림 ' + b.cutMax[1] + 'px' : '없음');
  }
