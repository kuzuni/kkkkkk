...
const MEASURE = `(() => {
  ...
  return { panelH: +p.h.toFixed(2), o, lay, pedT, slotBot,
    ...
    gapMid:  +(o.mid.t - o.mul.b).toFixed(2),                 /* ★ 919 — 바 ↓ **수반 구획 상변**(그려진 아래 이웃) */
...
(async () => {
  ...
  /* ⚑⚑ **919 이관(879 7회차) — 이 항은 방향이 거꾸로였다.** 옛 문구:
     «위 간극 > 아래 간극(지면선까지) × 1.5 — 바가 아래(수반·소환 버튼) 블록에 붙어 읽힌다(CQ 의 기능 축)».
     아래 끝점이 **지면선**(바 하변에서 15.76px · 그려지지 않는 구성선)이라 이 비는 어떤 배분에서도
     초록이었다(1600 2.51 · 긴 프레임 4.50 = 헛초록). 끝점을 **그려지는 아래 이웃**(수반 구획
     `.rw-mid` 상변)으로 옮기면 값이 **뒤집힌다** — 아래가 위의 **1.86~1.92배**이고,
     이는 다섯 프레임 전부에 같은 크기로 있다(1600 만의 결함이 아니다).
     ⇒ 자리를 비우지 않고 **참인 쪽을 못박는다**(333): 867 이 바를 «격자 아래 니치» 에 앉힌 결과가
       바로 이 배분이다. 879 6회차 채점자 EU 가 «바가 격자에 2배 가깝다» 로 적은 것이 이 값이고,
       그것을 **바꾸려는 작업은 886**(867 의 바 앵커를 다시 여는 건)이다 —
       886 이 바를 내리면 이 항이 빨개지는 것이 **의도한 동작**이다(그때 333 으로 다시 갈아 낀다). */
  ok(FRAMES.every(H => r[H].gapMid > r[H].gapUpDrawn * 1.5),
    '[2b] ★ 바 하변 ↓ **수반 구획 상변**(그려진 아래 이웃) > 격자 ↓ 바 × 1.5 — 867 의 니치 배치가 다섯 프레임에 그대로 있다(919 이관: 옛 «지면선» 끝점은 방향이 거꾸로였다)',
    FRAMES.map(H => H + ':' + (r[H].gapMid / r[H].gapUpDrawn).toFixed(2) + ':1').join(' · ')
      + ' · 옛 «지면선» 자로는 ' + FRAMES.map(H => (r[H].gapUp / r[H].gapDown).toFixed(2)).join('/') + ':1');
