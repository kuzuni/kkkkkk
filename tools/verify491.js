...
(async () => {
  ...
  const holdRun = async (pg) => {
    ...
    /* ⚑ 659·660 이관(작업 693) — «회당 피드백» 의 **잉크가 무엇인지**가 바뀌었다.
       ...
       ⚠ **새 항 이름은 [6-m]·[6-n] 이다** — [6-e]~[6-l] 은 613·614 가 [충전]·[회수]와 함께 걷어낸
         **은퇴한 이름**이고, 그 이름들이 아직 `docs/UI-REFERENCE.md`·`LESSONS.md`·review 491 에서
         옛 뜻으로 인용된다(365 «id 를 재사용하지 않았다» 와 같은 함정 — 되살리면 두 뜻이 한 이름을 쓴다). */
    await pg.evaluate(() => {
      const hd = document.querySelector('#trTemper .tr-tp.k0');
      window.__v491hb = 0;
      if (!hd) return;
  ...
  };
  {
    ...
    if (h) {
      ...
      ok(h.fx.hb >= 1,
         '[6-m] 회당 **맥박**(`jz-hb`)의 호스트가 **행**(.tr-tp) 이다 — 488 «버튼이 아니라 행» 의 축(693 신설)',
         'jz-hb ' + h.fx.hb + '회');
      ok(h.fx.plus.length === 0,
         '[6-n] ★음성항 — 폐지된 숫자 플로터(`.fx-plus.hb`)는 0장이다(659 «존나 후지다» 가 지켜진다 · 693 신설)',
         h.fx.plus.length + '장');
    }
  }
  ...
  /* ── §R5 되돌림(693 신설) — §6 의 갈아 끼운 두 항([6-f] 잉크 · [6-m] 맥박)이 무르지 않다는 증명 ──
       ...
       (`rtFirstFx` = 첫 발 버스트 · `hbBeat` = 회당 맥박). */
  {
    ...
    try {
      ...
      ok(!!h6 && h6.fx.hb === 0,
         '[R5-b] ★ 맥박을 뺀 사본에서는 `jz-hb` 가 0회다 — [6-m] 가 실제로 그것을 세고 있다',
         h6 ? h6.fx.hb + '회' : '표본 없음');
      await b6.ctx.close();
    } finally {
      try { fs.unlinkSync(NEG); } catch (_) {}
