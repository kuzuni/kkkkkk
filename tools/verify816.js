...
const fs = require('fs');
/* ⚑ 884 — [R1] 계열의 자. `probe816` 과 **같은 자**를 쓴다(402 «두 벌 금지») */
...
(async () => {
  ...
  /* ⚑⚑ 884 — **이 항의 자를 홀드에서 결정적 자로 옮겼다**(`tools/keepcov884.js` · 873 이 `travel838` 에서,
     871 이 [C1] 에서 한 것과 같은 처방). 838 5회차가 문턱을 «≥25%» → «≥5%» 로 한 번 내렸지만
     내려간 문턱도 **잡음 안**이었다: 같은 환경 11회 실측 최대 덮임 4.9~14.9% 로 한 번은 4.9%(빨강).
     흔든 것은 난수가 아니라 **표본 시각과 세대 위상**이라 시드만 고정해서는 안 줄어든다
     (884 §2 — 트리거 직전 재시드까지 해도 10.0~16.8%). 결정적 자는 단발 한 세대를 낳고
     `currentTime` 을 5ms 씩 감아 수명 전 구간을 훑으므로 **같은 시드에서 값이 소수점까지 재현**된다.
     ⚠ **문턱은 838 뒤에 다시 재서 맞췄다**(등재문 지시 — «그냥 내리는 수리는 반려»):
       수리 전 고정 시드 여덟 판 실측 **7.5~17.6%** · ≥5% 표본 **9~18개/77**. 홀드 값은 기록으로 남긴다. */
  const dPre = await sweep('none'), dNow = await sweep(null);
  console.log('       · 홀드 자(기록 전용): 최대 ' + p1((r1 ? r1.max : 0) * 100) + '% · ≥5% 표본 '
              + (r1 ? r1.n05 : '?') + '개 · ≥25% ' + (r1 ? r1.n25 : '?') + '개 (838 전에는 최대 70.4% · ≥25% 13개)');
  console.log('       · 결정적 자(884 · 고정 시드 ' + SEEDS.length + '판): 수리 전 판마다 최대 덮임 '
              + (dPre.per || []).map(s => p1(s.num.max * 100) + '%').join(' · '));
  ok(!dPre.err && dPre.covered === dPre.seeds.length && dPre.maxMin >= 0.05,
     'R1 신고를 지우면(816 이전) 숫자가 **모든 판에서** 다시 덮인다 — [B3]·[B4] 가 빨개지는 자리(884 결정적 자)',
     dPre.err || (dPre.covered + '/' + dPre.seeds.length + '판 · 최대 덮임 '
     + p1(dPre.maxMin * 100) + '~' + p1(dPre.maxMax * 100) + '% · ≥5% 표본 '
     + dPre.n05Min + '~' + dPre.n05Max + '개/' + dPre.frames));
  ok(!dPre.err && dPre.n05Min >= 4,
     'R1b 그 덮임이 **한 순간이 아니다** — ≥5% 표본이 모든 판에서 4개(20ms) 이상',
     dPre.err || ('≥5% 표본 ' + dPre.n05Min + '~' + dPre.n05Max + '개 / ' + dPre.frames + '표본(수명 380ms)'));
  /* ⚠ 문턱이 «0» 이 아니라 1% 인 이유 — 격자가 잉크 상자를 `floor`/`ceil` 로 한 칸씩 넓혀 훑어서
     구멍이 정확히 지켜져도 모서리 한 칸이 걸릴 수 있다(여덟 판 중 하나 = 2,576칸 중 1칸 = 0.04%).
     제품이 아니라 **자의 반올림**이고, 실측 최대 0.04% ↔ 문턱 1% = 25배 여유. */
  ok(!dNow.err && dNow.maxMax < 0.01 && dNow.coinMin >= 0.20,
     'R1c 짝 항 — **같은 자**로 제품 선언을 재면 여덟 판 전부 1% 미만이고, 그 판에서 코인(미신고 잉크)은 덮인다',
     dNow.err || ('숫자 최대 ' + (dNow.maxMax * 100).toFixed(2) + '% · 코인 '
