 ...
 *
 *   · 219(13회차) → **231**(14회차 — 금테 내측 8→20) → **174**(16회차 — 계단 1단 84 를 빼고 바닥 27) → **182**(879 7회차 — 바닥 27 → 35)
 ...
 *   [3] 1600 의 «아래 예약» 88c(받침 40 + 접합선 띠 13 + 바닥 35)가 실측으로 지켜진다 (879 7회차: 80 → 88)
 ...
 *   [R2] 예약 182 → 266 로 1600 에 계단 1단을 되살린 사본 → **아치 종횡비 < 1:1.25**
...
/* ⚑ 879 7회차 — **80 → 88.** 16회차의 80(받침 40 + 띠 13 + 바닥 27)에 8 을 더해 «바닥 27 → 35».
   근거는 879 6회차 채점 2인(ET·EU)의 1순위(«바 하변 ↔ 수반 상변» 띠가 혼자 잔존율 0.553)이고,
   재원은 av 다(gt 가 1600 에서 `av + 94` 분기라 av 를 d 줄이면 그 띠가 2d 는다).
   d 의 상한은 «배수 바가 안 줄어드는 av 하한 134.46» 이 정한다 — 예약으로는 89.3 이고 88 을 썼다.
   174 = 94 + 80  →  **182 = 94 + 88**(제품 `--rw-av` 둘째 인자). 사슬은 219 → 231 → 174 → 182. */
const RESERVE = 88;       /* «아래 예약» = 받침 40 + 띠 13 + 바닥 35 (182 = 94 + 88 · 879 7회차) */
...
const OVERRIDE = {
  ...
  /* [R2] 아래 예약 182 → 266(= 94 + 88 + 계단 1단 84). 나머지 두 인자는 원식 그대로다. */
  step: '#relw .rw-panel{--rw-av:min(calc(186px * var(--rwc,1)),' +
        'calc((var(--rw-tt) - 266px * var(--rwc,1)) / 2),' +
...
const MEASURE = () => {
  ...
  return {
    ...
    c80: probe('calc(88px * var(--rwc,1))'),   /* 879 7회차 — 80 → 88 */
    steps, visN, visH,
...
(async () => {
  ...
  /* [3] 1600 의 «아래 예약» — 16회차가 174(=94+80)로 확정하고 **879 7회차가 182(=94+88)로 올린**
     그 예약이 실측으로 서 있는가. 이 프레임에서만 `(tt − 182c)/2` 인자가 이기므로 sh 가 예약과
     **정확히** 같아야 한다. ⚠ 리터럴을 여기 옮겨 적는 대신 `--rwc` 를 문 클론으로 잰다(c88). */
  {
    const m = base[1600], b = budget(m);
    ok(Math.abs(m.sh - m.c80) < 1.5,
      `[1600] [3] 아래 예약 88c = 받침 40 + 띠 13 + 바닥 35 가 실측으로 선다 (879 7회차 182 = 94 + 88 · 16회차의 174 = 94 + 80 에서 올렸다)`,
      `sh ${m.sh.toFixed(2)} vs 88c ${m.c80.toFixed(2)} · av ${m.av.toFixed(2)} (= (tt ${m.tt.toFixed(1)} − 182c) / 2)`);
    ok(Math.abs(m.gd - m.c40) < 0.6 && b.rest >= 20 * m.rwc - 0.6,
      `[1600] [3] 그 예약의 내역 — 받침 40c · 띠 14c · 남는 바닥 ≥ 20c (879 7회차 뒤 바닥은 27 → 35)`,
  ...
  /* ── [R2] 1600 에 계단 1단을 되살린 사본(예약 182 → 266) ───────────────────
