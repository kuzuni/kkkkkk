 ...
 *   [2] 스킬 — «최대치 달성»(전 종 보유 Lv100)         → 딤 1건 · 소환 차단
 *   [3] 문구 — 딤/안내가 «최대치» 를 말하는가(수리 전 «도감 완성» = 빨강 = 등재문이 참)
 *   [4] 펫 — [1]·[2] 와 같은 축(자매 배너)
 *   [5] 장비(무기) — 최상위 등급 `maxLv=Infinity` 라 **구조적으로 안 잠긴다**(수리 전·후 동일)
 *   [6] 콘솔 에러 0
 *
 * ⚑ [1]·[2]·[4]·[5] 는 구조축(수리 전·후 같은 답)이고, **[3] 만 수리 전에 빨간 것이 정상**이다.
 *
...
(async () => {
  ...
  if (!s2) { ok(false, '[2] 표본 주입 실패'); }
    ...
    ok(s2.clk, '[2-b] 카드 딤(Lock) 1건', s2.clk ? '있음' : '없음');
    const f2 = await ev(page, FIRE, { b: 'skill' });
    info('소환 실행 ' + (f2 ? f2.fired : '?') + '회 · 안내 ' + JSON.stringify(f2 ? f2.notes : []));
    ok(!!f2 && f2.fired === 0, '[2-c] 소환이 차단된다', f2 ? f2.fired + '회' : 'n/a');

    /* ───────── [3] 문구 ───────── */
    blk('[3] 문구 — Lock 이 스스로를 무엇이라 부르는가 (수리 전 빨강이 정상)');
    ok(saysMax(s2.clkText), '[3-a] 카드 딤 문구가 «최대치» 를 말한다', '«' + s2.clkText + '»');
    const note = (f2 && f2.notes.length) ? f2.notes.join(' | ') : '';
    ok(saysMax(note), '[3-b] 소환 차단 안내가 «최대치» 를 말한다', '«' + note + '»');
  }
  ...
  if (p2) ok(p2.allMax && p2.clk, '[4-b] 최대치 달성 → 딤 1건', '만렙 ' + p2.allMax + ' / 딤 ' + p2.clk);
