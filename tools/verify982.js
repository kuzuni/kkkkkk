...
/* [C3]·[C4] 문턱 — «표에 없는 종이 안 움직였는가» 를 **Δ0 으로 물으면 자가 스스로 플레이키다**.
   ...
   **두 무리로 갈리고 사이가 비었다**: 안 댄 종의 흔들림 ≤ 0.02 ↔ 표에 적은 네 종의 이동 ≥ 0.06.
   ⚑ 다만 [C3] 은 **한 판 안에서** 두 번 재므로 그 판 흔들림만 탄다 — 실측 0.0015 다.
   ⇒ 실측이 **두 무리로 갈리고 사이가 비었다**: 안 댄 종의 흔들림 ≤ 0.002 ↔ 표에 적은 네 종의
   이동 ≥ 0.06. 빈 구간의 0.02 를 [C3] 이 쓰고, 그것이 «빈 구간» 임을 [C4] 가 반대쪽에서
   못박는다(825 — 한쪽만 세우면 문턱이 곧 손 상수다).
   ⚠ 판 사이 흔들림(0.017)은 이 행이 만든 것이 아니다(수리 전 트리에서 같다) — 곁다리 985 등재. */
const MOVE_MIN = 0.04;
