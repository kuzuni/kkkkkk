...
(async () => {
  ...
  /* ⚠ **6회차 재조정 — 키프레임 위치만 찍으면 «같은 상태» 표본이 겹친다.**
     [0,10,20,50,70,84,90,95] 는 0·20 이 둘 다 «착지», 10·50·70 이 셋 다 «짧은 들썩 정점» 이라
     8장 중 서로 다른 상태가 5개뿐이었다. 6회차 비평가 I 가 그 중복을 «한 바퀴 안에 3바퀴가 들어 있다 ·
     4~5Hz 진동» 으로 읽어 축 ① 을 4점으로 깎았다 — **연출이 아니라 표본이 만든 지적**이다
  ...
  console.log('bob ' + BOB.length + '장 — 카드1 확대, 키프레임 위치 ' + BOB.map(v => v + '%').join('/')
    + ' (짧은 들썩 곡선: 착지 0 → 상승 5 → 정점 10 → 하강 15 → 착지 20 ·'
    + ' 큰 점프 곡선: 깊은 웅크림 84 → 상승 87 → 정점 90 → 하강 93). 중복 상태 0쌍');
