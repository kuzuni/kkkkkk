/* 작업 198 — 기능 체크 표. `node tools/fnchk198.js`
 *
 ...
 *
 * 198 의 성질은 «유물 소환 시작가 = 1». 실제 버튼(#rwBasin)을 눌러
 *   가격 표시 · 유물석 잔액 · 누적 횟수 · 세이브(localStorage) 가 같이 움직이는지 본다. */
...
(async () => {
  ...
  /* ① 신규 세이브로 유물 페이지 열기 — 시작가가 1 인가 */
  line('① 유물 페이지 열기', await p.evaluate(() => { S.relic = 10; openRelw(); renderRelw(); return window.__snap(); }));

  /* ② [소환] 1회 실클릭 — 1 만 빠지고 다음 가격이 2 로 */
  ...
  await p.evaluate(() => { S.relic = 1 + 2 + 3 + 2; S.cnt.sumRelic = 0; S.own = {}; renderRelw(); });
  await p.dispatchEvent('#rwBasin', 'pointerdown');
  try { await p.waitForFunction(() => typeof rwHold === 'undefined' || !rwHold, null, { timeout: 15000 }); } catch (e) {}
  await p.dispatchEvent('#rwBasin', 'pointerup');
  line('⑤ 예산 8 로 홀드', await p.evaluate(() => window.__snap()));

  /* ⑥ 유물석 0 — 시작가 1 도 못 내면 실패하고 lack 이 켜지는가 */
  line('⑥ 유물석 0 에서 클릭', await p.evaluate(() => {
  ...
  /* ⑦ 구 세이브 이어받기 — 이미 37회 소환한 유저는 38 부터 */
