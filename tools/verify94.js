 ...
 try {
  ...
  eq('소스 · 남은 리터럴 문구', lits.slice().sort().join('/'),
     /* 160 — «아레나 중단»(작업 123) 추가. 목록은 «어떤 문구가 남아 있는지»의 기록이다.
        458 — «레이드 중단» 이 이 목록에서 빠진 것은 **사라져서가 아니라** 사유 분기
        (`died ? '레이드 실패' : '레이드 중단'`)로 옮겨 갔기 때문이다. 두 문구는 바로 아래
        «비리터럴 인자 안의 문구» 가 이름으로 받아 같은 «짧게» 자로 잰다 — 여기서 빼기만 하면
        그 두 줄이 게이트 밖으로 나가 얼마든지 길어질 수 있다. */
     ['던전 중단', '레이드 시작', '부활 중...', '승급전 시작', '아레나 중단'].sort().join('/'));

  /* 458 — 인자가 «리터럴 하나» 가 아닌 자리에도 사람이 읽는 문구가 들어 있다.
     그 문구도 94 규약(짧게)의 대상이므로 따로 뽑아 같은 자로 재고, 목록으로 못 박는다. */
  const inner = calls.filter(c => !/^'[^']*'$/.test(c))
                     .reduce((a, c) => a.concat((c.match(/'[^']*'/g) || []).map(s => s.slice(1, -1))), []);
  eq('소스 · 비리터럴 인자 안의 문구', inner.slice().sort().join('/'),
     ['STAGE ', 'BOSS ', '레이드 실패', '레이드 중단'].sort().join('/'));
  eq('소스 · 비리터럴 인자 안의 문구 중 14자 초과',
     inner.filter(t => t.length > 14).join(' | ') || 'none', 'none');
