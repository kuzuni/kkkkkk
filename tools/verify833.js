...
(async () => {
  ...
  /* ── 885 3회차 신설 ─────────────────────────────────────────────────────────
     ...
     ⚑ **위 수는 «정수 걸음» 자의 값이다(932 5회차 정오표 — 이 절의 단언은 한 항도 안 바뀐다).**
        `scan885b.py` 가 그 뒤 부분 화소로 갈렸다(덩이·문턱·창은 그대로, 모서리만). 같은 문턱 90 에서
        ref 는 글리프 합 130.04 · 틈 합 **24.75 → 26.63**(6.19 ×4 였던 것이 6.28/6.30/6.33/7.72)이다 —
        옛 값은 넷 다 정확히 3 ref px 에 굳어 있었다(K 배수). **방향은 안 바뀐다**(ref 틈 > 우리 틈이 결론이었다).
        옛 수를 그대로 재현하려면 `python3 tools/scan885b.py --glyph --ref-only --int` — 자는 `verify932` §8.
  ...
  for (const c of cards) {
    ...
    /* ⚑⚑ 895 1회차 이관 — **윗줄은 그대로, 아랫줄만 36 → 31.**
       이 항의 뜻(«크기로 폭을 좇지 마라» = 380 함정)은 **윗줄** 이야기였다 — 833 3회차가
       윗줄 글리프 폭이 **이미 ref +1.5%** 임을 재 놓았으므로 거기서 fs 를 만지면 맞는 것을 틀리게 한다.
       ⚠ **아랫줄은 폭을 좇는 게 아니라 «크기 자체» 가 틀렸다** — 895 가 세운 자(`tools/scan895.py`)로
       ref 와 우리를 같은 방법으로 재니 **세 축이 같이** 지목했다(문턱 5단 부호 유지 · 카드 3장):
         두께(베이스라인 수직) 27.22 ↔ ref 23.26 = **+17.0%** ·
         길이(베이스라인 방향)  58.96 ↔ ref 52.32 = **+12.7%** ·
         검정 획              3.00 ↔ ref  2.06 = **+45.4%**
       두께비로 36 → 30.8 · 길이비로 36 → 31.6 ⇒ **31** 하나가 둘을 같이 닫는다
       (수리 뒤 두께 **−0.5%** · 길이 **−1.7%**). 폭을 좇은 것이 아니므로 380 함정이 아니다.
       ⚠ 윗줄 잔차(두께 +6.0% · 길이 +3.0%)는 **손잡이가 없다** — 글리프 «폭» 은 이미 맞아 있어서
       fs 를 내리면 [1-h]·[1-g] 가 닫아 놓은 축을 되돌린다(«좁고 높은 서체» = 895 review §ⓒ 의 서체 축). */
    ok(ln[0].fs === 46 && ln[0].lh === 48 && ln[1].fs === 31 && ln[1].lh === 40,
      `[1-j] ${c.id} font-size/line-height — 윗줄 46/48 불변(380 함정) · 아랫줄 31/40 (895 실측 — 두께·길이 두 축이 같이 지목)`,
      `${ln[0].fs}/${ln[0].lh} · ${ln[1].fs}/${ln[1].lh}`);
    /* ⚑⚑ 895 **2회차 — 1회차의 이관(8/7 → 8/5)을 되돌렸다.** 이 항은 8/7 그대로가 맞다.
       1회차는 «ref 아랫줄 획 2.06 ↔ 우리 3.00 = +45.4%» 를 근거로 7 → 5 를 썼는데,
       그 **2.06 이 자의 결함이었다** — `scan895` 의 획 걸음이 «검정 화소를 한 칸씩 세는» 정수라
       값이 언제나 정수였고, ref 는 우리보다 K=2.0628 배 작아 바닥깎기가 **우리 px 로 2.06 씩** 손해였다.
       (1회차가 적은 2.063·4.126 은 정수 **1·2 × K** 그대로다.)
       비평 **GH·GI 가 각자 다른 자로 같은 곳을 짚었고**(GI 는 숫자만 보고 «K 와 소수 둘째 자리까지 같다»,
       GH 는 «ref 의 얇은 안티에일리어스를 깎아 먹는다»), 자를 **부분 화소**로 고쳐 다시 재니
       ref 아랫줄 **3.73**(GH 3.38 · GI 3.8 사이) ↔ 5px 일 때 우리 2.67 = **−28.3%** 였다.
       ⇒ 필요한 선언은 3.73 ÷ 0.534 = **6.99 ≈ 7** — **원래 값이 맞았다.** 되돌린 뒤 −1.3%.
       ⚑ 교훈은 «7 이 맞다» 가 아니라 **«정수로 세는 자로 ref 를 재면 언제나 우리가 더 두껍게 나온다»** 다.
       ⚠ 이 항은 **선언**만 묻는다(그려진 획은 `verify895` §B 가 화소로 묻는다). */
    /* ⚑⚑ 961 이관 — **윗줄만 8 → 10. 아랫줄 7 은 그대로다.**
       위 주석의 «8/7 그대로가 맞다» 는 **아랫줄** 판정이었다(1회차의 7 → 5 과교정을 되돌린 것).
       윗줄 8 은 «자 셋이 갈려서 못 옮긴다» 로 **판정을 미뤄 둔 값**이고(895 2회차 §B [B3] 이
       그 잔차를 양성으로 등재했다), 932 8회차가 계열이 다른 넷째 자(`scan932` 질량 적분)를
       같은 광선에 얹어 갈림을 닫았다: GH «맞다»(0%) 기각 · GI −15% · ⓐ −20.3% · **ⓑ −17.3%**
       ⇒ 환산비 0.534 로 옮기면 ⓐ 10.10 · ⓑ 9.73 = **둘 다 10**.
       ⚑ 계산만으로 안 옮겼다 — `probe961` 이 선언 8~12 를 실제로 굴려 화소로 재니
       **두 자가 같은 칸 10 을 골랐다**(ⓐ −0.3% · ⓑ +2.8%). 방향을 뒤집은 것이 아니라
       «아직 못 잰 값» 을 «잰 값» 으로 갈아 끼운 것이다(333 처방 · 885 4회차 [1-l] 선례).
       ⚠ 이 항은 **선언**만 묻는다(그려진 획은 `verify895` §B·§B' 와 `verify961` 이 화소로 묻는다). */
    ok(ln[0].stroke === 10 && ln[1].stroke === 7,
      `[1-k] ${c.id} 검정 획 선언 **10**/7 — 윗줄은 961 이 8 → 10 으로 옮겼다(자 넷이 −17~−20% 로 «얇다» · 환산 둘 다 10 · probe961 스윕도 10) · 아랫줄 7 은 895 2회차가 되돌려 굳힌 값(정수 걸음 자의 바닥값 2.06 이 부분 화소로 3.73 ⇒ 6.99 ≈ 7)`,
    ...
    /* ⚑ 895 1회차 이관 — **38.8 → 39.3.** 이 상수의 뜻은 숫자가 아니라 «**그려진** 잉크 cen 이
       ref 38.5 에 앉는다» 이고, 895 가 아랫줄 글리프를 줄이자(36 → 31) 그 뜻을 지키는 **상자** 값이
       옮겨졌다 — 잉크는 상자 안에서 line-box 반여백만큼 떠 있으므로 fs 가 바뀌면 상자↔잉크 사이가 바뀐다.
       실측(`tools/scan885c.py`): 상자 38.8 을 그대로 두면 cen 38.0 · base **34.4**(−3.5%) ·
       빈 띠 **8.0**(−13.8%) 이고, **39.3** 에서 cen 38.9(+1.2%) · base 35.6(**−0.4%**) ·
       빈 띠 9.0(**−3.0%**) 로 세 자가 같이 붙는다. ⚠ 상자를 안 옮기는 것이 «무변경» 이 아니다 —
       그림이 옮겨졌으니 상자를 그대로 두는 쪽이 오히려 ref 에서 멀어진다. */
    ok(ln.length === 2 && Math.abs((ln[1].top + ln[1].lh / 2) - (ln[0].top + ln[0].lh / 2) - 39.3) <= 0.3,
      `[1-o] ${c.id} 두 줄 중심 거리 39.3 (4회차 43.75 → 7회차 38.8 → 895 39.3 — 뜻은 그대로 «잉크 cen = ref 38.5», 아랫줄 fs 가 바뀌어 상자 값이 따라왔다)`,
  ...
  for (const c of cards) {
    for (const r of c.rbs) {
      ...
      ok(c.ban ? r.utop === 45 : r.utop === 58,
        `[3-a] ${c.id} 수량 top = ${c.ban ? 45 : 58} (배너형 49 → 45 — 8회차 채점 2인이 «숫자 윗변 − 리본 하변» 에서 +3.6~4.5 로 일치 · 불릿형은 GB 에서 부호 뒤집힘이라 안 건드린다)`, `${r.utop}`);
  ...
  for (const c of cards) {
    ...
    /* ⚑ 885 10회차 신설 — 판 «폭» 자체를 못박는다. [7-d] 는 판과 수량의 **관계**만 보므로
       판이 79 로 되돌아가고 δ 가 30 으로 같이 되돌아가면 **초록인 채로 3인 일치 수리가 사라진다**.
       배너 81.6 = GC −1.9~−3.7% · GD −1.31~−3.41px · GE −1.40~−3.38px 의 가운데(≈ −2.6px) ·
       불릿 91 = 안 건드린 값(GD·GE 둘 다 «맞다»). */
    ok(Math.abs(r2.b.w - (c.ban ? 81.6 : 91)) < 0.3 && Math.abs(r2.b.w - r2.b.h) < 0.3,
      `[7-e] ${c.id} 금색 판 = ${c.ban ? '81.6' : '91'} 정사각 (배너는 10회차 3인 일치로 79 → 81.6 · 356 등방)`,
      `${p2(r2.b.w)} × ${p2(r2.b.h)}`);
    /* δ = (수량 상자 139 − 판 폭) ÷ 2 — «파생» 이라는 사실 자체를 단언한다(10회차에 실제로 안 따라왔다). */
    ok(Math.abs((139 - r2.b.w) / 2 - (r2.b.x + r2.b.w / 2 - (r2.u.x + r2.u.w / 2) + (139 - r2.b.w) / 2)) < 0.3,
      `[7-f] ${c.id} 수량 상자 δ = (139 − 판 폭) ÷ 2 — 손 상수가 아니라 **파생값**이다`,
      `${p2((139 - r2.b.w) / 2)}`);
  }
  ...
  blk('§12 우변 노치 «깊이» — ref 두 형이 **한 값**이다 (923 2회차 이관 — 옛 «두 값» 은 정수 자의 한 칸이었다)');
  /* ⚑⚑ 923 2회차가 이 절의 방향을 뒤집었다(333 처방 — 자리를 비우지 않고 반대로 단언한다).
     8회차가 «형마다 두 값» 의 근거로 든 것은 `tools/scan667.py` 의 값이다:
       ref 파랑(배너형) 28.9 / 30.9 / 30.9  (평균 30.2) ↔ ref 초록(불릿형) 33.0 / 33.0 / 33.0
     그 자는 **ref 화소를 정수로 세고 ×k(2.0628) 한다** — 위 두 값은 ref 원본에서 **15 ↔ 16 화소**,
     곧 **정확히 한 칸** 차이다. 한 칸이 우리 2.06px 이므로 이 자는 두 형을 그보다 곱게 못 가른다
     (LESSONS 895-2회차 «정수로 세는 자는 해상도가 다른 두 그림을 못 견준다»).
     **부분화소 자 셋이 독립으로 «같다» 를 냈다**(923 1회차·채점 GJ·GK):
       scan923.py 50% 교차 — ref 배너 31.40 ↔ 불릿 31.67 (Δ0.27)
       GK(곧은변을 최빈값으로) — 31.36~31.44 ↔ 31.37~31.46 (Δ**0.01**)
       GJ — 형간 차 −0.14~+0.96 이고 문턱 사다리에서 **부호가 뒤집힌다**(= 차가 없다)
     ⇒ 과녁은 두 형 **한 값 31.4**. 그릴 수 있는 것은 정수뿐이라(`.ntc` 상자 폭 d+12 가 정수로
  ...
  /* ⚑ 923 2회차 — 방향이 뒤집힌 항이다. 옛 판정은 «blD − banD > 2»(달라야 통과)였고,
     그 근거는 위 주석이 밝힌 «정수 자의 한 칸» 이다. 다시 가르려는 편집은 여기서 빨개진다. */
  ok(banD.length > 0 && blD.length > 0 && blD.every(v => Math.abs(v - banD[0]) < 0.6),
    '[12-e] 두 형의 깊이가 **같다** — 다시 갈리면 빨강 (ref 두 형 차 0.01~0.27 = 부분화소 자 셋 일치)',
    `배너 ${banD.join('/')} ↔ 불릿 ${blD.join('/')}`);
  ok(blD.every((v) => v === blD[0]) && banD.every((v) => v === banD[0]),
    '[12-f] 카드 셋이 한 값 — 카드마다 손으로 다른 값을 적으면 빨강',
    `${banD.join('/')} · ${blD.join('/')}`);
  ...
  /* 6회차 DE [A]«카드1 아트 패널이 헤더 밴드를 덮는다 — 카드2·3 은 패널 상변이 135 인데 카드1 만 10
     ...
     이 화면은 **ref 자신이 형마다 두 값인 축이 셋**이다(피치·앵커·길이 — §12).
     ⚠ 923 2회차까지는 «넷» 이라고 적혀 있었다(깊이 포함) — 그 넷째는 정수 자가 만든 것이라 접혔다.
     일러스트도 «두 값» 축 중 하나다:
  ...
  /* ⚑ 6회차까지 이 절은 «원반 팽창 text-shadow 링» 의 선언을 지켰다. 7회차가 그 자리를 **이관한다**(333 처방) —
     ...
     **거리장 오프셋 폴리곤**(`tools/starpoly885.py` — 볼록 꼭짓점 호 · 오목 꼭짓점 교점)으로 옮겼다.
     ...
     ⚠ 별의 **형**(r/R = **0.647**)은 아래 [16-h] 가 지킨다. ⚑ 9회차 이관 — 7회차의 0.668 은 «ref 를 잰 값» 이고,
     8회차 채점 2인은 **같은 자를 ref 와 우리에 같이** 돌려 «우리가 그 위에 있다»(+2.4~5.1%)를 냈다.
     ref 절대값은 아직 갈리지만(0.646~0.684) 그 판정은 안 갈린다 — 그래서 렌더 편향으로 되돌린 값을 쓴다. */
  ...
  if (star) {
    ...
    if (G.length === 10) {
      ...
      ok(Math.abs(ri / Ro - 0.647) < 0.01,
        '[16-h] 안/바깥 반지름비 = **0.647** (8회차 채점 2인이 «우리가 ref 보다 +2.4~5.1% 크다» 로 일치 — 렌더 편향을 뺀 ref 추정 GB 0.642 · GC 0.652 의 가운데. 7회차 0.668 · 6회차 0.591 · 수리 전 글리프 0.420)',
        p2(ri / Ro));
  ...
  for (const c of caps) {
    ...
    /* 8회차 신설 — 립이 «border-left» 라는 사실 자체를 못박는다. 이 항이 없으면 다음 회차가
       ...
       ⚠ 896 1회차 — **여기서 보는 것은 «두께» 뿐이다.** 립을 크림·투명으로 칠하면 이 항도
       [17-c][17-d] 도 그대로 초록이다(실측: `scratch` 자로 세 판 비교). 립의 **색**은
       `verify667` **[G8a]** 가 임자다 — 좌단·립이 그 자의 자리이므로 여기 사본을 두지 않는다. */
  ...
  ok(caps.every(c => Math.abs(c.left - caps[0].left) < 0.01),
    /* 896 1회차 — 이름표가 [17-f] 로 겹쳐 있었다(8회차가 립 항에 같은 이름을 다시 썼다).
       보고서에서 «[17-f] 가 빨갛다» 가 두 항 중 어느 쪽인지 못 가리므로 이쪽을 [17-g] 로 옮긴다 —
       885 8회차 기록이 가리키는 «신설 [17-f]» 는 위 립 항이라 그 참조는 그대로 산다. */
    '[17-g] 좌단도 네 줄 한 값 — 이 절이 좌단을 **안 건드렸다**는 증거 (896 자 갈림 자리)',
  ...
  /* ⚑ 923 2회차 — 뒤집힌 [12-e] 의 되돌림 시험이다. 이 사보타주는 **불릿만** 40 으로 밀므로
     ...
     이 항이 없으면 «두 형이 같다» 는 단언이 **무를 때도 초록**일 수 있다. */
  ok(back11.every(c => Math.abs(c.d - NDEP) >= 0.6),
    '[R11c] 그때 [12-e]«두 형이 같다» 가 빨개진다 — 불릿만 40 이 되어 배너 41 과 갈린다',
    back11.map(c => c.d).join('/'));
  ...
  const now17 = await page.evaluate(() => {
    ...
    document.querySelectorAll('.pvc>.bdg').forEach((bd) => {
      const bb = bd.getBoundingClientRect();
      const es = [...bd.querySelectorAll('i,b')];
      const ln = es.map((e) => {
        const r = e.getBoundingClientRect();
        const cs = getComputedStyle(e);
        return { cx: r.x + r.width / 2 - bb.left, cy: r.y + r.height / 2 - bb.top,
                 boxc: parseFloat(cs.top) + parseFloat(cs.lineHeight) / 2 };
      });
      if (ln.length === 2) out.push({ dx: +(ln[1].cx - ln[0].cx).toFixed(2), dy: +(ln[1].cy - ln[0].cy).toFixed(2),
                                      d: +(ln[1].boxc - ln[0].boxc).toFixed(2) });
    });
    return out;
  });
  /* ⚑ 895 1회차 이관 — 예측값을 **상수에서 빼고 선언에서 판다**([1-n] 선례).
     이 시험이 묻는 것은 «d 가 얼마인가» 가 아니라 «두 줄이 **한 덩이로** 도는가» 다 —
     d 는 [1-o] 가 따로 못박으므로 여기서 또 적으면 상수가 둘이 되어 fs 를 옮길 때마다 같이 빨개진다
     (895 1회차에 실제로 그랬다: 38.8 → 39.3 하나에 이 항이 덩달아 빨개졌다).
     ⇒ 예측은 그 자리의 선언된 상자 거리 d 로 만든다: dx = −d·sin15 · dy = d·cos15. */
  ok(now17.length > 0 && now17.every((o) => {
    const th = 15 * Math.PI / 180;
    return Math.abs(o.dx + o.d * Math.sin(th)) <= 0.4 && Math.abs(o.dy - o.d * Math.cos(th)) <= 0.4;
  }), '[R17b] 원복하면 두 줄 어긋남이 **강체 예측**(dx −d·sin15 · dy d·cos15 — d 는 선언된 상자 거리)과 같다',
    now17.map(o => `d ${o.d} ⇒ dx ${o.dx} · dy ${o.dy}`).join(' / '));
