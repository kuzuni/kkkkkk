...
(async () => {
  ...
  for (const c of cards) {
    ...
    /* ⚑ 895 1회차 이관 — **8/7 → 8/5.** 이 항의 뜻은 «letter-spacing 은 획을 안 건드린다» 였고
       그 뜻은 그대로다(885 7회차가 자간을 2.9 → 5.2 로 옮길 때 획은 한 칸도 안 움직였다).
       바뀐 것은 **아랫줄 획이 처음으로 «재어졌다»** 는 것이다 — 895 의 자로 밖으로 나온 검정을 세니
       ref 는 윗줄 4.13 ↔ 아랫줄 **2.06**(정확히 절반)인데 우리는 4.00 ↔ **3.00** 이었다(+45.4%).
       ⚠ 이 항은 **선언**만 묻는다(그려진 획은 `verify895` [C1]·[C2] 가 화소로 묻는다) —
          선언과 그림을 한 항에 섞으면 어느 쪽이 틀렸는지 못 가른다. */
    ok(ln[0].stroke === 8 && ln[1].stroke === 5,
      `[1-k] ${c.id} 검정 획 선언 8/5 — 윗줄 불변 · 아랫줄 7 → 5 (895 실측 ref 비 «윗줄의 절반» · letter-spacing 은 여전히 획을 안 건드린다)`,
