 ...
 *       지금  씬 A·B·**D(50 코스튬 [강화])** — 델타가 아직 사는 자리로 표본을 옮겼다
 *
 ...
 *   §R-c  부품 사망 — `fxDelta` 가 아무것도 안 만든다          ⇒ [8-b] 빨강
 *   §R-d  씬 C 판정 무력화 — 훈련이 골드를 안 쓴다             ⇒ [7-a] 빨강
 *   §R-e  토큰 풀림 — 코스튬 델타«만» 크기를 손으로 적는다     ⇒ [8-b] 빨강
 *   §R-f  씬 D 판정 무력화 — 코스튬 강화가 안 일어난다         ⇒ [8-a] 빨강
 *
 * ⚑ **§R-d·§R-f 가 이 이관의 핵심을 지킨다.** 그 둘이 없으면 «씬이 안 났다» 를 «폐지됐다» 로 읽는
 *   헛초록이 정확히 종전 실패문의 모양으로 되살아난다(694 §R-d 와 같은 자리).
 * ⚑ **§R-e 는 [8] 이 왜 씬을 셋 유지해야 하는지**를 못박는다 — 491 2회차가 되돌린 488 의 사고
 *   («회당 플로터 자리에만 크기를 손으로 적었다»)를 그대로 재현한다. 씬 C 를 빼고 두 씬으로 줄였으면
 *   이 주입이 **안 잡힌다**(델타 계열이 자에서 통째로 빠지므로).
 *
 * §0 은 청정 트리에서 다섯 항이 전부 초록임을 먼저 확인한다(주입 시험이 헛되지 않게).
 *
 * ⚠ 이 자는 verify58 을 자식 프로세스로 7번 돌린다 — 십수 분 걸린다.
...
function runGate(srcRel) {
  ...
  for (const ln of out.split('\n')) {
    ...
    const tag = /\[(7-a|7-b|7-c|8-a|8-b)\]/.exec(txt);
...
const INJ = {
  ...
  /* 부품 `fxDelta` 자체가 죽으면 살아 있는 계열(50 코스튬)의 표본이 사라져 [8-b] 가 빨개져야 한다. */
  'R-c': ['[8-b]', "  d.className = 'fx-plus fx-delta';",
                   "  d.className = 'fx-plus fx-delta'; if(1) return;"],
  ...
  'R-e': ['[8-b]', "  d.className = 'fx-plus fx-delta';",
                   "  d.className = 'fx-plus fx-delta'; d.style.fontSize = '30px';"],
  /* 씬 D 의 전제 — 코스튬 강화가 안 나면 «델타 0장» 이 «부품이 죽었다» 와 구별되지 않는다.
     그 세계에서 [8-a] 가 빨개져야 [8-b] 의 표본 결손을 오독하지 않는다(§R-d 의 씬 D 판). */
  'R-f': ['[8-a]', "    if(cosUpgrade(cosSel)){", "    if(false && cosUpgrade(cosSel)){"],
};
...
(async () => {
  ...
  for (const k of ['7-a', '7-b', '7-c', '8-a', '8-b']) {
