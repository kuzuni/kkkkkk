 ...
 *
 * 936-예외: [E]·[E-R4] 의 «상자» 는 플레이어 자신이 그려진 텍셀(1×1)이라 플레이어를 따라 움직인다 —
 *   상자와 표본이 **같이** 움직이므로 928 의 «바탕이 바뀐다» 가 성립하지 않고, 게다가 이 자는
 *   **실시간 프레임**(셰이크가 살아 있는 판)을 표본으로 골라야 해서 자리를 못박으면 [E-R4] 의
 *   주입 시험이 통째로 사라진다. ⇒ 못박지 않는다(936 census 의 선언된 예외).
 *
 * 검사 항목:
