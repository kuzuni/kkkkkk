...
const MEASURE = `(() => {
  ...
  return { panelH,
    ...
    /* ⚑⚑ 879 7회차 이관 — **이 두 줄이 «식을 옮겨 적은» 자였다.** 바로 아래 주석이
       «식을 옮겨 적지 않고 그려진 것에서 되잰다 — 식이 바뀌어도 이 자는 안 늙는다» 라고
       적어 두고 있는데, 정작 이 위 두 줄은 min(186, (mid.t − 516 − 174)/2) 로 **제품의 식과
       리터럴 174 를 통째로 베껴** 두고 있었다. 879 7회차가 «아래 예약» 을 174 → 182 로 바꾸자
       제품 av 는 139.10 → 135.09 가 됐는데 이 자만 139.10 을 계속 말해 [5c] 가 Δ4.0 으로
       빨개졌다 — **제품이 틀린 것이 아니라 자가 늙은 것**이다(작업 388·402·125 계열).
       ⇒ archFl 과 같은 자로 되돌린다: av = (지면선 − 격자 하변) − gof.
       (gof 는 879 5회차가 격자만 위로 옮긴 양이라 빼야 «니치» 가 된다 — 879 MEASURE 와 한 벌.) */
    av: +((rel(R('#relw .rw-floor')).t - o.grid.b) - rwGof).toFixed(2),
    arch: +(516 + 2 * ((rel(R('#relw .rw-floor')).t - o.grid.b) - rwGof)).toFixed(2),
...
(async () => {
  ...
  /* ⚑⚑ **905 — 위 끝점의 자도 같은 병이었다.** 887 은 아래 끝점만 «칠해진 행» 규약으로
     세우고 **위** 끝점은 옛 자(«잉크 위 창에서 가장 긴 밝은 가로줄»)에 두었는데, 그 자가
     두 그림에서 밑판 검은 외곽선의 **반대편**을 짚고 있었다 — 우리 캡처는 외곽선 바로 위
     (−2px · 다섯 프레임 전부), 레퍼런스는 외곽선 아래(+2px · 좌우 여백 띠가 아래로 갈수록
     어두워져 «옆보다 밝다» 가 그림자 구간에서 이긴다). **부호가 뒤집힌다 = 같은 물체가 아니다.**
     위도 아래와 같은 걷개로 재면 **ref 위 12 : 아래 9 = 0.750** 이고, 우리 위 거울이 +3 → **+0.7**
     이 된다. 대역은 같은 방법(±1 눈금)으로 0.67~0.83. 상세 `docs/review/905-안내문위끝점자.md`. */
  {
    /* ⚑⚑ 948 이관 — 과녁이 **부분 화소** 축으로 옮겨졌다(0.750 은 정수 두 개의 비였다).
       거울도 같이 옮긴다 — 위 +0.7 → **−1.34** · 아래 −3 → **−3.58**(`probe948` [5] 실측).
       거울만 옛 축에 두면 이 자는 «화소를 재는 척» 하면서 딴 축의 값을 말하게 된다.
       대역도 «±1 눈금»(정수 격자의 한 칸)에서 «위상 스윕으로 잰 자의 재현성»(±9.6%)으로 갈렸다. */
    const { REF_RATIO, BAND } = require('./target948');
    const INK_UP = -1.34, INK_DN = -3.58;        /* 화소 − 상자 (948 실측 · verify887 [5] 가 지킨다) */
    const ratio = FRAMES.map(H => +((r[H].visGap + INK_DN) / (r[H].visAbove + INK_UP)).toFixed(3));
    ok(ratio.every(v => v >= BAND[0] && v <= BAND[1]),
      `[3] 안내문 **화소** 아래/위 비가 **레퍼런스 값 ${REF_RATIO}**(대역 ${BAND[0]}~${BAND[1]}) 안 — 948 이 과녁을 정수 0.750 에서 정정했다`,
      FRAMES.map((H, i) => H + ':' + ratio[i]).join(' · ') +
      ` · ref ${REF_RATIO}(scan887 U3 부분 화소 · 정수 걸음으로는 0.750 · 옛 0.90 은 위 끝점을 2 ref px 짧게 본 값 · ` +
      '옛 1.00 은 거기에 띠 «안» 까지 겹친 값) · 상자 축 참고 ' +
       ...
       (700 §preTree 의 1회차 함정). 이름에 pid(648 규약). */
  {
    ...
    const REV = [
      ...
      /* ⚑ 905 — 되돌림의 **대상이 또 옮겨졌다**(887 이 .468 → .5 로 옮긴 것과 같은 이유).
         7회차 .5 는 이제 되돌려도 0.837 이라 새 대역(0.67~0.83) 경계에 0.007 로 붙는다 —
         «되돌리면 확실히 빨개진다» 를 못 말한다. 이 회차가 옮긴 한 자리는 **10회차 어파인
         (.4737·T + 2.57) → .4286·T + 1.21** 이므로 되돌릴 것은 그 어파인이다.
         계보는 다 남긴다: .375(5회차) · .468(6회차) · .5(7회차) · .551(887) · 어파인(10회차). */
      ['R7', '**905 의 분할**을 10회차 어파인(.4737·T+2.57)으로 되돌리면 [3] 이 대역 위로 — 과녁을 0.90 → 0.750 으로 정정한 자리',
        'var(--rw-g3)) * .4286 + 1.21px * var(--rwc,1)));', 'var(--rw-g3)) * .4737 + 2.57px * var(--rwc,1)));',
        /* ⚑ 948 — 거울과 대역이 부분 화소로 옮겨졌으므로 이 시험도 같은 축에서 판정한다. */
        (rv) => { const { BAND } = require('./target948');
                  const v = (rv[2280].visGap - 3.58) / (rv[2280].visAbove - 1.34);
                  return { bad: !(v >= BAND[0] && v <= BAND[1]),
                           got: '2280 화소 아래/위 ' + v.toFixed(3) + ' (948 0.7338 · 옛 자로는 0.913 이라 안 보였다)' }; }],
