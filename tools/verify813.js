...
(async () => {
     ...
     대역을 **0.92~1.08** 로 좁힌다(6회차의 0.72~0.95 는 «틀린 과녁을 담은 넓은 대역» 이었다). */
  {
    ...
    ok(ratio.every(v => v >= 0.92 && v <= 1.08),
      '[3] 안내문 **잉크** 아래/위 비가 **레퍼런스 값 1.00**(대역 0.92~1.08) 안 — 위·아래가 같은 푸터',
      FRAMES.map((H, i) => H + ':' + ratio[i]).join(' · ') +
      ' · ref 1.00(CR) · 1.00(CS) · 1.00(scan813c 정정) · 상자 기준 참고 ' +
       ...
       (700 §preTree 의 1회차 함정). 이름에 pid(648 규약). */
  {
    ...
    const REV = [
      ...
      ['R7', '**7회차의 .5 분할**을 6회차 .468 로 되돌리면 [3] 이 대역 아래로 — 대역을 좁혀 과녁 1.00 을 못박았다',
        'var(--rw-g3)) * .5));', 'var(--rw-g3)) * .468));',
        (rv) => { const v = rv[2280].visGap / rv[2280].visAbove;
                  return { bad: !(v >= 0.92 && v <= 1.08), got: '2280 잉크 아래/위 ' + v.toFixed(3) + ' (7회차 1.000)' }; }],
    ];
