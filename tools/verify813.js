...
(async () => {
  ...
  {
    const INK_UP = 0.7, INK_DN = -3;             /* 화소 − 상자 (905 실측 · verify887 [5] 가 지킨다) */
    const ratio = FRAMES.map(H => +((r[H].visGap + INK_DN) / (r[H].visAbove + INK_UP)).toFixed(3));
    ok(ratio.every(v => v >= 0.67 && v <= 0.83),
      '[3] 안내문 **화소** 아래/위 비가 **레퍼런스 값 0.750**(대역 0.67~0.83) 안 — 905 가 과녁을 0.90 에서 정정했다',
      FRAMES.map((H, i) => H + ':' + ratio[i]).join(' · ') +
      ' · ref 0.750(scan887 U3 · 옛 0.90 은 위 끝점을 2 ref px 짧게 본 값 · 옛 1.00 은 거기에 띠 «안» 까지 겹친 값) · 상자 축 참고 ' +
       ...
       (700 §preTree 의 1회차 함정). 이름에 pid(648 규약). */
  {
    ...
    const REV = [
      ...
      ['R7', '**905 의 분할**을 10회차 어파인(.4737·T+2.57)으로 되돌리면 [3] 이 대역 위로 — 과녁을 0.90 → 0.750 으로 정정한 자리',
        ...
        (rv) => { const v = (rv[2280].visGap - 3) / (rv[2280].visAbove + 0.7);
                  return { bad: !(v >= 0.67 && v <= 0.83), got: '2280 화소 아래/위 ' + v.toFixed(3) + ' (905 0.750 · 옛 자로는 0.913 이라 안 보였다)' }; }],
