...
/* [6] 래칫 — «2회차가 E 를 안 건드렸다». 1600 은 [E2] 가 20px 을 벽으로 옮겼으므로 대상에서
   뺀다(그 이동은 `verify700` 의 ALLOW813 에 이름과 값으로 등재돼 있다).
   ⚠ 이 값들은 **이 자 자신의 눈금**이다 — `probe813` 은 같은 자리를 604.6(2280)으로 읽는데,
     두 도구가 팝업을 여는 방법이 달라(이 자는 `S.relic = 1e9` 를 먼저 넣는다) 패널 높이가
     ~17px 다르다. 1회차부터 있던 어긋남이고 값의 **차이**가 아니라 **표본**의 차이다.
     한 자 안에서만 비교하면 되므로 여기서는 이 자의 값을 쓴다.
   ⚠ 이 넷이 «수리 전 값» 인 근거는 `verify700` [C] 다 — 긴 세 프레임에서 움직인 것은
     `cap[t]` 와 `lintel[t]` 뿐이고 `grid[t]`·`mid[t]` 는 Δ0 이라 E 는 정의상 불변이다. */
const E_PRE = { 1841: 323.0, 1920: 363.32, 2280: 613.86, 2600: 867.16 };
...
(async () => {
  ...
  /* ── [5] 쌍ⓐ — «못 닫는다» 가 아니라 «닫는 길이 전부 남의 근거를 깬다» ── */
  {
    ...
    ok(FRAMES.filter(H => H !== 1600).every(H => Math.abs(r[H].gapMid - E_PRE[H]) < 1.0),
      '[6] 긴 네 프레임의 E(격자↔수반)가 1회차 실측 그대로 — 재배분은 E 에서 뺀 것이 아니다',
      FRAMES.map(H => H + ':' + r[H].gapMid + (E_PRE[H] ? '/' + E_PRE[H] : '')).join(' · '));
  }
