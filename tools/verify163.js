...
(async () => {
  ...
  /* ★ 10회차 — 착지 팽창이 스프라이트 스쿼시와 같은 폭이다(O «+21.4% vs 스프라이트 +8.3%» · P «설명 안 되는 50px»).
     ★ 11회차 재기준 — 10회차는 이것을 `.10 * k` 라는 **연속값**으로 적어 놓고 «같은 폭» 이라고 단언했는데,
     스프라이트 쪽만 9회차에 정수 픽셀 격자로 스냅돼 있어서 **회복 구간에서만 4.0~4.2pp 벌어졌다**
     (Q «t=560 에 4.02pp = 그림자 폭 760px 기준 30.5px» · R «t≈562 에 스프라이트 1.000 / 그림자 1.0414»).
     즉 «같은 값을 두 번 적는» 방식이라 한쪽만 바뀌면 조용히 어긋난다. 이제 **한 창구(ldSqQ)** 를 둘이 공유하므로
     값이 정의상 같다 — 규칙을 «두 값이 같은가» 가 아니라 «출처가 하나인가» 로 바꿔 단언한다.
     수치 증명(air=0 구간에서 두 배율이 소수점까지 같다)은 `node tools/probe163c.js` [C] 가 맡는다. */
  ok(/function ldSqQ\(k\) \{ return Math\.round\(LD_SC \* \(1 \+ LD_SQA \* k\)\) \/ LD_SC - 1; \}/.test(SRC),
    '★ 스쿼시 양자화가 한 창구(ldSqQ)에 있다');
  ok(/var sx = 1 \+ ldSqQ\(k\);/.test(SRC) && /var sq = ldSqQ\(k\);/.test(SRC)
      && /\(1 \+ sq - \.18 \* air\)/.test(SRC),
    '★ 스프라이트와 그림자가 **같은** 양자화 값을 쓴다 (되돌려 `.10 * k` 로 적으면 회복 중 4.02pp 벌어진다)');
  /* ★ 11회차 ⓐ — 보정이 «달리는 동안» 으로 갇혀 있다. 10회차는 주석에만 «idle 에서는 0» 이라고 적고
