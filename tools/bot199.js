...
/* ⚑ 199 15회차 — 앵커를 **s640 위로** 늘렸다(660·680·700·760). 14회차까지 이 목록의 마지막
   ...
   1.75 · 1.49 · 1.96). s760 은 부분(34킬 · pump0 0.33) · s800·s1200 은 실패 프로브로 남긴다 —
...
/* ⚠ 15회차 — **먼 실패 프로브 둘(s800·s1200)을 뺐다.** 지갑을 고친 뒤 그 행들의 pump0 는
   ...
   닿을 때까지 바퀴(4000)를 다 태워 둘이 보정 시간의 **45%** 를 먹었고, 그것이
...
function writeReport(rep) {
     ...
     앵커 안이 될 때까지(또는 앵커를 늘릴 때까지) 장기 수치를 표식 없이 믿지 마라. */
  {
    ...
    if (badRows.length && okRows.length) {
      ...
      L.push(`> ⚑⚑ **보정 프로브가 닿는 화력의 끝 — 실측 좌표 s${maxAnchor} … s${firstBad.s} 사이**(11회차 · 정정7 · ⚠ **12회차 정정: 이것은 «게임의 벽» 이 아니라 «이 지갑으로 세운 캐릭터의 끝» 이다** — **15회차 정정: 그 «지갑» 이 기본값이었던 것이 자의 결함이었다** — 기본 상한을 1e33·1e12 → **1e300·1e250**, 바퀴 상한을 600 → **4000** 으로 열자 같은 손잡이 없이 s660·s680·s700 이 선다(실측 pump0 1.75·1.49·1.96 · 프런티어 s639 → s680). 지금 남은 실패는 **배정밀도 상한**(골드 예산이 1e300 에서 멎는다)이다. 같은 표의 30일 봇이 그 위를 걷는 것이 그 증거다). 유효 앵커 마지막 = **s${maxAnchor}**(pump ${lastOk.pump == null ? '—' : lastOk.pump.toFixed(2)}) · 첫 실패 프로브 = **s${firstBad.s}**(pump ${firstBad.pump == null ? '—' : firstBad.pump.toExponential(1)} — 전 축 만개 후에도 목표의 그만큼). 실패 행도 [A] 표에 «✖ 대역 밖» 으로 실려 있다(재현 경로 있음).`);
