...
/* ⚑ 27회차 — 말미 창 규약 ⓑ 의 문턱. 창 W 와 그 절반의 기울기 차가 이 값 이내면 «정상(定常)».
   실측 근거는 `probe199r27` [E]: r26-base 정상 장부에서 W3 **32.1%** · W7 **15.6%** ·
   W14 **10.0%** · W29 **50.3%** 로 **한 자릿수 골짜기가 W14 하나**다. 15% 는 그 골짜기와
   봉우리 사이의 값이고, 채택 창(W14)은 문턱에 **붙지 않는다**(여유 5.0%p — 574·709·825
   «문턱에 붙은 자» 의 반대). ⚠ 이 식의 입력에는 ④ 비가 **한 번도 안 들어간다**(기울기 둘뿐) —
   «자를 결과에 맞춘다» 가 구조적으로 불가능하다는 것이 이 상수의 정당화다. */
...
function writeReport(rep) {
  ...
  /* ⓑ' **장부 정상성** — 창 W 와 그 절반의 **지속** 기울기가 얼마나 어긋나는가(두 정책 중 나쁜 쪽).
     ...
     **분모**다(`probe863` [2]·[3]·[4] 가 커밋된 스냅 넷으로 못박는다):
       · r26 → r30 에서 대충의 **절대** 어긋남은 1,555 → 1,438 로 **줄었는데**(−7.5%)
         이 비는 10.0% → 13.2% 로 **늘었다**(+32%). 늘린 것은 말미가 아니라 분모다 —
         30회차 이관이 완전히 평평한 축(오프라인 `OFF_DIA_PM` 10→2.7)을 대충 지속 장부의
         **40% → 16%** 로 깎았고, 대충은 퀘스트(일일)을 안 돌아 그 자리를 아무도 안 메웠다.
       · 어긋남의 **89.6%가 축 하나(던전)**이고 그 축은 «추세» 가 아니라 **맥박**이다 —
  ...
  /* ⓑ **정책 대조 정상성** — 863 이 규약 자로 세운 것. 창 W 와 그 절반에서 «부지런 지속기울기 ÷
     ...
     되돌림: `rstatOf` 를 `statOf` 로 되돌리면 27~30회차 그림으로 정확히 돌아간다
       ...
       그래서 25·26 두 회차가 «W7 하나에 걸린 판정» 위에서 지렛대를 골랐다. */
    {
      ...
      if (rowsW.length) {
        ...
        L.push(`_⚑ **863 — 규약 자를 ⓑ' 장부 정상성에서 ⓑ 정책 대조 정상성으로 갈았다(문턱 ${(100 * TAIL_STAT_MAX).toFixed(0)}% 는 그대로).**`
          ...
          + ` **절대 어긋남이 줄었는데도**(대충 1,555 → 1,438) 비만 10.0% → 13.2% 로 올랐다. 어긋남의 89.6%는 축 하나(던전)이고`
