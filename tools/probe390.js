  ...
  for (const r of rows) {
    ...
    for (const k of ['tall', 'short']) {
      ...
      console.log(`    ${k.padEnd(6)} f${d.frameH}${d.shortf ? ' .shortf' : '        '} ${String(d.ov).padEnd(7)} pad ${String(d.pad ? d.pad.t : '?').padStart(3)}/${String(d.pad ? d.pad.b : '?').padStart(3)}` +
        `  상자 ${String(d.box.top).padStart(4)}..${String(d.box.bot).padStart(4)} (h${String(d.box.h).padStart(4)})  본문넘침 ${d.over === null ? '?' : d.over}`);
      if (k !== 'short') continue;
      if (typeof d.pedgeBot !== 'number' || typeof d.tabsTop !== 'number') {
        console.log('           ⚠ 기준선을 못 찾았다 — 판정 불가'); undecidable++; continue;
      }
      const tIn = d.pedgeBot - d.box.top;   /* >0 이면 HUD 를 파고들었다 */
      const bIn = d.box.bot - d.tabsTop;    /* >0 이면 탭바를 파고들었다 */
      if (tIn > 0 || bIn > 0) {
        bad++;
        console.log(`           ✖ 침범 — HUD(${d.pedgeBot}) ${tIn > 0 ? '+' + tIn + 'px' : 'ok'} · 탭바(${d.tabsTop}) ${bIn > 0 ? '+' + bIn + 'px' : 'ok'}`);
      } else {
        console.log(`           ✔ 띠 안 — HUD 여유 ${-tIn}px · 탭바 여유 ${-bIn}px`);
      }
    }
    console.log('');
  }
