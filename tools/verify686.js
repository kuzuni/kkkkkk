...
(async () => {
  ...
  /* ⚑ 769 이관 — 686 은 «가로를 안 건드렸다» 를 **좌표 사본**(632 · 340)으로 적었다. 769 가
     왼쪽 `.td` 의 과예약(392 → 236)을 걷어 그 폭을 버튼에 넘기면서 가로가 476 · 496 이 됐다.
     686 이 이 항으로 지키려던 뜻(«세로 확대가 가로 규약을 안 흔든다»)은 안 죽었으므로 과녁만
     옮긴다(333 처방): 우변 여백 26(584) 불변 · 좌변은 `.td` 우변 + 16(584 [7-b] 간격).
     폭 자체의 근거(자릿수 예산)는 `verify769` [D] 가 잰다. */
  ok(p1(G.row.w - G.tb.x2) === 26 && Math.abs(G.tb.x - (G.td.x2 + 16)) < 0.5,
    '[2-b] ★ 가로 규약 Δ0 — 우변 여백 26(584) · 좌변 = `.td` 우변 + 16(769 이후 476 · 폭 496)',
    `x ${G.tb.x} w ${G.tb.w} (.td 우변 ${G.td.x2} · 우여백 ${p1(G.row.w - G.tb.x2)})`);
