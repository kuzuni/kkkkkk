/* 작업 222 캡처 하네스 — 스킬 슬롯 해금 표시(자물쇠/[+])를 눈으로 본다.
 *   222-sheet-low.png   07 스킬 시트 · 스테이지 1 (2칸 해금)
 *   222-sheet-high.png  07 스킬 시트 · 스테이지 80 (8칸 해금)
 *   222-hud-low.png     하단 HUD 슬롯 줄 · 스테이지 1
 *   222-hud-high.png    하단 HUD 슬롯 줄 · 스테이지 80
 * 실행: node tools/cap222.js
 */
...
(async () => {
  ...
  try {
    ...
    const sheet = async (name, best) => {
      ...
      await page.screenshot({ path: path.join(OUT, '222-sheet-' + name + '.png'),
                              clip: { x: 0, y: Math.max(0, box.y - 40), width: 1080, height: box.h + 140 } });
      console.log('222-sheet-' + name + '.png');
    };
    ...
    const hud = async (name, best) => {
      ...
      await page.screenshot({ path: path.join(OUT, '222-hud-' + name + '.png'),
                              clip: { x: 0, y: Math.max(0, box.y - 20), width: 1080, height: box.h + 40 } });
      console.log('222-hud-' + name + '.png');
    };
