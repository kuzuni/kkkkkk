...
/* ⚑ 713 이관(2026-09-02 주인 정정) — **그리드 안 카드가 ref 보다 11px 위에 앉는다(의도적 이탈)**.
   ...
   **아래쪽**밖에 없다. 그런데 이 패널은 **하변이 84 버튼 줄에 묶여 있어**(패널 하변 = 버튼 상변 − 20)
   아래에 자리를 만들면 내용이 그만큼 위로 밀린다 — 산수로도 피할 수 없다:
       ...
       **이 자의 스캐너(`scan12r.py`)가 고정 창이라 못 따라왔다**(잉크 10×26 → 5×11). 잔여 739.
   ⇒ ref 축의 y 목표에 이 −11 을 명시적으로 얹는다. 값을 무르게 푼 것이 아니라 **한 상수로 적어**
     그 이상의 표류는 그대로 빨개지게 둔다. 측정표 12 §정오표에 같은 값이 적혀 있다.
   되돌리려면 `#sumw` 의 네 값(868/212/99/113)을 원복하고 여기 DY 를 0 으로 두면 된다. */
const DY = -11;
const T = {
  /* 잉크 bbox [x, y, w, h] */
...
(async () => {
  ...
  chk('카드 2행 상·하단 (x78 · 713 DY −11)', m.cap.cardrows.flat(),
