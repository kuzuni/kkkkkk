 ...
 *   [G] 89 레이아웃 회귀 — 닷을 넣어도 수반 400×216 · 라벨·코스트 알약 자리가 안 움직인다.
 *
...
(async () => {
  ...
  if (!s100.dot) {
    ...
    /* 수반 로컬 좌표(400×216 기준) — 화면은 fit() 배율을 타므로 정규화해서 되돌린다 */
    const loc = [px(q[0] * 400), px(q[1] * 216)];
  ...
  /* ══ [G] 89 레이아웃 회귀 ═══════════════════════════════════════════════ */
  ok(JSON.stringify(s100.off) === JSON.stringify([400, 216, 61, 149, 278, 97]),
    '[G] 89 회귀 — 수반 400×216 · 코스트 알약 (61,149) 278w · 라벨 top 97 불변(닷은 절대배치)',
