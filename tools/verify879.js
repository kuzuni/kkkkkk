...
const PED_GAP = 12;        /* 867 — 바 하변 ↔ **지면선**(886 정정 · 제품 `--rw-ped − --rw-mb-seat`) */
...
(async () => {
  ...
  const MB_GAP = ROW_PITCH * MB_GAP_K;   /* 39 = 26 × 1.5 — 제품의 `RW_MB_ROWG * 1.5` 와 한 벌 */
  const want = H => Math.min(1, (r[H].av / r[H].sc - PED_GAP - MB_GAP) / SHELL_H);
  ok(FRAMES.every(H => Math.abs(r[H].mbs - want(H)) < 0.01),
    '[1] ★ 배율 = min(1, (니치 av − 12 − 39) / 98) — «그려진 니치» 에서 되잰 값과 일치(JS 사본 드리프트 감지)',
    FRAMES.map(H => H + ':' + r[H].mbs.toFixed(3) + '↔' + want(H).toFixed(3)).join(' · '));

  /* ── [2] 읽힘 — 813 8회차 채점 2인의 ② 지적이 닫히는 자리 ────────────────── */
  ok(r[1600].gapUp >= ROW_PITCH * MB_GAP_K,
    '[2] ★ 1600 의 «격자 하변 ↓ 바» 가 격자 행 간의 1.5배 이상 — «격자의 한 줄»(×1.0) 로 안 읽힌다',
  ...
  /* ── [5] 867 규약이 배율과 무관하게 산다 ──────────────────────────────────
     ...
     ⚠ **879 의 항등식이 이 정정 위에서 다시 읽힌다** — 니치 av 는 그 16 을 이미 물고 있어서
       «안 덮게» 하는 순간 [2](간극 ×1.5)나 [2c](바 85%) 중 하나가 반드시 빨개진다
       (`probe886` [3] 이 갈래를 제품에 넣어서 쟀다: clear 0 → ×1.35 또는 82.7%).
       그래서 879 가 멈춘 자리(배율 0.867)는 **두 항이 동시에 초록인 유일한 자리**다.
     여기서는 값이 아니라 **이름만** 고친다(333 — 자리를 비우지 않는다). */
  ok(FRAMES.every(H => Math.abs(r[H].gapDown - PED_GAP) <= 1),
    '[5] 바 하변 ↔ **지면선**(`--rw-fl`) = 12px — 다섯 프레임 전부(원점이 하변이라 축소가 이 값을 안 건드린다 · 886 정정)',
    at('gapDown'));
  ...
  /* ── §R 되돌림 ─────────────────────────────────────────────────────────── */
  const rv = await sweep(browser, '1');
  ok(rv[1600].gapUp < ROW_PITCH * MB_GAP_K,
