 ...
 *                 에서 되잰다 ⇒ CSS 의 예산식이 바뀌면 배율이 저절로 따라온다.
 *                 ⚠ **940 정정 — 이 항은 «JS 사본 드리프트 감지» 가 아니었다.** 7회차가 제품의
 *                 «아래 예약» 을 174 → 182 로 옮겼을 때 `rwMulFit()` 의 사본만 안 따라와
 *                 1600 에서 av 139.10(제품 135.09)을 들고 있었는데 **이 항은 초록이었다** —
 *                 다섯 프레임 전부 배율이 `min()` 의 상한 1 에 붙어 있어 클램프가 둘을 같은 1 로
 *                 뭉갠다(아래 `PED_GAP` 주석이 그 사실을 이미 적고 있었다). 940 이 사본 자체를
 *                 지웠고(이제 `rwMulFit` 도 자 막대로 제품에게 묻는다), **클램프가 안 무는 자리**를
 *                 일부러 만들어 묻는 항은 `verify940` §R 이다. 여기는 `RW_MB_*` 넷을 지킨다.
...
/* ⚑ 910 이관 — 얹힘(`--rw-mb-seat`)이 4 → 0 이 되어 이 값이 12 → **16** 이다(= `--rw-ped` − 얹힘).
   `rwMulFit()` 의 `RW_MB_FL` 과 한 벌이라 여기만 늙으면 [1] 이 그 짝을 거꾸로 읽는다.
   ⚠ 다섯 프레임 전부 배율이 상한 1 에 걸려 있어 [1] 은 지금 **상한이 답을 준다** — 예산이
     4 줄었는데도(1600 1.088 → 1.047) 배율은 안 움직인다. 밸브가 실제로 무는지는 §R4 가 잰다.
   ⚑ **940 — 이 «상한이 답을 준다» 가 헛초록의 자리였다.** 같은 클램프가 `rwMulFit()` 의 늙은
     예산 사본(av 139.10 ↔ 제품 135.09)도 같이 뭉개서 [1] 이 그 갈림을 4회차 내내 못 봤다.
     사본은 940 이 지웠고, 클램프를 일부러 푸는 항은 `verify940` §R 이다. */
